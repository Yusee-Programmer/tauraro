...
2. Defining functions...");
    tauraro_value_t* sqrt_func = define_function("libm.so.6", "sqrt", "double", {"double"});
    tauraro_value_t* pow_func = define_function("libm.so.6", "pow", "double", {"double", "double"});
    tauraro_value_t* sin_func = define_function("libm.so.6", "sin", "double", {"double"});
    tauraro_value_t* cos_func = define_function("libm.so.6", "cos", "double", {"double"});
    printf("%s\n", "   Functions defined: sqrt, pow, sin, cos");
    /* unsupported statement */;
    printf("%s\n", "
3. Testing functions:");
    tauraro_value_t* result = call_function("libm.so.6", "sqrt", {16});
    printf("%p\n", /* unsupported expr */);
    tauraro_value_t* result = call_function("libm.so.6", "pow", {2, 3});
    printf("%p\n", /* unsupported expr */);
    tauraro_value_t* result = call_function("libm.so.6", "sin", {1.5708});
    printf("%p\n", /* unsupported expr */);
    /* unsupported statement */;
    printf("%s\n", "
4. Manual memory management:");
    tauraro_value_t* buffer = allocate(1024);
    printf("%s\n", "   Allocated 1KB buffer");
    /* unsupported statement */;
    printf("%s\n", "   Batch calculations:");
    for (int64_t i = 0; i < 5; i += 1) {
        tauraro_value_t* x = tauraro_float_from_int((i + 1));
        tauraro_value_t* sqrt_result = call_function("libm.so.6", "sqrt", {x});
        printf("%p\n", /* unsupported expr */);
    }
    free(buffer);
    printf("%s\n", "   Buffer freed");
...
5. Arena memory management:");
    ...
    printf("%s\n", "   Arena created");
    tauraro_value_t* temp1 = allocate(512);
    tauraro_value_t* temp2 = allocate(512);
    printf("%s\n", "   Allocated 2 buffers in arena");
    /* unsupported statement */;
    ...
    printf("%s\n", "
6. Memory statistics:");
    tauraro_value_t* stats = memory_stats();
    printf("%p\n", stats);
    /* unsupported statement */;
    printf("%s\n", "
7. Cleanup:");
