...
double process_array(void) {
    /* unsupported statement */;
    tauraro_value_t* buffer = allocate(1024);
    printf("%s\n", "Allocated 1KB buffer");
    /* unsupported statement */;
    double total = 0;
    for (int64_t i = 0; i < 100; i += 1) {
        tauraro_value_t* x = tauraro_float_from_int(i);
        /* unsupported statement */;
        tauraro_value_t* square = (x * x);
...
int64_t batch_process(void) {
    create_arena("batch_arena");
    printf("%s\n", "Arena created");
    /* unsupported statement */;
    tauraro_value_t* buf1 = allocate(256);
    tauraro_value_t* buf2 = allocate(512);
    tauraro_value_t* buf3 = allocate(1024);
    printf("%s\n", "Allocated 3 buffers");
    /* unsupported statement */;
    int64_t count = 0;
    for (int64_t i = 0; i < 50; i += 1) {
        if (((i % 2) == 0)) {
            int64_t count = (count + 1);
        }
    }
    /* unsupported statement */;
...
int64_t fibonacci(int64_t n) {
    /* unsupported statement */;
    tauraro_value_t* buffer = allocate(2048);
    if ((n <= 1)) {
        free(buffer);
        return n;
    }
...
int main(int argc, char** argv) {
    /* unsupported statement */;
    /* unsupported statement */;
    printf("%s\n", "=== Manual Memory Management with Math ===");
    /* unsupported statement */;
    printf("%s\n", "
1. Automatic memory management:");
    tauraro_value_t* result = calculate_sum(10);
    printf("%p\n", /* unsupported expr */);
    /* unsupported statement */;
    printf("%s\n", "
2. Manual memory management:");
    tauraro_value_t* sum_of_squares = process_array();
    printf("%p\n", /* unsupported expr */);
    /* unsupported statement */;
    printf("%s\n", "
3. Arena memory management:");
    tauraro_value_t* even_count = batch_process();
    printf("%p\n", /* unsupported expr */);
    /* unsupported statement */;
    printf("%s\n", "
