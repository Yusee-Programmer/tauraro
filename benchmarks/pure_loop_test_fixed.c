...
int main() {
    ...
    // Pure loop test without any imports
    arg_0 = tauraro_value_new(); arg_0->type = TAURARO_STRING; arg_0->data.str_val = strdup("Starting pure loop test...");
    ...
    if (i_iter->type == TAURARO_LIST) {
        int iter_len = i_iter->data.list_val->size;
        for (i_index = 0; i_index < iter_len; i_index++) {
            tauraro_value_t* i = i_iter->data.list_val->items[i_index];
            binop_left = total;
        ...
        for (int i_c_loop_idx = start; (step > 0) ? (i_c_loop_idx < stop) : (i_c_loop_idx > stop); i_c_loop_idx += step) {
    ...
    arg_0 = tauraro_value_new(); arg_0->type = TAURARO_STRING; arg_0->data.str_val = strdup("Loop completed!");
    temp = tauraro_print(1, (tauraro_value_t*[]){arg_0});
