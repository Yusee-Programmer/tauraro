#!/usr/bin/env python3
"""
Benchmark Runner Script (Normalized Version)
Executes all Tauraro and Python benchmarks with equivalent workloads and compares performance
"""
...
class BenchmarkRunner:
    ...
    def run_tauraro_benchmark(self, benchmark_name):
        ...
        # Try normalized benchmark first, fallback to original
        normalized_file = self.tauraro_dir / f"{benchmark_name}_benchmark_normalized.tr"
        original_file = self.tauraro_dir / f"{benchmark_name}_benchmark.tr"
        
        if normalized_file.exists():
            benchmark_file = normalized_file
        elif original_file.exists():
            benchmark_file = original_file
        else:
            print(f"Warning: Tauraro benchmark file not found for {benchmark_name}")
            return None
    ...
    def run_all_benchmarks(self):
        ...
        # Only run benchmarks that have normalized versions
        benchmarks = ["sorting", "function", "loop"]
        
        print("Starting Tauraro vs Python Benchmark Suite (Normalized)")
