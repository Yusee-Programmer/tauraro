...
#endif
// Compatibility typedef for FFI references
...
int main(int argc, char** argv) {
    ...
    double (*_ffi_func_1)(double, double); // pow
    double (*_ffi_func_0)(double); // sqrt

    /* unsupported statement */;
    printf("%s\n", "=== Simple FFI Test ===");
    /* unsupported statement */;
    printf("%s\n", "\n1. Loading math library...");
    (_ffi_lib_0 = FFI_DLOPEN("libm.so.6"), _ffi_lib_0 == NULL ? fprintf(stderr, "Failed to load library: libm.so.6\n"), NULL : (void*)_ffi_lib_0);
    printf("%s\n", "Library loaded");
    /* unsupported statement */;
    printf("%s\n", "\n2. Defining sqrt...");
    (_ffi_func_0 = (void*)FFI_DLSYM(_ffi_lib_0, "sqrt"), _ffi_func_0 == NULL ? fprintf(stderr, "Failed to load function: sqrt\n"), NULL : (void*)_ffi_func_0);
    printf("%s\n", "sqrt defined - ready to call");
    /* unsupported statement */;
    printf("%s\n", "\n3. Defining pow...");
    (_ffi_func_1 = (void*)FFI_DLSYM(_ffi_lib_0, "pow"), _ffi_func_1 == NULL ? fprintf(stderr, "Failed to load function: pow\n"), NULL : (void*)_ffi_func_1);
    printf("%s\n", "pow defined - ready to call");
    printf("%s\n", "\n=== FFI Test Complete ===");
    printf("%s\n", "FFI system successfully loaded library and defined functions!");
    return 0;
}
