...
__attribute__((malloc,returns_nonnull,hot)) EncodedFunc* EncodedFunc_init(TrStr name) {
    ...
    e->is_main = false;
    /* pass */
    ByteBuf* _cltmp_t3220 = ByteBuf_init();
...
__attribute__((hot)) EncodedFunc* encode_func(LFunc* lf) {
    ...
    while ((ppi < lf->params->len)) {
        /* pass */
        ({ TrStr _at_t3221 = (List_TrStr_get(lf->params, ppi)); _st_argreg(c, ppi, _var_disp(lf, _at_t3221)); _tr_str_release(_at_t3221); });
        /* pass */
        ppi = (ppi + 1LL);
    }
    /* pass */
    ...
    while ((bi < lf->blocks->len)) {
        ...
        while ((ii < blk->insts->len)) {
            /* pass */
            __auto_type _t3222 = (*((LInst*)List_ptr_get(blk->insts, ii)));
...
__auto_type sidx = _t3222.data.IStr.str_idx;
                /* pass */
                ByteBuf_u8(c, 72LL);
                /* pass */
                ByteBuf_u8(c, 141LL);
...
__auto_type iargs = _t3222.data.ICallInd.args;
                ...
                __auto_type acode = _t3222.data.IAsm.code;
__auto_type acons = _t3222.data.IAsm.cons;
                /* pass */
                /* pass */
...
__auto_type src = _t3222.data.IStoreB.src;
        ...
        /* pass */
        __auto_type _t3223 = blk->term;
        if (_t3223.tag == LTerm_TRetInt) {
            __auto_type rv = _t3223.data.TRetInt.v;
            /* pass */
            _emit_return(c, rv);
        } else if (_t3223.tag == LTerm_TRetVal) {
            __auto_type rvreg = _t3223.data.TRetVal.v;
            /* pass */
            ...
            /* pass */
            ByteBuf_u8(c, 195LL);
        } else if (_t3223.tag == LTerm_TRetVoid) {
            /* pass */
            _emit_return(c, 0LL);
        } else if (_t3223.tag == LTerm_TBr) {
            __auto_type target = _t3223.data.TBr.target;
            /* pass */
            ByteBuf_u8(c, 233LL);
            /* pass */
        ...
        } else if (_t3223.tag == LTerm_TCondBr) {
            __auto_type cond = _t3223.data.TCondBr.cond;
__auto_type tb = _t3223.data.TCondBr.then_b;
__auto_type eb = _t3223.data.TCondBr.else_b;
            /* pass */
