...
__attribute__((hot)) void Sema__check_spawn_nested_rc(Sema* self, HirExpr* arg_expr) {
    ...
    if (reaches) {
        /* pass */
        ({ TrStr _at_t369 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[T-7] a value crossing the thread boundary transitively holds a plain (non-atomic) reference-counted class inside '")), _tr_strz(n))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' (e.g. via a Mutex/Vec/field). Even wrapped in Shared/Sendable, that nested class's refcount is thread-local and would race across threads.\n      FIX: make the nested state 'Shared[T]' (atomic, like Rust's 'Arc'), or restructure so only one thread ever owns it."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t369); _tr_str_release(_at_t369); });
    }
}
...
__attribute__((hot)) void Sema_check_ownership_cycles(Sema* self, Program* prog) {
    ...
    while ((di < prog->decls->len)) {
        /* pass */
        __auto_type _t371 = (*((Decl*)List_ptr_get(prog->decls, di)));
        if (_t371.tag == Decl_DClass) {
            __auto_type c = _t371.data.DClass.cls;
            Sema__scan_class_ptrs(self, c);
        } else if (_t371.tag == Decl_DActor) {
            __auto_type c = _t371.data.DActor.cls;
            Sema__scan_class_ptrs(self, c);
        } else if (_t371.tag == Decl_DExtend) {
            __auto_type ms = _t371.data.DExtend.methods;
            /* pass */
            long long mi = 0LL;
            /* pass */
            while ((mi < ms->len)) {
        ...
        } else if (_t371.tag == Decl_DFunction) {
            __auto_type f = _t371.data.DFunction.func;
...
__attribute__((hot)) void Sema_append_drops_from_excl_multi(Sema* self, HirBlock* hb, long long from_idx, List_TrStr* excludes) {
    ...
    while ((idx >= from_idx)) {
        ...
        while ((di < drops->len)) {
            ...
            /* pass */
            __auto_type _t373 = (*d);
            if (_t373.tag == HirStmt_SAutoDrop) {
                __auto_type dn = _t373.data.SAutoDrop.name;
                /* pass */
                long long ei = 0LL;
                /* pass */
...
__attribute__((hot)) void Sema_collect_idents(Sema* self, HirExpr* e, List_TrStr* out) {
    ...
    } else if (_t374.tag == HirExpr_EBinOp) {
        __auto_type l = _t374.data.EBinOp.left;
__auto_type r = _t374.data.EBinOp.right;
        /* pass */
        Sema_collect_idents(self, l, out);
        /* pass */
        Sema_collect_idents(self, r, out);
    } else if (_t374.tag == HirExpr_EUnaryOp) {
        __auto_type inner = _t374.data.EUnaryOp.expr;
        Sema_collect_idents(self, inner, out);
    } else if (_t374.tag == HirExpr_ECall) {
        __auto_type callee = _t374.data.ECall.callee;
__auto_type args = _t374.data.ECall.args;
        /* pass */
    ...
    } else if (_t374.tag == HirExpr_EMethodCall) {
        __auto_type obj = _t374.data.EMethodCall.obj;
__auto_type args = _t374.data.EMethodCall.args;
        /* pass */
    ...
    } else if (_t374.tag == HirExpr_EPropAccess) {
        __auto_type obj = _t374.data.EPropAccess.obj;
        Sema_collect_idents(self, obj, out);
    } else if (_t374.tag == HirExpr_EIndex) {
        __auto_type obj = _t374.data.EIndex.obj;
__auto_type idx = _t374.data.EIndex._tr_v_index;
        /* pass */
        Sema_collect_idents(self, obj, out);
        /* pass */
        Sema_collect_idents(self, idx, out);
    } else if (_t374.tag == HirExpr_ECast) {
        __auto_type inner = _t374.data.ECast.expr;
        Sema_collect_idents(self, inner, out);
    } else if (_t374.tag == HirExpr_ETryExpr) {
        __auto_type inner = _t374.data.ETryExpr.expr;
        Sema_collect_idents(self, inner, out);
    } else if (_t374.tag == HirExpr_EIfElse) {
        __auto_type c = _t374.data.EIfElse.cond;
__auto_type t = _t374.data.EIfElse.then_e;
__auto_type f = _t374.data.EIfElse.else_e;
        /* pass */
    ...
    } else if (_t374.tag == HirExpr_EList) {
        __auto_type items = _t374.data.EList.items;
        /* pass */
        long long i = 0LL;
        /* pass */
        while ((i < items->len)) {
    ...
    } else if (_t374.tag == HirExpr_ESet) {
        __auto_type items = _t374.data.ESet.items;
        /* pass */
        long long i = 0LL;
        /* pass */
        while ((i < items->len)) {
    ...
    } else if (_t374.tag == HirExpr_ETuple) {
        __auto_type items = _t374.data.ETuple.items;
        /* pass */
        long long i = 0LL;
        /* pass */
        while ((i < items->len)) {
    ...
    } else if (_t374.tag == HirExpr_EDict) {
        __auto_type keys = _t374.data.EDict.keys;
__auto_type vals = _t374.data.EDict.vals;
        /* pass */
        long long i = 0LL;
        /* pass */
        while ((i < keys->len)) {
    ...
    } else if (_t374.tag == HirExpr_ERange) {
        __auto_type s = _t374.data.ERange.start;
__auto_type en = _t374.data.ERange.end;
        /* pass */
        Sema_collect_idents(self, s, out);
        /* pass */
        Sema_collect_idents(self, en, out);
    } else if (_t374.tag == HirExpr_EAwait) {
        __auto_type inner = _t374.data.EAwait.expr;
        Sema_collect_idents(self, inner, out);
    } else if (_t374.tag == HirExpr_EAwaitTimeout) {
        __auto_type inner = _t374.data.EAwaitTimeout.expr;
__auto_type to = _t374.data.EAwaitTimeout.timeout_ms;
        /* pass */
        Sema_collect_idents(self, inner, out);
        /* pass */
        Sema_collect_idents(self, to, out);
    } else if (_t374.tag == HirExpr_EYield) {
        __auto_type inner = _t374.data.EYield.expr;
        Sema_collect_idents(self, inner, out);
    } else if (1) {
        __auto_type _ = _t374;
        /* pass */
...
__attribute__((hot)) TrStr Sema_compute_region(Sema* self, Expr* e) {
    ...
    /* pass */
    __auto_type _t375 = (*e);
    if (_t375.tag == Expr_EBinOp) {
        __auto_type cr_op = _t375.data.EBinOp.op;
        /* pass */
        ...
        /* pass */
        return _tr_str_lit("");
    } else if (_t375.tag == Expr_EIdent) {
        __auto_type cr_nm = _t375.data.EIdent.name;
        /* pass */
    ...
    } else if (_t375.tag == Expr_ECall) {
        __auto_type cr_callee = _t375.data.ECall.callee;
__auto_type cr_args = _t375.data.ECall.args;
        /* pass */
        if ((((unsigned long long)(cr_callee)) != ((unsigned long long)(0LL)))) {
            /* pass */
            __auto_type _t376 = (*cr_callee);
            if (_t376.tag == Expr_EIdent) {
                __auto_type cr_fn = _t376.data.EIdent.name;
                /* pass */
...
__attribute__((hot)) bool Sema_field_is_borrow(Sema* self, Expr* obj, TrStr field) {
    ...
    if (_t377.tag == Expr_EIdent) {
        __auto_type fb_obj = _t377.data.EIdent.name;
        /* pass */
        if ((strcmp(_tr_strz(fb_obj), _tr_strz(_tr_str_lit("self"))) == 0)) {
            /* pass */
            TrStr _strtmp_t378 = self->current_class_name;
            _tr_str_release(cls_name);
            cls_name = _strtmp_t378;
        } else {
            /* pass */
            ...
            if (((strcmp(_tr_strz(fb_sym->name), _tr_strz(_tr_str_lit(""))) != 0) && (((unsigned long long)(fb_sym->ty)) != ((unsigned long long)(0LL))))) {
                /* pass */
                TrStr _strtmp_t379 = (*fb_sym->ty)->name;
                _tr_str_release(cls_name);
...
__auto_type args = _t381.data.ECall.args;
        ...
        /* pass */
        __auto_type _t382 = (*callee);
        if (_t382.tag == HirExpr_EIdent) {
            __auto_type _cnm = _t382.data.EIdent.name;
            /* pass */
        ...
        while ((i < args->len)) {
            ...
            if (_call_is_c_free) {
                /* pass */
                __auto_type _t383 = (*((HirExpr*)List_ptr_get(args, i)));
                if (_t383.tag == HirExpr_ECast) {
                    __auto_type _cf_inner = _t383.data.ECast.expr;
                    /* pass */
...
__auto_type mname = _t381.data.EMethodCall.method;
...
__auto_type method = _t385.data.EMethodCall.method;
__auto_type args = _t385.data.EMethodCall.args;
        ...
        if (_t387.tag == HirExpr_EIdent) {
            __auto_type _ocnm2 = _t387.data.EIdent.name;
            /* pass */
...
__attribute__((hot)) bool Sema_block_ends_in_jump(Sema* self, HirBlock* b) {
    ...
    /* pass */
    __auto_type _t388 = (*last);
    if (_t388.tag == HirStmt_SReturn) {
        __auto_type _ = _t388.data.SReturn.val;
        return true;
    } else if (_t388.tag == HirStmt_SBreak) {
        __auto_type _ = _t388.data.SBreak.val;
        return true;
    } else if (_t388.tag == HirStmt_SContinue) {
        return true;
    } else if (1) {
        __auto_type _ = _t388;
        return false;
...
__attribute__((hot)) void Sema_declare(Sema* self, TrStr name, SymbolKind kind, AstType** ty, bool is_mut) {
    ...
    if ((((strcmp(_tr_strz(name), _tr_strz(_tr_str_lit("self"))) != 0) && (strcmp(_tr_strz(name), _tr_strz(_tr_str_lit("_"))) != 0)) && (_tr_strlen(_tr_strz(name)) > 1LL))) {
        ...
        if ((strcmp(_tr_strz(cat), _tr_strz(_tr_str_lit(""))) != 0)) {
            /* pass */
            ({ TrStr _at_t392 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[N-1] '")), _tr_strz(name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is a "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(cat)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" and cannot be used as a name. Choose a different name (e.g. 'my_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("')."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t392); _tr_str_release(_at_t392); });
        } else {
            /* pass */
            ...
            if (is_toplevel_decl) {
                ...
                if ((strcmp(_tr_strz(kcat), _tr_strz(_tr_str_lit(""))) != 0)) {
                    /* pass */
                    ({ TrStr _at_t393 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[N-1] '")), _tr_strz(name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is a "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(kcat)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" and is reserved. Choose a different name (e.g. 'my_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("')."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t393); _tr_str_release(_at_t393); });
                }
            }
        }
    }
...
__attribute__((hot)) bool Sema_is_known_name(Sema* self, TrStr name) {
    ...
    /* pass */
    if (({ TrStr _wt_t394 = (_tr_str_wrap(_tr_str_slice(_tr_strz(name), 0LL, 4LL))); __auto_type _wr = (((_tr_strlen(_tr_strz(name)) >= 4LL) && (strcmp(_wt_t394.data, _tr_strz(_tr_str_lit("_tr_"))) == 0))); _tr_str_release(_wt_t394); _wr; })) {
        /* pass */
        return true;
    }
    /* pass */
...
__attribute__((hot)) TrStr Sema_type_ref_name(Sema* self, Expr* raw) {
    ...
    /* pass */
    __auto_type _t395 = (*raw);
    if (_t395.tag == Expr_EIdent) {
        __auto_type nm = _t395.data.EIdent.name;
        /* pass */
...
__attribute__((hot)) HirProgram* Sema_analyze(Sema* self, Program* prog) {
    ...
    while ((ppi < prog->decls->len)) {
        /* pass */
        __auto_type _t396 = (*((Decl*)List_ptr_get(prog->decls, ppi)));
    ...
    if (self->strict_mode) {
        ...
        while ((i < prog->decls->len)) {
            /* pass */
            __auto_type _t397 = (*((Decl*)List_ptr_get(prog->decls, i)));
            if (_t397.tag == Decl_DClass) {
                __auto_type mc = _t397.data.DClass.cls;
                /* pass */
                long long mmi = 0LL;
                /* pass */
                while ((mmi < mc->methods->len)) {
                    ...
                    if (_block_mutates_self(mm->body)) {
                        /* pass */
                        ({ TrStr _dkt_t398 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(mc->name), _tr_strz(_tr_str_lit(".")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(mm->name)); _tr_str_release(_cl); _cres; })); _tr_dict_set(self->mutating_methods, _tr_strz(_dkt_t398), true); _tr_str_release(_dkt_t398); });
                    }
                    /* pass */
                    mmi = (mmi + 1LL);
                }
            } else if (_t397.tag == Decl_DActor) {
                __auto_type ac = _t397.data.DActor.cls;
                /* pass */
                long long ami = 0LL;
                /* pass */
                while ((ami < ac->methods->len)) {
                    ...
                    if (_block_mutates_self(am->body)) {
                        /* pass */
                        ({ TrStr _dkt_t399 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(ac->name), _tr_strz(_tr_str_lit(".")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(am->name)); _tr_str_release(_cl); _cres; })); _tr_dict_set(self->mutating_methods, _tr_strz(_dkt_t399), true); _tr_str_release(_dkt_t399); });
                    }
                    /* pass */
                    ami = (ami + 1LL);
                }
            } else if (_t397.tag == Decl_DExtend) {
                __auto_type mtarget = _t397.data.DExtend.target;
__auto_type mmethods = _t397.data.DExtend.methods;
                /* pass */
                long long emi = 0LL;
                /* pass */
                while ((emi < mmethods->len)) {
                    ...
                    if (_block_mutates_self(em->body)) {
                        /* pass */
                        ({ TrStr _dkt_t400 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(mtarget), _tr_strz(_tr_str_lit(".")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(em->name)); _tr_str_release(_cl); _cres; })); _tr_dict_set(self->mutating_methods, _tr_strz(_dkt_t400), true); _tr_str_release(_dkt_t400); });
                    }
                    /* pass */
                    emi = (emi + 1LL);
                }
    ...
    while ((i < prog->decls->len)) {
        ...
        /* pass */
        __auto_type _t401 = (*d);
        if (_t401.tag == Decl_DFunction) {
            __auto_type f = _t401.data.DFunction.func;
            /* pass */
            List_ptr_append(hp->functions, Sema_lower_func(self, f));
        } else if (_t401.tag == Decl_DClass) {
            ...
            /* pass */
            List_ptr_append(hp->classes, Sema_lower_class(self, c));
        } else if (_t401.tag == Decl_DEnum) {
            __auto_type e = _t401.data.DEnum.enm;
            /* pass */
            List_ptr_append(hp->enums, Sema_lower_enum(self, e));
        } else if (_t401.tag == Decl_DInterface) {
            __auto_type i_def = _t401.data.DInterface.iface;
            /* pass */
            List_ptr_append(hp->interfaces, Sema_lower_interface(self, i_def));
        } else if (_t401.tag == Decl_DExtend) {
            __auto_type target = _t401.data.DExtend.target;
__auto_type methods = _t401.data.DExtend.methods;
            /* pass */
        ...
        } else if (_t401.tag == Decl_DTopLevelStmt) {
            __auto_type s = _t401.data.DTopLevelStmt.stmt;
            /* pass */
            List_ptr_append(hp->top_level_stmts, Sema_lower_stmt(self, s));
        } else if (_t401.tag == Decl_DExtern) {
            __auto_type functions = _t401.data.DExtern.functions;
            /* pass */
            long long ei = 0LL;
            /* pass */
        ...
        } else if (_t401.tag == Decl_DDecoratorDef) {
            __auto_type f = _t401.data.DDecoratorDef.func;
            /* pass */
        ...
        } else if (_t401.tag == Decl_DTypeAlias) {
            __auto_type alias_name = _t401.data.DTypeAlias.name;
__auto_type target_ty = _t401.data.DTypeAlias.target;
            /* pass */
...
__attribute__((hot)) void Sema_compute_return_ownership(Sema* self, HirProgram* hp) {
    ...
    while ((ci < hp->classes->len)) {
        ...
        while ((mi < c->methods->len)) {
            ...
            if (Sema__fn_ret_is_heap_class(self, m)) {
                /* pass */
                ({ TrStr _at_t402 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(c->name), _tr_strz(_tr_str_lit(".")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(m->name)); _tr_str_release(_cl); _cres; })); List_TrStr_append(keys, _at_t402); _tr_str_release(_at_t402); });
                /* pass */
    ...
    while ((ei < hp->enums->len)) {
        ...
        while ((emi < en->methods->len)) {
            ...
            if (Sema__fn_ret_is_heap_class(self, em)) {
                /* pass */
                ({ TrStr _at_t403 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(en->name), _tr_strz(_tr_str_lit(".")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(em->name)); _tr_str_release(_cl); _cres; })); List_TrStr_append(keys, _at_t403); _tr_str_release(_at_t403); });
                /* pass */
    ...
    while ((ki < keys->len)) {
        ...
        /* pass */
        ({ TrStr _dkt_t404 = (List_TrStr_get(keys, ki)); _tr_dict_set(self->fn_ret_owned, _tr_strz(_dkt_t404), kf_owned); _tr_str_release(_dkt_t404); });
        /* pass */
        ki = (ki + 1LL);
    }
    /* pass */
    ...
    while ((wi < fns->len)) {
        ...
        /* pass */
        hf2->returns_owned = ({ TrStr _dkt_t405 = (List_TrStr_get(keys, wi)); __auto_type _wr = (((bool)(uintptr_t)_tr_dict_get(self->fn_ret_owned, _tr_strz(_dkt_t405)))); _tr_str_release(_dkt_t405); _wr; });
        /* pass */
        wi = (wi + 1LL);
    }
...
__attribute__((hot)) void Sema__collect_returns_stmt(Sema* self, HirStmt* sp, List_ptr* out) {
    ...
    /* pass */
    __auto_type _t406 = (*sp);
    if (_t406.tag == HirStmt_SReturn) {
        __auto_type re = _t406.data.SReturn.val;
        /* pass */
        if ((((unsigned long long)(re)) != ((unsigned long long)(0LL)))) {
            /* pass */
            List_ptr_append(out, re);
        }
    } else if (_t406.tag == HirStmt_SIf) {
        __auto_type tb = _t406.data.SIf.then_b;
__auto_type eb = _t406.data.SIf.else_b;
        /* pass */
        Sema__collect_returns(self, tb, out);
        /* pass */
        Sema__collect_returns(self, eb, out);
    } else if (_t406.tag == HirStmt_SWhile) {
        __auto_type wb = _t406.data.SWhile.body;
        Sema__collect_returns(self, wb, out);
    } else if (_t406.tag == HirStmt_SFor) {
        __auto_type fb = _t406.data.SFor.body;
        Sema__collect_returns(self, fb, out);
    } else if (_t406.tag == HirStmt_SForUnpack) {
        __auto_type fub = _t406.data.SForUnpack.body;
        Sema__collect_returns(self, fub, out);
    } else if (_t406.tag == HirStmt_SMatch) {
        __auto_type marms = _t406.data.SMatch.arms;
        /* pass */
        long long ai = 0LL;
        /* pass */
        while ((ai < marms->len)) {
    ...
    } else if (_t406.tag == HirStmt_SUnsafe) {
        __auto_type ub = _t406.data.SUnsafe.body;
        Sema__collect_returns(self, ub, out);
    } else if (_t406.tag == HirStmt_SWith) {
        __auto_type wb2 = _t406.data.SWith.body;
        Sema__collect_returns(self, wb2, out);
    } else if (_t406.tag == HirStmt_STaskGroup) {
        __auto_type tgb = _t406.data.STaskGroup.body;
        Sema__collect_returns(self, tgb, out);
    } else if (_t406.tag == HirStmt_SGpuBlock) {
        __auto_type gb = _t406.data.SGpuBlock.body;
        Sema__collect_returns(self, gb, out);
    } else if (_t406.tag == HirStmt_SDefer) {
        __auto_type ds = _t406.data.SDefer.stmt;
        Sema__collect_returns_stmt(self, ds, out);
    } else if (_t406.tag == HirStmt_STry) {
        __auto_type tb2 = _t406.data.STry.try_body;
__auto_type catches = _t406.data.STry.catches;
__auto_type fb2 = _t406.data.STry.finally_b;
        /* pass */
...
__attribute__((hot)) bool Sema__ret_yields_owned(Sema* self, HirExpr* e) {
    ...
    if (_t407.tag == HirExpr_EIdent) {
        ...
        if (_t408.tag == HirExpr_EIdent) {
            __auto_type cn = _t408.data.EIdent.name;
            /* pass */
        ...
        __auto_type _tr_v_recv = _t407.data.EMethodCall.obj;
__auto_type m = _t407.data.EMethodCall.method;
        /* pass */
        ...
        /* pass */
        __auto_type _t409 = (*_tr_v_recv);
        if (_t409.tag == HirExpr_EIdent) {
            __auto_type rn = _t409.data.EIdent.name;
            /* pass */
            if (_tr_dict_contains(self->classes, _tr_strz(rn))) {
                /* pass */
                rt = rn;
        ...
        /* pass */
        return ({ TrStr _at_t410 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(rt), _tr_strz(_tr_str_lit(".")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(m)); _tr_str_release(_cl); _cres; })); __auto_type _wr = (Sema__owned_of(self, _at_t410)); _tr_str_release(_at_t410); _wr; });
    } else if (_t407.tag == HirExpr_EIndex) {
        return false;
    } else if (_t407.tag == HirExpr_EIfElse) {
        __auto_type a = _t407.data.EIfElse.then_e;
__auto_type b2 = _t407.data.EIfElse.else_e;
        /* pass */
...
__attribute__((hot)) void Sema_register_decl(Sema* self, Decl* d) {
    /* pass */
    __auto_type _t411 = (*d);
    if (_t411.tag == Decl_DFunction) {
        __auto_type f = _t411.data.DFunction.func;
        /* pass */
        ...
        if ((f->params->len > 0LL)) {
            ...
            if (_vp->is_variadic) {
                ...
                /* pass */
                ({ TrStr _dvt_t412 = (_tr_str_wrap(_tr_int_to_str((long long)((f->params->len - 1LL))))); _tr_dict_set(self->variadic_fns, _tr_strz(f->name), _tr_str_box(_tr_str_retain(_dvt_t412))); _tr_str_release(_dvt_t412); });
                /* pass */
                _tr_dict_set(self->variadic_elem_ty, _tr_strz(f->name), box_asttype(_velem_ty));
            }
        }
    } else if (_t411.tag == Decl_DClass) {
        __auto_type c = _t411.data.DClass.cls;
        /* pass */
    ...
    } else if (_t411.tag == Decl_DActor) {
        __auto_type c = _t411.data.DActor.cls;
        /* pass */
        Sema_declare(self, c->name, SymbolKind_make_SClass(), box_asttype(AstType_init(c->name)), false);
        /* pass */
        _tr_dict_set(self->classes, _tr_strz(c->name), _tr_obj_retain(c));
    ...
    } else if (_t411.tag == Decl_DEnum) {
        __auto_type e = _t411.data.DEnum.enm;
        /* pass */
    ...
    } else if (_t411.tag == Decl_DInterface) {
        __auto_type i = _t411.data.DInterface.iface;
        /* pass */
    ...
    } else if (_t411.tag == Decl_DExtend) {
        __auto_type target = _t411.data.DExtend.target;
__auto_type methods = _t411.data.DExtend.methods;
        /* pass */
        long long hi = 0LL;
        /* pass */
        while ((hi < methods->len)) {
    ...
    } else if (_t411.tag == Decl_DExtern) {
        __auto_type abi = _t411.data.DExtern.abi;
__auto_type functions = _t411.data.DExtern.functions;
        /* pass */
        long long ei = 0LL;
        /* pass */
        while ((ei < functions->len)) {
    ...
    } else if (_t411.tag == Decl_DDecoratorDef) {
        __auto_type f = _t411.data.DDecoratorDef.func;
        /* pass */
    ...
    } else if (_t411.tag == Decl_DTypeAlias) {
        __auto_type alias_name = _t411.data.DTypeAlias.name;
__auto_type target_ty = _t411.data.DTypeAlias.target;
        /* pass */
        ...
        if ((((unsigned long long)(target_ty)) != ((unsigned long long)(0LL)))) {
            /* pass */
            TrStr _strtmp_t414 = (*target_ty)->name;
            _tr_str_release(resolved_name);
            resolved_name = _strtmp_t414;
        }
        /* pass */
...
__attribute__((hot)) HirFunction* Sema_lower_func(Sema* self, FunctionDef* f) {
    ...
    if ((((strcmp(_tr_strz(f->name), _tr_strz(_tr_str_lit("main"))) != 0) && (strcmp(_tr_strz(f->name), _tr_strz(_tr_str_lit(""))) != 0)) && (_tr_strlen(_tr_strz(f->name)) > 1LL))) {
        ...
        if ((strcmp(_tr_strz(fn_cat), _tr_strz(_tr_str_lit(""))) != 0)) {
            /* pass */
            ({ TrStr _at_t415 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[N-1] '")), _tr_strz(f->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is a "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(fn_cat)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" and cannot be used as a function name."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t415); _tr_str_release(_at_t415); });
            ...
            if ((strcmp(_tr_strz(fn_kcat), _tr_strz(_tr_str_lit(""))) != 0)) {
                /* pass */
                ({ TrStr _at_t416 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[N-1] '")), _tr_strz(f->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is a "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(fn_kcat)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" and is reserved. Choose a different function name."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t416); _tr_str_release(_at_t416); });
            }
        }
    }
    /* pass */
    ...
    if ((self->strict_mode && (f->outlives_a->len > 0LL))) {
        ...
        while ((wbi < f->outlives_a->len)) {
            ...
            if ((!w_ok_a)) {
                /* pass */
                ({ TrStr _at_t417 = (({ TrStr _cl = (({ TrStr _cr = (List_TrStr_get(f->outlives_a, wbi)); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("[L-2] region '")), _cr.data); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' in the `where ... outlives ...` clause is not a parameter of this function."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t417); _tr_str_release(_at_t417); });
            }
            /* pass */
            if ((!w_ok_b)) {
                /* pass */
                ({ TrStr _at_t418 = (({ TrStr _cl = (({ TrStr _cr = (List_TrStr_get(f->outlives_b, wbi)); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("[L-2] region '")), _cr.data); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' in the `where ... outlives ...` clause is not a parameter of this function."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t418); _tr_str_release(_at_t418); });
            }
            /* pass */
            wbi = (wbi + 1LL);
        }
            ...
            while ((infer_i < f->params->len)) {
                ...
                if (((strcmp(_tr_strz(infer_p->name), _tr_strz(_tr_str_lit("self"))) != 0) && (((unsigned long long)(infer_p->ty)) != ((unsigned long long)(0LL))))) {
                    ...
                    if ((!Sema_is_primitive_name(self, infer_pty->name))) {
                        /* pass */
                        TrStr _strtmp_t419 = infer_p->name;
                        _tr_str_release(infer_from);
                        infer_from = _strtmp_t419;
                        /* pass */
    ...
    if ((((unsigned long long)(f->ret_ty)) != ((unsigned long long)(0LL)))) {
        ...
        while ((rri < rregs->len)) {
            ...
            if ((!rok)) {
                /* pass */
                ({ TrStr _at_t420 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[L-2] region source '")), _tr_strz(rnm))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' named in 'from' is not a parameter or a region parameter of the enclosing type.\n      FIX: name a parameter the borrow comes from, or declare 'class/enum/interface <T> from "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(rnm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(":'."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t420); _tr_str_release(_at_t420); });
            }
            /* pass */
            rri = (rri + 1LL);
            _tr_str_release(rnm);
    ...
    /* pass */
    HirBlock* _cltmp_t421 = Sema_lower_block(self, f->body);
    ...
    if (self->strict_mode) {
        ...
        while ((_bci < _bconf->len)) {
            /* pass */
            ({ TrStr _at_t422 = (({ TrStr _cl = (({ TrStr _cr = (List_TrStr_get(_bconf, _bci)); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("[B-1] ")), _cr.data); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(".\n      A place may have MANY shared `ref` borrows, OR exactly ONE exclusive `mut` borrow — never both. End one borrow's last use before the other begins."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t422); _tr_str_release(_at_t422); });
            /* pass */
            _bci = (_bci + 1LL);
        }
        /* pass */
        ...
        while ((_b3i < _b3->len)) {
            /* pass */
            ({ TrStr _at_t423 = (({ TrStr _cl = (({ TrStr _cr = (List_TrStr_get(_b3, _b3i)); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("[B-3] ")), _cr.data); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(".\n      A shared `ref T` parameter cannot be mutated — declare it `mut ref T` for an exclusive (mutable) borrow."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t423); _tr_str_release(_at_t423); });
            /* pass */
            _b3i = (_b3i + 1LL);
        }
...
__attribute__((hot)) HirClass* Sema_lower_class(Sema* self, ClassDef* c) {
        ...
        if ((((unsigned long long)(pf_f->ty)) != ((unsigned long long)(0LL)))) {
            ...
            if (((strcmp(_tr_strz(pf_ty->name), _tr_strz(_tr_str_lit("Shared"))) == 0) && (pf_ty->args->len > 0LL))) {
                ...
                if ((strcmp(_tr_strz(pf_inner->name), _tr_strz(c->name)) == 0)) {
                    /* pass */
                    ({ TrStr _at_t425 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[S-1] '")), _tr_strz(c->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' has a 'Shared["))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(c->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("]' field '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pf_f->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' - this creates a reference cycle that leaks memory.\n      FIX: Use 'Weak["))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(c->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("]' for back-references to break the cycle."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t425); _tr_str_release(_at_t425); });
                }
            }
        }
        /* pass */
    ...
    while ((_ifc_i < c->iface_names->len)) {
        ...
        if ((strcmp(_tr_strz(_ifc_nm), _tr_strz(_tr_str_lit("Sendable"))) == 0)) {
            ...
            /* pass */
            ({ TrStr _at_t426 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[I-1] Class '")), _tr_strz(c->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' declares 'implements "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_ifc_nm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' but interface '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_ifc_nm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is not defined.\n      FIX: Define 'interface "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_ifc_nm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(":' before this class, or check for typos."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t426); _tr_str_release(_at_t426); });
        } else {
            /* pass */
            ...
            while ((_im_i < _idef->methods->len)) {
                ...
                while ((_cm_i < c->methods->len)) {
                    ...
                    if ((strcmp(_tr_strz(_cmeth->name), _tr_strz(_imeth->name)) == 0)) {
                        ...
                        if ((((unsigned long long)(_imeth->ret_ty)) != ((unsigned long long)(0LL)))) {
                            /* pass */
                            TrStr _strtmp_t427 = (*_imeth->ret_ty)->name;
                        ...
                        if ((((unsigned long long)(_cmeth->ret_ty)) != ((unsigned long long)(0LL)))) {
                            /* pass */
                            TrStr _strtmp_t428 = (*_cmeth->ret_ty)->name;
                        ...
                        if (((((!_iret_is_generic) && (strcmp(_tr_strz(_iret), _tr_strz(_tr_str_lit("void"))) != 0)) && (strcmp(_tr_strz(_iret), _tr_strz(_tr_str_lit(""))) != 0)) && (strcmp(_tr_strz(_cret), _tr_strz(_iret)) != 0))) {
                            /* pass */
                            ({ TrStr _at_t429 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[I-3] Class '")), _tr_strz(c->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("': method '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_cmeth->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' returns '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_cret)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' but interface '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_ifc_nm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' declares '-> "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_iret)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("'.\n      FIX: Change return type to '-> "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_iret)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("'."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t429); _tr_str_release(_at_t429); });
                        }
                        /* pass */
                        ...
                        if ((_ip_cnt != _cp_cnt)) {
                            /* pass */
                            ({ TrStr _at_t430 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[I-3] Class '")), _tr_strz(c->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("': method '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_cmeth->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' has "))); _tr_str_release(_cl); _cres; })); TrStr _cr = (_tr_str_wrap(_tr_int_to_str((long long)(_cp_cnt)))); TrStr _cres = _tr_strx_concat(_cl.data, _cr.data); _tr_str_release(_cl); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" parameter(s) but interface '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_ifc_nm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' requires "))); _tr_str_release(_cl); _cres; })); TrStr _cr = (_tr_str_wrap(_tr_int_to_str((long long)(_ip_cnt)))); TrStr _cres = _tr_strx_concat(_cl.data, _cr.data); _tr_str_release(_cl); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(".\n      FIX: Match the parameter list in '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_ifc_nm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' exactly."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t430); _tr_str_release(_at_t430); });
                        }
                    }
                    /* pass */
                    _cm_i = (_cm_i + 1LL);
                ...
                if ((!_found)) {
                    ...
                    while ((_pi < _imeth->params->len)) {
                        ...
                        if ((strcmp(_tr_strz(_p->name), _tr_strz(_tr_str_lit("self"))) != 0)) {
                            ...
                            if ((((unsigned long long)(_p->ty)) != ((unsigned long long)(0LL)))) {
                                /* pass */
                                TrStr _strtmp_t431 = (*_p->ty)->name;
                                _tr_str_release(_pty);
                                _pty = _strtmp_t431;
                            }
                            /* pass */
                            TrStr _strtmp_t432 = ({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_sig), _tr_strz(_tr_str_lit(", ")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_p->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(": "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_pty)); _tr_str_release(_cl); _cres; });
                    ...
                    if ((((unsigned long long)(_imeth->ret_ty)) != ((unsigned long long)(0LL)))) {
                        /* pass */
                        TrStr _strtmp_t433 = (*_imeth->ret_ty)->name;
                        _tr_str_release(_iret2);
                        _iret2 = _strtmp_t433;
                    }
                    /* pass */
                    TrStr _strtmp_t434 = ({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_sig), _tr_strz(_tr_str_lit(") -> ")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_iret2)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(":"))); _tr_str_release(_cl); _cres; });
                    _tr_str_release(_sig);
                    _sig = _strtmp_t434;
                    /* pass */
                    ({ TrStr _at_t435 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[I-2] Class '")), _tr_strz(c->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' implements '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_ifc_nm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' but is missing method '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_imeth->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("'.\n      FIX: Add to 'extend "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(c->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(":'  "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_sig)); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t435); _tr_str_release(_at_t435); });
                    _tr_str_release(_sig);
                    _tr_str_release(_iret2);
                }
                /* pass */
...
__attribute__((hot)) void Sema_apply_escape_marks(Sema* self, HirStmt* _hs) {
    ...
    if (_t437.tag == HirStmt_SExpr) {
        __auto_type _es_e = _t437.data.SExpr.expr;
        /* pass */
        Sema_mark_escaped_str_args(self, _es_e);
        /* pass */
        Sema_mark_escaped_coll_args(self, _es_e);
    } else if (_t437.tag == HirStmt_SLet) {
        __auto_type _sl_v = _t437.data.SLet.val;
        /* pass */
    ...
    } else if (_t437.tag == HirStmt_SAssign) {
        __auto_type _sa_t = _t437.data.SAssign.target;
__auto_type _sa_v = _t437.data.SAssign.val;
        /* pass */
    ...
    } else if (_t437.tag == HirStmt_SReturn) {
        __auto_type _sr_e = _t437.data.SReturn.val;
        /* pass */
    ...
    } else if (_t437.tag == HirStmt_SIf) {
        __auto_type _si_c = _t437.data.SIf.cond;
        /* pass */
        Sema_mark_escaped_str_args(self, _si_c);
        /* pass */
        Sema_mark_escaped_coll_args(self, _si_c);
    } else if (_t437.tag == HirStmt_SWhile) {
        __auto_type _sw_c = _t437.data.SWhile.cond;
        /* pass */
        Sema_mark_escaped_str_args(self, _sw_c);
        /* pass */
        Sema_mark_escaped_coll_args(self, _sw_c);
    } else if (_t437.tag == HirStmt_SFor) {
        __auto_type _sf_iter = _t437.data.SFor.iter;
        /* pass */
        Sema_mark_escaped_coll_args(self, _sf_iter);
    } else if (_t437.tag == HirStmt_SForUnpack) {
        __auto_type _sfu_iter = _t437.data.SForUnpack.iter;
        /* pass */
...
__attribute__((hot)) HirBlock* Sema_lower_block(Sema* self, Block* b) {
    ...
    while ((i < b->stmts->len)) {
        ...
        if ((((unsigned long long)(orig_ptr)) != ((unsigned long long)(0LL)))) {
            /* pass */
            __auto_type _t438 = (*orig_ptr);
            if (_t438.tag == Stmt_SReturn) {
                __auto_type _ = _t438.data.SReturn.val;
                /* pass */
                List_TrStr* ret_excl = (void*)List_TrStr_new();
                /* pass */
                __auto_type _t439 = (*_hs);
                if (_t439.tag == HirStmt_SReturn) {
                    __auto_type lowered_ret = _t439.data.SReturn.val;
                    /* pass */
                ...
                __auto_type _ = _t438.data.SBreak.val;
                /* pass */
...
__attribute__((hot)) HirStmt* Sema_lower_stmt(Sema* self, Stmt* s_ptr) {
    ...
    /* pass */
    __auto_type _t440 = s;
    if (_t440.tag == Stmt_SExpr) {
        __auto_type e = _t440.data.SExpr.expr;
        /* pass */
        ...
        if ((((unsigned long long)(e)) != ((unsigned long long)(0LL)))) {
            /* pass */
            __auto_type _t441 = (*e);
            if (_t441.tag == Expr_ECall) {
                __auto_type callee = _t441.data.ECall.callee;
__auto_type args = _t441.data.ECall.args;
                /* pass */
                if ((((unsigned long long)(callee)) != ((unsigned long long)(0LL)))) {
                    /* pass */
                    __auto_type _t442 = (*callee);
                    if (_t442.tag == Expr_EIdent) {
                        __auto_type fn_name = _t442.data.EIdent.name;
                        /* pass */
                        ...
                        if (((fn_sym->kind.tag == SymbolKind_make_SFunction().tag) && (strcmp(_tr_strz((*fn_sym->ty)->name), _tr_strz(_tr_str_lit("Result"))) == 0))) {
                            /* pass */
                            ({ TrStr _at_t443 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[T-4] '")), _tr_strz(fn_name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("()' returns a Result and its error must be handled. FIX: Assign the result and match on it, use '?' to propagate, or '_ = "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(fn_name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("(...)' to explicitly discard."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t443); _tr_str_release(_at_t443); });
                        }
                    } else if (1) {
                        __auto_type _ = _t442;
                        /* pass */
        ...
        /* pass */
        return h_s_expr;
    } else if (_t440.tag == Stmt_SReturn) {
        __auto_type e = _t440.data.SReturn.val;
        /* pass */
        if (((self->strict_mode && self->current_func_ret_borrow_str) && (((unsigned long long)(e)) != ((unsigned long long)(0LL))))) {
            /* pass */
            __auto_type _t444 = (*e);
            if (_t444.tag == Expr_EBinOp) {
                __auto_type l3_op = _t444.data.EBinOp.op;
                /* pass */
                if ((strcmp(_tr_strz(l3_op), _tr_strz(_tr_str_lit("+"))) == 0)) {
                    /* pass */
                    ({ TrStr _at_t445 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[L-3] borrow-returning function returns a freshly-built string (owned), not a borrow of region '")), _tr_strz(self->current_func_ret_from))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("'.\n      FIX: return a slice/view of the region source, or change the return type to a plain owned 'str'."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t445); _tr_str_release(_at_t445); });
                }
            } else if (_t444.tag == Expr_EIdent) {
                __auto_type l3_nm = _t444.data.EIdent.name;
                /* pass */
                ...
                if ((strcmp(_tr_strz(l3_sym->name), _tr_strz(_tr_str_lit(""))) != 0)) {
                    ...
                    if (l3_sym->is_param) {
                        /* pass */
                        TrStr _strtmp_t446 = _tr_str_retain(l3_nm);
                        _tr_str_release(l3_eff);
                        l3_eff = _strtmp_t446;
                    } else {
                        /* pass */
                        TrStr _strtmp_t447 = l3_sym->borrows_region;
                        _tr_str_release(l3_eff);
                        l3_eff = _strtmp_t447;
                    }
                    /* pass */
                    if ((strcmp(_tr_strz(l3_eff), _tr_strz(_tr_str_lit("@owned"))) == 0)) {
                        /* pass */
                        ({ TrStr _at_t448 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[L-3] borrow-returning function returns '")), _tr_strz(l3_nm))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("', a freshly-built string (owned), not a borrow of region '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(self->current_func_ret_from)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("'.\n      FIX: return a slice/view of the region source, or change the return type to a plain owned 'str'."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t448); _tr_str_release(_at_t448); });
                        ...
                        if ((!l3_in)) {
                            ...
                            while ((l3di < self->current_func_ret_regions->len)) {
                                /* pass */
                                if (({ TrStr _at_t449 = (List_TrStr_get(self->current_func_ret_regions, l3di)); __auto_type _wr = (Sema_region_outlives(self, l3_eff, _at_t449)); _tr_str_release(_at_t449); _wr; })) {
                                    /* pass */
                                    l3_in = true;
                                }
                                /* pass */
        ...
        if ((((((unsigned long long)(e)) != ((unsigned long long)(0LL))) && (!self->in_unsafe)) && (strcmp(_tr_strz(self->current_func_ret_from), _tr_strz(_tr_str_lit(""))) == 0))) {
            ...
            if (_t451.tag == Expr_EIdent) {
                __auto_type ret_name = _t451.data.EIdent.name;
                /* pass */
                ...
                if (((strcmp(_tr_strz(ret_sym->name), _tr_strz(_tr_str_lit(""))) != 0) && (ret_sym->ptr_region == 0LL))) {
                    ...
                    if ((((unsigned long long)(ret_sym->ty)) != ((unsigned long long)(0LL)))) {
                        ...
                        if ((strcmp(_tr_strz((*ret_sym->ty)->name), _tr_strz(_tr_str_lit("Pointer"))) == 0)) {
                            /* pass */
                            ({ TrStr _at_t452 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[L-1] '")), _tr_strz(ret_name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is a local Pointer that may not outlive this function call. Returning it is unsafe.\n      FIX: Annotate the return type with 'from <param>' if the pointer borrows from a parameter, or wrap the allocation in 'unsafe:' if it is heap-allocated."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t452); _tr_str_release(_at_t452); });
                        }
                    }
                }
            } else if (1) {
    ...
    } else if (_t440.tag == Stmt_SLet) {
        __auto_type name = _t440.data.SLet.name;
__auto_type ownership = _t440.data.SLet.ownership;
__auto_type is_mut = _t440.data.SLet.is_mut;
__auto_type is_const = _t440.data.SLet.is_const;
__auto_type is_shared = _t440.data.SLet.is_shared;
__auto_type ty_ptr = _t440.data.SLet.ty;
__auto_type val_ptr = _t440.data.SLet.val;
        /* pass */
        if (((((unsigned long long)(ty_ptr)) != ((unsigned long long)(0LL))) && (((unsigned long long)(val_ptr)) != ((unsigned long long)(0LL))))) {
            ...
            if (_t453.tag == Expr_ELitNone) {
                ...
                if ((((((strcmp(_tr_strz(m7_ty_name), _tr_strz(_tr_str_lit("Option"))) != 0) && (strcmp(_tr_strz(m7_ty_name), _tr_strz(_tr_str_lit("None"))) != 0)) && (strcmp(_tr_strz(m7_ty_name), _tr_strz(_tr_str_lit("void"))) != 0)) && (strcmp(_tr_strz(m7_ty_name), _tr_strz(_tr_str_lit(""))) != 0)) && (strcmp(_tr_strz(m7_ty_name), _tr_strz(_tr_str_lit("Pointer"))) != 0))) {
                    /* pass */
                    ({ TrStr _at_t454 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[M-7] Cannot assign 'none' to '")), _tr_strz(name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' which has type '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(m7_ty_name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("'. Only Option[T] can hold 'none'. FIX: Use 'Option["))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(m7_ty_name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("]' as the type, or give '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' a real initial value."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t454); _tr_str_release(_at_t454); });
                }
            } else if (1) {
                __auto_type _ = _t453;
                /* pass */
        ...
        if (((((unsigned long long)(val_ptr)) != ((unsigned long long)(0LL))) && (!is_shared))) {
            ...
            if (_t455.tag == Expr_EIdent) {
                __auto_type m1_src = _t455.data.EIdent.name;
                /* pass */
        ...
        if ((((unsigned long long)(val_ptr)) != ((unsigned long long)(0LL)))) {
            /* pass */
            ({ TrStr _at_t456 = (Sema_compute_region(self, val_ptr)); Sema_set_borrows_region(self, name, _at_t456); _tr_str_release(_at_t456); });
        }
        /* pass */
        if ((((unsigned long long)(val_ptr)) != ((unsigned long long)(0LL)))) {
            /* pass */
            ...
            if (_t457.tag == Expr_ECall) {
                __auto_type ce_callee = _t457.data.ECall.callee;
__auto_type ce_args = _t457.data.ECall.args;
                /* pass */
                if ((((unsigned long long)(ce_callee)) != ((unsigned long long)(0LL)))) {
                    /* pass */
                    __auto_type _t458 = (*ce_callee);
                    if (_t458.tag == Expr_EIdent) {
                        __auto_type ce_fn = _t458.data.EIdent.name;
                        /* pass */
                        ...
                        if (((strcmp(_tr_strz(ce_sym->name), _tr_strz(_tr_str_lit(""))) != 0) && (((unsigned long long)(ce_sym->ty)) != ((unsigned long long)(0LL))))) {
                            ...
                            if ((((ce_ret->is_borrow || (ce_ret->from_regions->len > 0LL)) && (ce_ret->from_index >= 0LL)) && (ce_ret->from_index < ce_args->len))) {
                                /* pass */
                                __auto_type _t459 = (*((Expr*)List_ptr_get(ce_args, ce_ret->from_index)));
                                if (_t459.tag == Expr_EIdent) {
                                    __auto_type ce_src = _t459.data.EIdent.name;
                                    /* pass */
        ...
        if (((((unsigned long long)(val_ptr)) != ((unsigned long long)(0LL))) && (strcmp(_tr_strz(ty->name), _tr_strz(_tr_str_lit("Pointer"))) == 0))) {
            ...
            bool heap_rhs = false;
            /* pass */
            __auto_type _t460 = (*val_ptr);
            if (_t460.tag == Expr_ECall) {
                heap_rhs = true;
            } else if (_t460.tag == Expr_EMethodCall) {
                heap_rhs = true;
            } else if (1) {
                __auto_type _ = _t460;
                /* pass */
        ...
        if ((((unsigned long long)(val_ptr)) != ((unsigned long long)(0LL)))) {
            ...
            if (_t461.tag == Expr_EMethodCall) {
                __auto_type pc_obj = _t461.data.EMethodCall.obj;
__auto_type pc_m = _t461.data.EMethodCall.method;
                /* pass */
                TrStr _strtmp_t462 = _tr_str_retain(pc_m);
                _tr_str_release(pc_meth);
                pc_meth = _strtmp_t462;
                /* pass */
                __auto_type _t463 = (*pc_obj);
                if (_t463.tag == Expr_EIdent) {
                    __auto_type pc_src = _t463.data.EIdent.name;
                    TrStr _strtmp_t464 = _tr_str_retain(pc_src);
            ...
            } else if (_t461.tag == Expr_EIndex) {
                __auto_type pc_iobj = _t461.data.EIndex.obj;
                /* pass */
                TrStr _strtmp_t465 = _tr_str_lit("get");
                _tr_str_release(pc_meth);
                pc_meth = _strtmp_t465;
                /* pass */
                __auto_type _t466 = (*pc_iobj);
                if (_t466.tag == Expr_EIdent) {
                    __auto_type pc_isrc = _t466.data.EIdent.name;
                    TrStr _strtmp_t467 = _tr_str_retain(pc_isrc);
                    _tr_str_release(pc_cont_nm);
                    pc_cont_nm = _strtmp_t467;
                } else if (1) {
                    __auto_type _ = _t466;
                    /* pass */
                }
    ...
    } else if (_t440.tag == Stmt_SAssign) {
        __auto_type target = _t440.data.SAssign.target;
__auto_type val = _t440.data.SAssign.val;
        /* pass */
        ...
        if ((self->strict_mode && (((unsigned long long)(target)) != ((unsigned long long)(0LL))))) {
            ...
            if (_t468.tag == Expr_EPropAccess) {
                __auto_type l5_obj = _t468.data.EPropAccess.obj;
__auto_type l5_field = _t468.data.EPropAccess.prop;
                /* pass */
                if (((strcmp(_tr_strz(Sema_compute_region(self, val)), _tr_strz(_tr_str_lit("@owned"))) == 0) && Sema_field_is_borrow(self, l5_obj, l5_field))) {
                    /* pass */
                    ({ TrStr _at_t469 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[L-5] storing a freshly-built (owned) string into the borrow field '")), _tr_strz(l5_field))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' (declared 'ref').\n      FIX: store a borrow of the field's region, or make the field a plain owned 'str'."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t469); _tr_str_release(_at_t469); });
                }
            } else if (1) {
                __auto_type _ = _t468;
                /* pass */
        ...
        if ((((unsigned long long)(target)) != ((unsigned long long)(0LL)))) {
            ...
            if (_t470.tag == Expr_EIdent) {
                __auto_type sa_decl_name = _t470.data.EIdent.name;
                /* pass */
                ({ TrStr _at_t471 = (Sema_compute_region(self, val)); Sema_set_borrows_region(self, sa_decl_name, _at_t471); _tr_str_release(_at_t471); });
                /* pass */
                ...
                if ((strcmp(_tr_strz(sa_existing->name), _tr_strz(_tr_str_lit(""))) == 0)) {
                    ...
                    /* pass */
                    ({ TrStr _at_t472 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[M-8] Cannot assign to '")), _tr_strz(sa_decl_name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' a second time because it is immutable.\n      FIX: Declare it as 'mut "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(sa_decl_name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" = ...' if it needs to change."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t472); _tr_str_release(_at_t472); });
                }
            } else if (1) {
                __auto_type _ = _t470;
                /* pass */
        ...
        if ((((unsigned long long)(val)) != ((unsigned long long)(0LL)))) {
            /* pass */
            __auto_type _t473 = (*val);
            if (_t473.tag == Expr_EIdent) {
                __auto_type sa_src = _t473.data.EIdent.name;
                /* pass */
        ...
        if ((((unsigned long long)(target)) != ((unsigned long long)(0LL)))) {
            /* pass */
            __auto_type _t474 = (*target);
            if (_t474.tag == Expr_EIdent) {
                __auto_type pd_tgt = _t474.data.EIdent.name;
        ...
        if ((((unsigned long long)(target)) != ((unsigned long long)(0LL)))) {
            ...
            if (_t475.tag == Expr_EIdent) {
                __auto_type pc_tgt = _t475.data.EIdent.name;
    ...
    } else if (_t440.tag == Stmt_SIf) {
        __auto_type cond = _t440.data.SIf.cond;
__auto_type then_b = _t440.data.SIf.then_b;
__auto_type elifs = _t440.data.SIf.elifs;
__auto_type else_b = _t440.data.SIf.else_b;
        /* pass */
        if ((((unsigned long long)(cond)) != ((unsigned long long)(0LL)))) {
            /* pass */
            __auto_type _t476 = (*cond);
            if (_t476.tag == Expr_EIdent) {
                __auto_type t5_name = _t476.data.EIdent.name;
                /* pass */
        ...
        while ((si_uti < si_then_moved->len)) {
            /* pass */
            ({ TrStr _at_t478 = (List_TrStr_get(si_then_moved, si_uti)); Sema_unmark_moved(self, _at_t478); _tr_str_release(_at_t478); });
            /* pass */
            si_uti = (si_uti + 1LL);
        }
        /* pass */
        ...
        while ((si_uii < si_then_inited->len)) {
            /* pass */
            ({ TrStr _at_t479 = (List_TrStr_get(si_then_inited, si_uii)); Sema_unmark_init(self, _at_t479); _tr_str_release(_at_t479); });
            /* pass */
            si_uii = (si_uii + 1LL);
        }
        /* pass */
        ...
        if ((elifs->len > 0LL)) {
            ...
            while ((k >= 0LL)) {
                ...
                /* pass */
                HirBlock* _cltmp_t480 = _tr_obj_retain(nested);
            ...
            /* pass */
            HirBlock* _cltmp_t481 = _tr_obj_retain(chain);
            _tr_obj_release(helse, _trdrop_HirBlock);
            helse = _cltmp_t481;
        } else {
            /* pass */
            HirBlock* _cltmp_t482 = Sema_lower_block(self, else_b);
            _tr_obj_release(helse, _trdrop_HirBlock);
            helse = _cltmp_t482;
        }
        /* pass */
        ...
        while ((si_uei < si_else_moved->len)) {
            /* pass */
            ({ TrStr _at_t483 = (List_TrStr_get(si_else_moved, si_uei)); Sema_unmark_moved(self, _at_t483); _tr_str_release(_at_t483); });
            /* pass */
            si_uei = (si_uei + 1LL);
        }
        /* pass */
        ...
        while ((si_uei2 < si_else_inited->len)) {
            /* pass */
            ({ TrStr _at_t484 = (List_TrStr_get(si_else_inited, si_uei2)); Sema_unmark_init(self, _at_t484); _tr_str_release(_at_t484); });
            /* pass */
            si_uei2 = (si_uei2 + 1LL);
        }
        /* pass */
        ...
        if ((si_then_jumps && (!si_else_jumps))) {
            ...
            while ((si_ei0 < si_else_moved->len)) {
                /* pass */
                ({ TrStr _at_t485 = (List_TrStr_get(si_else_moved, si_ei0)); Sema_mark_moved(self, _at_t485); _tr_str_release(_at_t485); });
                /* pass */
                si_ei0 = (si_ei0 + 1LL);
            }
            /* pass */
            ...
            while ((si_ini0 < si_else_inited->len)) {
                /* pass */
                ({ TrStr _at_t486 = (List_TrStr_get(si_else_inited, si_ini0)); Sema_mark_init(self, _at_t486); _tr_str_release(_at_t486); });
                /* pass */
                si_ini0 = (si_ini0 + 1LL);
            }
            ...
            while ((si_mi0 < si_then_moved->len)) {
                /* pass */
                ({ TrStr _at_t487 = (List_TrStr_get(si_then_moved, si_mi0)); Sema_mark_moved(self, _at_t487); _tr_str_release(_at_t487); });
                /* pass */
                si_mi0 = (si_mi0 + 1LL);
            }
            /* pass */
            ...
            while ((si_ti0 < si_then_inited->len)) {
                /* pass */
                ({ TrStr _at_t488 = (List_TrStr_get(si_then_inited, si_ti0)); Sema_mark_init(self, _at_t488); _tr_str_release(_at_t488); });
                /* pass */
                si_ti0 = (si_ti0 + 1LL);
            }
    ...
    } else if (_t440.tag == Stmt_SWhile) {
        __auto_type cond = _t440.data.SWhile.cond;
__auto_type body = _t440.data.SWhile.body;
__auto_type decorators = _t440.data.SWhile.decorators;
        /* pass */
        ...
        while ((sw_mi < sw_loop_moved->len)) {
            /* pass */
            ({ TrStr _at_t489 = (List_TrStr_get(sw_loop_moved, sw_mi)); Sema_unmark_moved(self, _at_t489); _tr_str_release(_at_t489); });
            /* pass */
            ({ TrStr _at_t490 = (List_TrStr_get(sw_loop_moved, sw_mi)); Sema_mark_maybe_moved(self, _at_t490); _tr_str_release(_at_t490); });
            /* pass */
            sw_mi = (sw_mi + 1LL);
        }
        /* pass */
        ...
        while ((sw_ii < sw_loop_inited->len)) {
            /* pass */
            ({ TrStr _at_t491 = (List_TrStr_get(sw_loop_inited, sw_ii)); Sema_unmark_init(self, _at_t491); _tr_str_release(_at_t491); });
            /* pass */
            ({ TrStr _at_t492 = (List_TrStr_get(sw_loop_inited, sw_ii)); Sema_mark_maybe_init(self, _at_t492); _tr_str_release(_at_t492); });
            /* pass */
            sw_ii = (sw_ii + 1LL);
        }
        /* pass */
    ...
    } else if (_t440.tag == Stmt_SFor) {
        __auto_type var = _t440.data.SFor.var;
__auto_type iter = _t440.data.SFor.iter;
__auto_type body = _t440.data.SFor.body;
__auto_type decorators = _t440.data.SFor.decorators;
__auto_type for_is_ref = _t440.data.SFor.is_ref;
        /* pass */
        Sema_enter_scope(self);
        /* pass */
        ...
        if (for_is_ref) {
            /* pass */
            __auto_type _t493 = (*iter);
            if (_t493.tag == Expr_EIdent) {
                __auto_type for_coll = _t493.data.EIdent.name;
                /* pass */
        ...
        while ((sf_mi < sf_loop_moved->len)) {
            /* pass */
            ({ TrStr _at_t494 = (List_TrStr_get(sf_loop_moved, sf_mi)); Sema_unmark_moved(self, _at_t494); _tr_str_release(_at_t494); });
            /* pass */
            ({ TrStr _at_t495 = (List_TrStr_get(sf_loop_moved, sf_mi)); Sema_mark_maybe_moved(self, _at_t495); _tr_str_release(_at_t495); });
            /* pass */
            sf_mi = (sf_mi + 1LL);
        }
        /* pass */
        ...
        while ((sf_ii < sf_loop_inited->len)) {
            /* pass */
            ({ TrStr _at_t496 = (List_TrStr_get(sf_loop_inited, sf_ii)); Sema_unmark_init(self, _at_t496); _tr_str_release(_at_t496); });
            /* pass */
            ({ TrStr _at_t497 = (List_TrStr_get(sf_loop_inited, sf_ii)); Sema_mark_maybe_init(self, _at_t497); _tr_str_release(_at_t497); });
            /* pass */
            sf_ii = (sf_ii + 1LL);
        }
        /* pass */
        ...
        return hstmt;
    } else if (_t440.tag == Stmt_SForUnpack) {
        __auto_type vars = _t440.data.SForUnpack.vars;
__auto_type iter = _t440.data.SForUnpack.iter;
__auto_type body = _t440.data.SForUnpack.body;
        /* pass */
        Sema_enter_scope(self);
        /* pass */
        ...
        /* pass */
        __auto_type _t498 = (*h_iter_fu);
        if (_t498.tag == HirExpr_ECall) {
            __auto_type fu_callee = _t498.data.ECall.callee;
__auto_type fu_args = _t498.data.ECall.args;
            /* pass */
            __auto_type _t499 = (*fu_callee);
            if (_t499.tag == HirExpr_EIdent) {
                __auto_type fu_fn = _t499.data.EIdent.name;
                /* pass */
        ...
        } else if (_t498.tag == HirExpr_EMethodCall) {
            __auto_type fu_obj = _t498.data.EMethodCall.obj;
__auto_type fu_meth = _t498.data.EMethodCall.method;
            /* pass */
        ...
        while ((vi_fu < vars->len)) {
            /* pass */
            ({ TrStr _at_t500 = (List_TrStr_get(vars, vi_fu)); Sema_declare(self, _at_t500, SymbolKind_make_SVariable(), box_asttype(((AstType*)List_ptr_get(fu_tys, vi_fu))), false); _tr_str_release(_at_t500); });
            /* pass */
            vi_fu = (vi_fu + 1LL);
        }
        /* pass */
    ...
    } else if (_t440.tag == Stmt_SMatch) {
        __auto_type subj = _t440.data.SMatch.expr;
__auto_type arms = _t440.data.SMatch.arms;
        ...
        while ((k < arms->len)) {
            ...
            /* pass */
            __auto_type _t501 = arm->pat;
            if (_t501.tag == Pattern_PWild) {
                ex_has_wild = true;
            } else if (_t501.tag == Pattern_PBind) {
                __auto_type _ = _t501.data.PBind.name;
                ex_has_wild = true;
            } else if (_t501.tag == Pattern_PVariant) {
                __auto_type ex_vn = _t501.data.PVariant.variant;
                List_TrStr_append(ex_covered, ex_vn);
            } else if (_t501.tag == Pattern_PVariantBind) {
                __auto_type ex_vn2 = _t501.data.PVariantBind.variant;
                List_TrStr_append(ex_covered, ex_vn2);
            } else if (_t501.tag == Pattern_PVariantBindMany) {
                __auto_type ex_vn3 = _t501.data.PVariantBindMany.variant;
            ...
            /* pass */
            HirMatchArm* h_arm = ({ HirBlock* _aot_t502 = (Sema_lower_block(self, (*arm->body))); __auto_type _wr = (HirMatchArm_init(arm->pat, _aot_t502)); _tr_obj_release(_aot_t502, _trdrop_HirBlock); _wr; });
            /* pass */
        ...
        if (((!ex_has_wild) && _tr_dict_contains(self->enums, _tr_strz(ex_ty_name)))) {
            ...
            if ((ex_missing->len > 0LL)) {
                ...
                if ((ex_missing->len > 1LL)) {
                    /* pass */
                    TrStr _strtmp_t503 = _tr_strx_concat(_tr_strz(ex_msg), _tr_strz(_tr_str_lit("s")));
                    _tr_str_release(ex_msg);
                    ex_msg = _strtmp_t503;
                }
                /* pass */
                TrStr _strtmp_t504 = _tr_strx_concat(_tr_strz(ex_msg), _tr_strz(_tr_str_lit(": ")));
                _tr_str_release(ex_msg);
                ex_msg = _strtmp_t504;
                /* pass */
                long long ex_mi = 0LL;
                ...
                while ((ex_mi < ex_missing->len)) {
                    ...
                    /* pass */
                    TrStr _strtmp_t506 = ({ TrStr _cr = (List_TrStr_get(ex_missing, ex_mi)); TrStr _cres = _tr_strx_concat(_tr_strz(ex_msg), _cr.data); _tr_str_release(_cr); _cres; });
                    _tr_str_release(ex_msg);
                    ex_msg = _strtmp_t506;
                    /* pass */
                    ex_mi = (ex_mi + 1LL);
    ...
    } else if (_t440.tag == Stmt_STry) {
        __auto_type try_body = _t440.data.STry.try_body;
__auto_type catches = _t440.data.STry.catches;
__auto_type finally_b = _t440.data.STry.finally_b;
        /* pass */
        self->block_depth = (self->block_depth + 1LL);
        /* pass */
        Sema_open_block(self);
        ...
        while ((k < catches->len)) {
            ...
            /* pass */
            HirBlock* _cltmp_t508 = Sema_lower_block(self, (*cc->body));
            _tr_obj_release(hcc_val->body, _trdrop_HirBlock);
            hcc_val->body = _cltmp_t508;
            /* pass */
            /* unsafe block */
            /* pass */
    ...
    } else if (_t440.tag == Stmt_SRaise) {
        __auto_type e = _t440.data.SRaise.val;
        return box_hirstmt(HirStmt_ctor_SRaise(Sema_lower_expr(self, e)));
    } else if (_t440.tag == Stmt_SAssert) {
        __auto_type cond = _t440.data.SAssert.cond;
__auto_type msg = _t440.data.SAssert.msg;
        return box_hirstmt(HirStmt_ctor_SAssert(Sema_lower_expr(self, cond), Sema_lower_expr(self, msg)));
    } else if (_t440.tag == Stmt_SDefer) {
        __auto_type inner = _t440.data.SDefer.stmt;
        /* pass */
        return box_hirstmt(HirStmt_ctor_SDefer(Sema_lower_stmt(self, inner)));
    } else if (_t440.tag == Stmt_SWith) {
        __auto_type items = _t440.data.SWith.items;
__auto_type aliases = _t440.data.SWith.aliases;
__auto_type body = _t440.data.SWith.body;
        /* pass */
        Sema_enter_scope(self);
        /* pass */
        ...
        while ((k < items->len)) {
            ...
            if (((k < aliases->len) && (strcmp(_tr_strz(List_TrStr_get(aliases, k)), _tr_strz(_tr_str_lit(""))) != 0))) {
                ...
                /* pass */
                ({ TrStr _at_t509 = (List_TrStr_get(aliases, k)); Sema_declare(self, _at_t509, SymbolKind_make_SVariable(), box_asttype(wi_ty), true); _tr_str_release(_at_t509); });
            }
            /* pass */
            k = (k + 1LL);
        }
    ...
    } else if (_t440.tag == Stmt_SAsm) {
        __auto_type code = _t440.data.SAsm.code;
__auto_type outputs = _t440.data.SAsm.outputs;
__auto_type inputs = _t440.data.SAsm.inputs;
__auto_type clobbers = _t440.data.SAsm.clobbers;
        /* pass */
        return box_hirstmt(HirStmt_ctor_SAsm(code, outputs, inputs, clobbers));
    } else if (_t440.tag == Stmt_SSpawn) {
        __auto_type e = _t440.data.SSpawn.expr;
        /* pass */
        if ((!self->in_async_fn)) {
            /* pass */
            ({ TrStr _at_t510 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[C-5] 'spawn:' used outside an async function. FIX: Declare '")), _tr_strz(self->current_func_name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' as 'async def "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(self->current_func_name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("(...)' to use spawn inside it."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t510); _tr_str_release(_at_t510); });
        }
        /* pass */
    ...
    } else if (_t440.tag == Stmt_STaskGroup) {
        __auto_type body = _t440.data.STaskGroup.body;
        /* pass */
        if ((!self->in_async_fn)) {
            /* pass */
            ({ TrStr _at_t511 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[C-6] 'taskgroup:' used outside an async function. FIX: Declare '")), _tr_strz(self->current_func_name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' as 'async def "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(self->current_func_name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("(...)' to use taskgroup inside it."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t511); _tr_str_release(_at_t511); });
        }
        /* pass */
        self->block_depth = (self->block_depth + 1LL);
        /* pass */
    ...
    } else if (_t440.tag == Stmt_SChanSelect) {
        __auto_type cs_cases = _t440.data.SChanSelect.cases;
        /* pass */
        ...
        while ((csi2 < cs_cases->len)) {
            ...
            /* pass */
            HirBlock* _cltmp_t512 = Sema_lower_block(self, arm->body);
            _tr_obj_release(harm_v->body, _trdrop_HirBlock);
            harm_v->body = _cltmp_t512;
            /* pass */
    ...
    } else if (_t440.tag == Stmt_SGpuBlock) {
        __auto_type body = _t440.data.SGpuBlock.body;
        /* pass */
        self->block_depth = (self->block_depth + 1LL);
        /* pass */
        Sema_open_block(self);
    ...
    } else if (_t440.tag == Stmt_SBreak) {
        __auto_type bv = _t440.data.SBreak.val;
        /* pass */
    ...
    } else if (_t440.tag == Stmt_SContinue) {
        return box_hirstmt(HirStmt_make_SContinue());
    } else if (_t440.tag == Stmt_SPass) {
        return box_hirstmt(HirStmt_make_SPass());
    } else if (_t440.tag == Stmt_SLocalDecl) {
        __auto_type ldecl = _t440.data.SLocalDecl.decl;
        /* pass */
        ...
        /* pass */
        __auto_type _t513 = (*ldecl);
        if (_t513.tag == Decl_DFunction) {
            __auto_type ld_f = _t513.data.DFunction.func;
            /* pass */
            List_ptr_append(self->nested_functions, Sema_lower_func(self, ld_f));
        } else if (_t513.tag == Decl_DClass) {
            __auto_type ld_c = _t513.data.DClass.cls;
            /* pass */
            List_ptr_append(self->nested_classes, Sema_lower_class(self, ld_c));
        } else if (_t513.tag == Decl_DActor) {
            __auto_type ld_c = _t513.data.DActor.cls;
            /* pass */
            List_ptr_append(self->nested_classes, Sema_lower_class(self, ld_c));
        } else if (_t513.tag == Decl_DEnum) {
            __auto_type ld_e = _t513.data.DEnum.enm;
            /* pass */
            List_ptr_append(self->nested_enums, Sema_lower_enum(self, ld_e));
        } else if (_t513.tag == Decl_DInterface) {
            __auto_type ld_i = _t513.data.DInterface.iface;
            /* pass */
            List_ptr_append(self->nested_interfaces, Sema_lower_interface(self, ld_i));
        } else if (_t513.tag == Decl_DExtend) {
            __auto_type ld_target = _t513.data.DExtend.target;
__auto_type ld_methods = _t513.data.DExtend.methods;
            /* pass */
        ...
        /* pass */
        return box_hirstmt(HirStmt_make_SPass());
    } else if (_t440.tag == Stmt_SUnsafe) {
        __auto_type body = _t440.data.SUnsafe.body;
        /* pass */
    ...
    } else if (_t440.tag == Stmt_SMultiLet) {
        __auto_type names = _t440.data.SMultiLet.names;
__auto_type is_mut = _t440.data.SMultiLet.is_mut;
__auto_type val_ptr = _t440.data.SMultiLet.val;
        /* pass */
        HirExpr* hval = Sema_lower_expr(self, val_ptr);
        /* pass */
        ...
        while ((i < names->len)) {
            ...
            /* pass */
            ({ TrStr _at_t514 = (List_TrStr_get(names, i)); Sema_declare(self, _at_t514, SymbolKind_make_SVariable(), box_asttype(nty), is_mut); _tr_str_release(_at_t514); });
            /* pass */
            i = (i + 1LL);
        }
        /* pass */
        return box_hirstmt(HirStmt_ctor_SMultiLet(names, is_mut, hval));
    } else if (_t440.tag == Stmt_SLine) {
        __auto_type n = _t440.data.SLine.n;
        /* pass */
        self->current_line = n;
        /* pass */
...
__attribute__((hot)) void Sema_declare_pattern_binds_typed(Sema* self, Pattern pat, AstType* subj_ty) {
    /* pass */
    __auto_type _t515 = pat;
    if (_t515.tag == Pattern_PBind) {
        __auto_type name = _t515.data.PBind.name;
        Sema_declare(self, name, SymbolKind_make_SVariable(), box_asttype(subj_ty), false);
    } else if (_t515.tag == Pattern_PVariantBind) {
        __auto_type type_name = _t515.data.PVariantBind.type_name;
__auto_type variant_name = _t515.data.PVariantBind.variant;
__auto_type field = _t515.data.PVariantBind.field;
        /* pass */
    ...
    } else if (_t515.tag == Pattern_PVariantBindMany) {
        __auto_type type_name = _t515.data.PVariantBindMany.type_name;
__auto_type variant_name = _t515.data.PVariantBindMany.variant;
__auto_type fields = _t515.data.PVariantBindMany.fields;
        /* pass */
        long long _pi = 0LL;
        /* pass */
    ...
    } else if (_t515.tag == Pattern_PTuple) {
        __auto_type first = _t515.data.PTuple.first;
__auto_type second = _t515.data.PTuple.second;
        /* pass */
    ...
    } else if (_t515.tag == Pattern_POr) {
        __auto_type pats = _t515.data.POr.patterns;
        /* pass */
        long long i = 0LL;
        /* pass */
        while ((i < pats->len)) {
...
__attribute__((hot)) void Sema_collect_stmt_refs(Sema* self, HirStmt* s, List_TrStr* out) {
    ...
    /* pass */
    __auto_type _t516 = (*s);
    if (_t516.tag == HirStmt_SExpr) {
        __auto_type e = _t516.data.SExpr.expr;
        Sema_collect_idents(self, e, out);
    } else if (_t516.tag == HirStmt_SLet) {
        __auto_type v = _t516.data.SLet.val;
        Sema_collect_idents(self, v, out);
    } else if (_t516.tag == HirStmt_SAssign) {
        __auto_type t = _t516.data.SAssign.target;
__auto_type v = _t516.data.SAssign.val;
        /* pass */
        Sema_collect_idents(self, t, out);
        /* pass */
        Sema_collect_idents(self, v, out);
    } else if (_t516.tag == HirStmt_SReturn) {
        __auto_type v = _t516.data.SReturn.val;
        /* pass */
        if ((((unsigned long long)(v)) != ((unsigned long long)(0LL)))) {
            /* pass */
            Sema_collect_idents(self, v, out);
        }
    } else if (_t516.tag == HirStmt_SBreak) {
        __auto_type v = _t516.data.SBreak.val;
        /* pass */
        if ((((unsigned long long)(v)) != ((unsigned long long)(0LL)))) {
            /* pass */
            Sema_collect_idents(self, v, out);
        }
    } else if (_t516.tag == HirStmt_SRaise) {
        __auto_type v = _t516.data.SRaise.val;
        Sema_collect_idents(self, v, out);
    } else if (_t516.tag == HirStmt_SIf) {
        __auto_type c = _t516.data.SIf.cond;
__auto_type tb = _t516.data.SIf.then_b;
__auto_type eb = _t516.data.SIf.else_b;
        /* pass */
        Sema_collect_idents(self, c, out);
        /* pass */
    ...
    } else if (_t516.tag == HirStmt_SWhile) {
        __auto_type c = _t516.data.SWhile.cond;
__auto_type b2 = _t516.data.SWhile.body;
        /* pass */
        Sema_collect_idents(self, c, out);
        /* pass */
        Sema_collect_block_refs(self, b2, out);
    } else if (_t516.tag == HirStmt_SFor) {
        __auto_type it = _t516.data.SFor.iter;
__auto_type b2 = _t516.data.SFor.body;
        /* pass */
        Sema_collect_idents(self, it, out);
        /* pass */
        Sema_collect_block_refs(self, b2, out);
    } else if (_t516.tag == HirStmt_SForUnpack) {
        __auto_type it = _t516.data.SForUnpack.iter;
__auto_type b2 = _t516.data.SForUnpack.body;
        /* pass */
        Sema_collect_idents(self, it, out);
        /* pass */
        Sema_collect_block_refs(self, b2, out);
    } else if (_t516.tag == HirStmt_SMatch) {
        __auto_type subj = _t516.data.SMatch.expr;
__auto_type arms = _t516.data.SMatch.arms;
        /* pass */
    ...
    } else if (_t516.tag == HirStmt_SUnsafe) {
        __auto_type b2 = _t516.data.SUnsafe.body;
        Sema_collect_block_refs(self, b2, out);
    } else if (1) {
        __auto_type _ = _t516;
        /* pass */
...
__attribute__((hot)) HirExpr* Sema_lower_do_value(Sema* self, Block* do_body) {
    ...
    while ((do_i < do_n)) {
        ...
        if ((do_i == (do_n - 1LL))) {
            /* pass */
            __auto_type _t517 = (*do_s);
            if (_t517.tag == Stmt_SExpr) {
                __auto_type de = _t517.data.SExpr.expr;
                /* pass */
...
__attribute__((hot)) AstType* Sema_infer_break_type_stmt(Sema* self, HirStmt* s) {
    /* pass */
    __auto_type _t518 = (*s);
    if (_t518.tag == HirStmt_SBreak) {
        __auto_type bv = _t518.data.SBreak.val;
        /* pass */
        if ((((unsigned long long)(bv)) != ((unsigned long long)(0LL)))) {
            /* pass */
        ...
        /* pass */
        return AstType_init(_tr_str_lit("void"));
    } else if (_t518.tag == HirStmt_SIf) {
        __auto_type then_b = _t518.data.SIf.then_b;
__auto_type else_b = _t518.data.SIf.else_b;
        /* pass */
    ...
    } else if (_t518.tag == HirStmt_SMatch) {
        __auto_type m_arms = _t518.data.SMatch.arms;
        /* pass */
        long long ai = 0LL;
        /* pass */
        while ((ai < m_arms->len)) {
        ...
        /* pass */
        return AstType_init(_tr_str_lit("void"));
    } else if (_t518.tag == HirStmt_STry) {
        __auto_type tb = _t518.data.STry.try_body;
__auto_type fb = _t518.data.STry.finally_b;
        /* pass */
...
__attribute__((hot)) HirExpr* Sema_lower_expr(Sema* self, Expr* e_ptr) {
    ...
    if (_t519.tag == Expr_ELitInt) {
        ...
        if ((((((strcmp(_tr_strz(sym->name), _tr_strz(_tr_str_lit(""))) == 0) && (strcmp(_tr_strz(name), _tr_strz(_tr_str_lit(""))) != 0)) && (!self->in_assign_target)) && (!self->in_recv_pos)) && (!Sema_is_known_name(self, name)))) {
            /* pass */
            ({ TrStr _at_t520 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[N-3] name '")), _tr_strz(name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is not defined.\n      FIX: check the spelling, declare it with 'mut "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" = ...', or import it before use."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t520); _tr_str_release(_at_t520); });
        }
        /* pass */
        ...
        if ((sym->is_freed && (strcmp(_tr_strz(sym->name), _tr_strz(_tr_str_lit(""))) != 0))) {
            /* pass */
            ({ TrStr _at_t521 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[M-6] '")), _tr_strz(name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' was freed by 'dealloc()' and can no longer be used.\n      FIX: Remove all uses of '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' after 'dealloc()', or restructure so the pointer is freed only when no longer needed."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t521); _tr_str_release(_at_t521); });
            ...
            /* pass */
            ({ TrStr _at_t522 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[M-1] '")), _tr_strz(name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' was moved and cannot be used again.\n      FIX: Use the variable that now owns it, or call .clone() to copy before moving."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t522); _tr_str_release(_at_t522); });
            ...
            /* pass */
            ({ TrStr _at_t523 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[M-5] '")), _tr_strz(name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' may have been moved on some code paths, making this use unsafe.\n      FIX: Ensure '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is not moved before this point on any branch, or restructure so the use is inside the branch where it's still valid."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t523); _tr_str_release(_at_t523); });
        }
        /* pass */
        if ((((((!sym->is_init) && (!sym->is_maybe_init)) && (strcmp(_tr_strz(sym->name), _tr_strz(_tr_str_lit(""))) != 0)) && (sym->kind.tag == SymbolKind_make_SVariable().tag)) && (!self->in_assign_target))) {
            /* pass */
            ({ TrStr _at_t524 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[I-1] Variable '")), _tr_strz(name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is used before being assigned a value.\n      FIX: Assign a value before use, e.g. 'mut "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" = <default>'."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t524); _tr_str_release(_at_t524); });
            ...
            /* pass */
            ({ TrStr _at_t525 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[I-2] '")), _tr_strz(name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is not initialized on all code paths before this use.\n      FIX: Initialize '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' before the if/loop, or ensure every branch assigns a value."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t525); _tr_str_release(_at_t525); });
        }
        /* pass */
        bool is_move = false;
        /* pass */
    ...
    } else if (_t519.tag == Expr_EBinOp) {
        __auto_type op = _t519.data.EBinOp.op;
__auto_type left = _t519.data.EBinOp.left;
__auto_type right = _t519.data.EBinOp.right;
        /* pass */
        ...
        if (((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("=="))) == 0) || (strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("!="))) == 0))) {
            ...
            if (((strcmp(_tr_strz(lref), _tr_strz(_tr_str_lit(""))) != 0) || (strcmp(_tr_strz(rref), _tr_strz(_tr_str_lit(""))) != 0))) {
                ...
                if ((strcmp(_tr_strz(lname), _tr_strz(_tr_str_lit(""))) == 0)) {
                    /* pass */
                    TrStr _strtmp_t526 = hir_expr_type(hleft)->name;
                ...
                if ((strcmp(_tr_strz(rname), _tr_strz(_tr_str_lit(""))) == 0)) {
                    /* pass */
                    TrStr _strtmp_t527 = hir_expr_type(hright)->name;
    ...
    } else if (_t519.tag == Expr_EUnaryOp) {
        __auto_type op = _t519.data.EUnaryOp.op;
__auto_type expr = _t519.data.EUnaryOp.expr;
        /* pass */
    ...
    } else if (_t519.tag == Expr_ECall) {
        __auto_type callee = _t519.data.ECall.callee;
__auto_type args = _t519.data.ECall.args;
        /* pass */
        if ((((unsigned long long)(callee)) != ((unsigned long long)(0LL)))) {
            /* pass */
            __auto_type _t528 = (*callee);
            if (_t528.tag == Expr_EIdent) {
                __auto_type ato_n = _t528.data.EIdent.name;
                /* pass */
                ...
                if ((strcmp(_tr_strz(ato_n), _tr_strz(_tr_str_lit("instanceOf"))) == 0)) {
                    ...
                    /* pass */
                    __auto_type _t529 = (*((Expr*)List_ptr_get(args, 1LL)));
                    if (_t529.tag == Expr_EIdent) {
                        __auto_type io_tn = _t529.data.EIdent.name;
                        TrStr _strtmp_t530 = _tr_str_retain(io_tn);
                        _tr_str_release(io_target_n);
                        io_target_n = _strtmp_t530;
                    } else if (_t529.tag == Expr_EIndex) {
                        __auto_type io_base = _t529.data.EIndex.obj;
                        /* pass */
                        __auto_type _t531 = (*io_base);
                        if (_t531.tag == Expr_EIdent) {
                            __auto_type io_tn2 = _t531.data.EIdent.name;
                            TrStr _strtmp_t532 = _tr_str_retain(io_tn2);
                            _tr_str_release(io_target_n);
                            io_target_n = _strtmp_t532;
                        } else if (1) {
                            __auto_type _ = _t531;
                            /* pass */
                        }
                ...
                if ((strcmp(_tr_strz(ato_n), _tr_strz(_tr_str_lit("inspect"))) == 0)) {
                    ...
                    /* pass */
                    __auto_type _t533 = (*((Expr*)List_ptr_get(args, 0LL)));
                    if (_t533.tag == Expr_EIdent) {
                        __auto_type isp_tn = _t533.data.EIdent.name;
                        TrStr _strtmp_t534 = _tr_str_retain(isp_tn);
                        _tr_str_release(isp_target_n);
                        isp_target_n = _strtmp_t534;
                    } else if (_t533.tag == Expr_EIndex) {
                        __auto_type isp_base = _t533.data.EIndex.obj;
                        /* pass */
                        __auto_type _t535 = (*isp_base);
                        if (_t535.tag == Expr_EIdent) {
                            __auto_type isp_tn2 = _t535.data.EIdent.name;
                            TrStr _strtmp_t536 = _tr_str_retain(isp_tn2);
                            _tr_str_release(isp_target_n);
                            isp_target_n = _strtmp_t536;
                        } else if (1) {
                            __auto_type _ = _t535;
                            /* pass */
                        }
                    ...
                    if ((strcmp(_tr_strz(isp_target_n), _tr_strz(_tr_str_lit(""))) == 0)) {
                        ...
                        /* pass */
                        TrStr _strtmp_t537 = hir_expr_type(isp_obj)->name;
                        _tr_str_release(isp_target_n);
                        isp_target_n = _strtmp_t537;
                    }
                    /* pass */
                    return ({ TrStr _at_t538 = (Sema_build_inspect_str(self, isp_target_n)); __auto_type _wr = (box_hirexpr(HirExpr_ctor_ELitStr(_at_t538, AstType_init(_tr_str_lit("str"))))); _tr_str_release(_at_t538); _wr; });
                }
            } else if (1) {
                __auto_type _ = _t528;
                /* pass */
        ...
        while ((p23_k < args->len)) {
            ...
            if ((((unsigned long long)(p23_arg)) != ((unsigned long long)(0LL)))) {
                /* pass */
                __auto_type _t539 = (*p23_arg);
                if (_t539.tag == Expr_EIdent) {
                    __auto_type p23_n = _t539.data.EIdent.name;
                    TrStr _strtmp_t540 = _tr_str_retain(p23_n);
            ...
            if ((strcmp(_tr_strz(p23_nm), _tr_strz(_tr_str_lit(""))) != 0)) {
                ...
                if (((!Sema_is_primitive(self, (*p23_sym->ty))) && (strcmp(_tr_strz(p23_sym->name), _tr_strz(_tr_str_lit(""))) != 0))) {
                    ...
                    if (_tr_dict_contains(p23_seen, _tr_strz(p23_nm))) {
                        /* pass */
                        ({ TrStr _at_t541 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[M-3] '")), _tr_strz(p23_nm))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' appears twice in the same call, creating aliased mutable access.\n      FIX: Clone one of the arguments: "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(p23_nm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(".clone()"))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t541); _tr_str_release(_at_t541); });
                    }
                    /* pass */
        ...
        while ((p23_ui < p23_borrow_names->len)) {
            /* pass */
            ({ TrStr _at_t542 = (List_TrStr_get(p23_borrow_names, p23_ui)); Sema_unmark_borrow(self, _at_t542); _tr_str_release(_at_t542); });
            /* pass */
            p23_ui = (p23_ui + 1LL);
        }
        /* pass */
        if ((((unsigned long long)(callee)) != ((unsigned long long)(0LL)))) {
            ...
            if (_t543.tag == Expr_EIdent) {
                __auto_type vfn_n = _t543.data.EIdent.name;
                /* pass */
        ...
        if (_t544.tag == Expr_EIdent) {
            __auto_type n = _t544.data.EIdent.name;
            /* pass */
            ...
            if ((((((((_tr_dict_contains(self->classes, _tr_strz(n)) || (strcmp(_tr_strz(n), _tr_strz(_tr_str_lit("StringObj"))) == 0)) || (strcmp(_tr_strz(n), _tr_strz(_tr_str_lit("StringBuilder"))) == 0)) || (strcmp(_tr_strz(n), _tr_strz(_tr_str_lit("List"))) == 0)) || (strcmp(_tr_strz(n), _tr_strz(_tr_str_lit("Vec"))) == 0)) || (strcmp(_tr_strz(n), _tr_strz(_tr_str_lit("Pointer"))) == 0)) || (strcmp(_tr_strz(n), _tr_strz(_tr_str_lit("Map"))) == 0)) || (strcmp(_tr_strz(n), _tr_strz(_tr_str_lit("Dict"))) == 0))) {
                ...
                if ((!self->in_unsafe)) {
                    /* pass */
                    ({ TrStr _at_t545 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[U-1] '")), _tr_strz(n))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' used outside an 'unsafe:' block.\n      FIX: Wrap raw memory operations in 'unsafe:' to signal manual memory management, e.g.\n          unsafe:\n              p = "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(n)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("[T](n)"))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t545); _tr_str_release(_at_t545); });
                }
            } else {
                /* pass */
            ...
            __auto_type base = _t544.data.EIndex.obj;
__auto_type idx = _t544.data.EIndex._tr_v_index;
            /* pass */
            __auto_type _t546 = (*base);
            if (_t546.tag == Expr_EIdent) {
                __auto_type gn = _t546.data.EIdent.name;
                /* pass */
                if (((strcmp(_tr_strz(gn), _tr_strz(_tr_str_lit("alloc"))) == 0) || (strcmp(_tr_strz(gn), _tr_strz(_tr_str_lit("dealloc"))) == 0))) {
                    ...
                    if ((!self->in_unsafe)) {
                        /* pass */
                        ({ TrStr _at_t548 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[U-1] 'alloc'/'dealloc' used outside an 'unsafe:' block.\n      FIX: Wrap raw memory operations in 'unsafe:' to signal manual memory management, e.g.\n          unsafe:\n              p = ")), _tr_strz(gn))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("[T](n)"))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t548); _tr_str_release(_at_t548); });
                    }
                    ...
                    if (_t549.tag == Expr_EIdent) {
                        __auto_type tn = _t549.data.EIdent.name;
                        /* pass */
                    ...
                    if (_t550.tag == Expr_EIdent) {
                        __auto_type targ_ca = _t550.data.EIdent.name;
                    ...
                    if (_t551.tag == Expr_EIdent) {
                        __auto_type farg_c = _t551.data.EIdent.name;
                        /* pass */
                        AstType* fret = AstType_init(farg_c);
                        /* pass */
                        ret_ty = fret;
                        /* pass */
                        hcallee = ({ TrStr _at_t552 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(gn), _tr_strz(_tr_str_lit("__MONO_")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(farg_c)); _tr_str_release(_cl); _cres; })); __auto_type _wr = (box_hirexpr(HirExpr_ctor_EIdent(_at_t552, fret, false))); _tr_str_release(_at_t552); _wr; });
                    } else if (1) {
                        __auto_type _ = _t551;
                        /* pass */
                    }
        ...
        } else if (_t544.tag == Expr_EPropAccess) {
            __auto_type obj = _t544.data.EPropAccess.obj;
__auto_type variant = _t544.data.EPropAccess.prop;
            /* pass */
            __auto_type _t553 = (*obj);
            if (_t553.tag == Expr_EIdent) {
                __auto_type type_name = _t553.data.EIdent.name;
                /* pass */
                if (_tr_dict_contains(self->enums, _tr_strz(type_name))) {
                    /* pass */
            ...
            } else if (_t553.tag == Expr_EIndex) {
                __auto_type base2 = _t553.data.EIndex.obj;
__auto_type idx2 = _t553.data.EIndex._tr_v_index;
                /* pass */
                if (((strcmp(_tr_strz(variant), _tr_strz(_tr_str_lit("init"))) == 0) || (strcmp(_tr_strz(variant), _tr_strz(_tr_str_lit("new"))) == 0))) {
                    /* pass */
                    __auto_type _t554 = (*base2);
                    if (_t554.tag == Expr_EIdent) {
                        __auto_type gn2 = _t554.data.EIdent.name;
                        /* pass */
                        if ((((((strcmp(_tr_strz(gn2), _tr_strz(_tr_str_lit("Pointer"))) == 0) || (strcmp(_tr_strz(gn2), _tr_strz(_tr_str_lit("List"))) == 0)) || (strcmp(_tr_strz(gn2), _tr_strz(_tr_str_lit("Vec"))) == 0)) || (strcmp(_tr_strz(gn2), _tr_strz(_tr_str_lit("Map"))) == 0)) || (strcmp(_tr_strz(gn2), _tr_strz(_tr_str_lit("Dict"))) == 0))) {
                            /* pass */
                            __auto_type _t555 = (*idx2);
                            if (_t555.tag == Expr_EIdent) {
                                __auto_type tn2 = _t555.data.EIdent.name;
                                /* pass */
                            ...
                            if (_t556.tag == Expr_EIdent) {
                                __auto_type targ_cb = _t556.data.EIdent.name;
        ...
        if ((((unsigned long long)(callee)) != ((unsigned long long)(0LL)))) {
            ...
            if (_t557.tag == Expr_EIdent) {
                __auto_type aa_nm = _t557.data.EIdent.name;
                /* pass */
        ...
        if ((((unsigned long long)(callee)) != ((unsigned long long)(0LL)))) {
            ...
            if (_t558.tag == Expr_EIdent) {
                __auto_type df_nm = _t558.data.EIdent.name;
                /* pass */
                if (((strcmp(_tr_strz(df_nm), _tr_strz(_tr_str_lit("dealloc"))) == 0) && (args->len > 0LL))) {
                    ...
                    if ((((unsigned long long)(df_arg0)) != ((unsigned long long)(0LL)))) {
                        /* pass */
                        __auto_type _t559 = (*df_arg0);
                        if (_t559.tag == Expr_EIdent) {
                            __auto_type df_ptr = _t559.data.EIdent.name;
                            /* pass */
        ...
        Dict_free(p23_seen);
        return box_hirexpr(HirExpr_ctor_ECall(hcallee, hl, ret_ty));
    } else if (_t519.tag == Expr_EMethodCall) {
        __auto_type obj = _t519.data.EMethodCall.obj;
__auto_type method = _t519.data.EMethodCall.method;
__auto_type args = _t519.data.EMethodCall.args;
        /* pass */
        ...
        if ((strcmp(_tr_strz(method), _tr_strz(_tr_str_lit("free"))) == 0)) {
            ...
            if (_t560.tag == Expr_EIdent) {
                __auto_type free_nm = _t560.data.EIdent.name;
                /* pass */
        ...
        if (_t561.tag == Expr_EIdent) {
            __auto_type _rn = _t561.data.EIdent.name;
            /* pass */
            TrStr _strtmp_t562 = _tr_str_retain(_rn);
            _tr_str_release(_recv_name);
            _recv_name = _strtmp_t562;
            /* pass */
        ...
        if (((((strcmp(_tr_strz(method), _tr_strz(_tr_str_lit("push"))) == 0) || (strcmp(_tr_strz(method), _tr_strz(_tr_str_lit("pop"))) == 0)) || (strcmp(_tr_strz(method), _tr_strz(_tr_str_lit("insert"))) == 0)) || (strcmp(_tr_strz(method), _tr_strz(_tr_str_lit("remove"))) == 0))) {
            ...
            if (_t563.tag == Expr_EIdent) {
                __auto_type pc_src = _t563.data.EIdent.name;
                TrStr _strtmp_t564 = _tr_str_retain(pc_src);
                _tr_str_release(pc_obj_nm);
                pc_obj_nm = _strtmp_t564;
            } else if (1) {
                __auto_type _ = _t563;
                /* pass */
            }
            ...
            if (((strcmp(_tr_strz(pc_obj_nm), _tr_strz(_tr_str_lit(""))) != 0) && _tr_dict_contains(self->container_borrows, _tr_strz(pc_obj_nm)))) {
                ...
                /* pass */
                ({ TrStr _at_t565 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[M-4] Cannot mutate '")), _tr_strz(pc_obj_nm))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' while '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pc_borrow_var)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' holds a reference into it.\n      FIX: Finish using '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pc_borrow_var)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' before modifying '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pc_obj_nm)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("', or copy it first: 'mut copy = "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pc_borrow_var)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("'."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t565); _tr_str_release(_at_t565); });
            }
        }
        /* pass */
        ...
        if (_t566.tag == Expr_EIdent) {
            __auto_type _bm_n = _t566.data.EIdent.name;
            TrStr _strtmp_t567 = _tr_str_retain(_bm_n);
            _tr_str_release(_bm_obj_nm);
            _bm_obj_nm = _strtmp_t567;
        } else if (1) {
            __auto_type _ = _t566;
            /* pass */
        }
        ...
        if (((strcmp(_tr_strz(method), _tr_strz(_tr_str_lit("init"))) == 0) || (strcmp(_tr_strz(method), _tr_strz(_tr_str_lit("new"))) == 0))) {
            ...
            if (((strcmp(_tr_strz(hobj_ty->name), _tr_strz(_tr_str_lit("void"))) == 0) || (strcmp(_tr_strz(hobj_ty->name), _tr_strz(_tr_str_lit(""))) == 0))) {
                /* pass */
                __auto_type _t568 = (*hobj);
                if (_t568.tag == HirExpr_EIdent) {
                    __auto_type recv_nm = _t568.data.EIdent.name;
                    ret_ty = AstType_init(recv_nm);
                } else if (_t568.tag == HirExpr_EIndex) {
                    __auto_type idx_base = _t568.data.EIndex.obj;
__auto_type idx_arg = _t568.data.EIndex._tr_v_index;
                    /* pass */
                    __auto_type _t569 = (*idx_base);
                    if (_t569.tag == HirExpr_EIdent) {
                        __auto_type gn = _t569.data.EIdent.name;
                        /* pass */
                        ret_ty = AstType_init(gn);
                        /* pass */
                        __auto_type _t570 = (*idx_arg);
            ...
            while ((_tsi < hl->len)) {
                ...
                if ((!Sema_is_sendable_ty(self, _tsa_ty))) {
                    /* pass */
                    ({ TrStr _at_t571 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[T-1] Type '")), _tr_strz(_tsa_ty->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is not Sendable and cannot be passed to Thread.spawn.\n      FIX: Wrap in Mutex["))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tsa_ty->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("] for exclusive access, or add 'implements Sendable' to '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tsa_ty->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' to confirm it is thread-safe."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t571); _tr_str_release(_at_t571); });
                } else if (((self->strict_mode && Sema__is_rc_class(self, _tsa_ty->name)) && (!Sema__expr_is_shared(self, List_ptr_get(hl, _tsi))))) {
                    /* pass */
                    ({ TrStr _at_t572 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[T-7] '")), _tr_strz(_tsa_ty->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is a plain reference-counted class — its refcount is thread-local (non-atomic), so it is not 'Send' and cannot be passed to Thread.spawn (exactly why Rust's 'Rc' is '!Send'): retaining/releasing it from another thread races the count.\n      FIX: share it as 'Shared["))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tsa_ty->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("]' (atomic refcount, like Rust's 'Arc'). A 'Sendable' class must itself hold only Sendable + atomically-shared state."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t572); _tr_str_release(_at_t572); });
                }
                /* pass */
            ...
            while ((_psi < hl->len)) {
                ...
                if ((!Sema_is_sendable_ty(self, _psa_ty))) {
                    /* pass */
                    ({ TrStr _at_t573 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[T-1] Type '")), _tr_strz(_psa_ty->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is not Sendable and cannot be passed to ThreadPool.spawn.\n      FIX: Wrap in Mutex["))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_psa_ty->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("] for exclusive access, or add 'implements Sendable' to '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_psa_ty->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' to confirm it is thread-safe."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t573); _tr_str_release(_at_t573); });
                } else if (((self->strict_mode && Sema__is_rc_class(self, _psa_ty->name)) && (!Sema__expr_is_shared(self, List_ptr_get(hl, _psi))))) {
                    /* pass */
                    ({ TrStr _at_t574 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[T-7] '")), _tr_strz(_psa_ty->name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' is a plain reference-counted class — its refcount is thread-local (non-atomic), so it cannot be passed to ThreadPool.spawn (like Rust's 'Rc' being '!Send').\n      FIX: share it as 'Shared["))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_psa_ty->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("]' (atomic refcount, like Rust's 'Arc')."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t574); _tr_str_release(_at_t574); });
                }
                /* pass */
            ...
            if ((((!_is_builtin_dispatch) && (!Sema_class_method_exists(self, hobj_ty->name, method))) && (!Sema_is_universal_method(self, method)))) {
                /* pass */
                ({ TrStr _at_t575 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[E-1] No method '")), _tr_strz(method))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' found on type '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(hobj_ty->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("'.\n      FIX: Define 'pub def "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(method)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("(self, ...)' in '"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(hobj_ty->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' or its base class via 'extend "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(hobj_ty->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(":'."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t575); _tr_str_release(_at_t575); });
            }
            /* pass */
            long long _mi = 0LL;
            /* pass */
    ...
    } else if (_t519.tag == Expr_EPropAccess) {
        __auto_type obj = _t519.data.EPropAccess.obj;
__auto_type prop = _t519.data.EPropAccess.prop;
        /* pass */
    ...
    } else if (_t519.tag == Expr_EIndex) {
        __auto_type obj = _t519.data.EIndex.obj;
__auto_type idx_inner = _t519.data.EIndex._tr_v_index;
        /* pass */
        ...
        if (_t576.tag == Expr_EIdent) {
            __auto_type n = _t576.data.EIdent.name;
            TrStr _strtmp_t577 = _tr_str_retain(n);
            _tr_str_release(obj_name);
            obj_name = _strtmp_t577;
        } else if (1) {
            __auto_type _ = _t576;
            /* pass */
        }
        ...
        /* pass */
        __auto_type _t578 = (*idx_inner);
        if (_t578.tag == Expr_ETuple) {
            __auto_type _tup_targs = _t578.data.ETuple.items;
            /* pass */
            is_generic = true;
            /* pass */
            long long _tti = 0LL;
            ...
            if ((generic_args->len > 0LL)) {
                ...
                /* pass */
                TrStr _strtmp_t579 = (*((AstType**)List_ptr_get(generic_args, 0LL)))->name;
                _tr_str_release(generic_arg_n);
                generic_arg_n = _strtmp_t579;
            }
        } else if (_t578.tag == Expr_ETypeArg) {
            __auto_type targ_ty = _t578.data.ETypeArg.ty;
            /* pass */
            is_generic = true;
            /* pass */
            generic_arg_ty = targ_ty;
            /* pass */
            TrStr _strtmp_t580 = (*targ_ty)->name;
            _tr_str_release(generic_arg_n);
            generic_arg_n = _strtmp_t580;
        } else if (_t578.tag == Expr_EIdent) {
            __auto_type iname = _t578.data.EIdent.name;
            /* pass */
            bool is_param = false;
            /* pass */
            ...
            if ((((((((((((((Sema_is_primitive_name(self, iname) || (strcmp(_tr_strz(iname), _tr_strz(_tr_str_lit("str"))) == 0)) || (strcmp(_tr_strz(iname), _tr_strz(_tr_str_lit("Str"))) == 0)) || is_param) || (_tr_strlen(_tr_strz(iname)) == 1LL)) || _tr_dict_contains(self->classes, _tr_strz(iname))) || (strcmp(_tr_strz(iname), _tr_strz(_tr_str_lit("StringObj"))) == 0)) || (strcmp(_tr_strz(iname), _tr_strz(_tr_str_lit("StringBuilder"))) == 0)) || _tr_dict_contains(self->enums, _tr_strz(iname))) || (strcmp(_tr_strz(iname), _tr_strz(_tr_str_lit("Vec"))) == 0)) || (strcmp(_tr_strz(iname), _tr_strz(_tr_str_lit("List"))) == 0)) || (strcmp(_tr_strz(iname), _tr_strz(_tr_str_lit("Map"))) == 0)) || (strcmp(_tr_strz(iname), _tr_strz(_tr_str_lit("Tuple"))) == 0)) || (strcmp(_tr_strz(iname), _tr_strz(_tr_str_lit("tuple"))) == 0))) {
                ...
                is_generic = true;
                /* pass */
                TrStr _strtmp_t581 = _tr_str_retain(iname);
                _tr_str_release(generic_arg_n);
            ...
            if ((((unsigned long long)(nested_ty)) != ((unsigned long long)(0LL)))) {
                /* pass */
                TrStr _strtmp_t582 = (*nested_ty)->name;
                _tr_str_release(generic_arg_n);
        ...
        if ((is_generic && obj_is_type)) {
            ...
            if ((((strcmp(_tr_strz(eff_ty_n), _tr_strz(_tr_str_lit("void"))) == 0) || (strcmp(_tr_strz(eff_ty_n), _tr_strz(_tr_str_lit(""))) == 0)) && ((((((strcmp(_tr_strz(obj_name), _tr_strz(_tr_str_lit("Vec"))) == 0) || (strcmp(_tr_strz(obj_name), _tr_strz(_tr_str_lit("List"))) == 0)) || (strcmp(_tr_strz(obj_name), _tr_strz(_tr_str_lit("Map"))) == 0)) || (strcmp(_tr_strz(obj_name), _tr_strz(_tr_str_lit("Dict"))) == 0)) || (strcmp(_tr_strz(obj_name), _tr_strz(_tr_str_lit("Set"))) == 0)) || (strcmp(_tr_strz(obj_name), _tr_strz(_tr_str_lit("Pointer"))) == 0)))) {
                /* pass */
                TrStr _strtmp_t583 = _tr_str_retain(obj_name);
                _tr_str_release(eff_ty_n);
                eff_ty_n = _strtmp_t583;
            }
            /* pass */
    ...
    } else if (_t519.tag == Expr_ESizeOf) {
        __auto_type ty = _t519.data.ESizeOf.ty;
        /* pass */
    ...
    } else if (_t519.tag == Expr_ECast) {
        __auto_type expr = _t519.data.ECast.expr;
__auto_type ty = _t519.data.ECast.ty;
        /* pass */
        if ((((unsigned long long)(ty)) == ((unsigned long long)(0LL)))) {
            /* pass */
    ...
    } else if (_t519.tag == Expr_EFString) {
        __auto_type parts = _t519.data.EFString.parts;
        /* pass */
    ...
    } else if (_t519.tag == Expr_ETuple) {
        __auto_type items = _t519.data.ETuple.items;
        /* pass */
    ...
    } else if (_t519.tag == Expr_EList) {
        __auto_type items = _t519.data.EList.items;
        /* pass */
        List_ptr* hitems = (void*)List_ptr_new();
        /* pass */
        long long n = 0LL;
    ...
    } else if (_t519.tag == Expr_EClosure) {
        __auto_type params = _t519.data.EClosure.params;
__auto_type ret_ty = _t519.data.EClosure.ret_ty;
__auto_type body = _t519.data.EClosure.body;
__auto_type is_async = _t519.data.EClosure.is_async;
        /* pass */
        Sema_enter_scope(self);
        /* pass */
    ...
    } else if (_t519.tag == Expr_EIfElse) {
        __auto_type cond = _t519.data.EIfElse.cond;
__auto_type then_e = _t519.data.EIfElse.then_expr;
__auto_type else_e = _t519.data.EIfElse.else_expr;
        /* pass */
        HirExpr* hcond = Sema_lower_expr(self, cond);
        /* pass */
    ...
    } else if (_t519.tag == Expr_EDo) {
        __auto_type do_body = _t519.data.EDo.body;
        /* pass */
        return Sema_lower_do_value(self, do_body);
    } else if (_t519.tag == Expr_EMatch) {
        __auto_type m_subj = _t519.data.EMatch.subj;
__auto_type m_arms = _t519.data.EMatch.arms;
        /* pass */
    ...
    } else if (_t519.tag == Expr_ELoop) {
        __auto_type loop_body = _t519.data.ELoop.body;
        /* pass */
        Sema_enter_scope(self);
        /* pass */
        List_i64_append(self->loop_scope_base, (self->scopes->len - 1LL));
    ...
    } else if (_t519.tag == Expr_EWhileExpr) {
        __auto_type we_cond = _t519.data.EWhileExpr.cond;
__auto_type we_body = _t519.data.EWhileExpr.body;
__auto_type we_else = _t519.data.EWhileExpr.else_body;
        /* pass */
    ...
    } else if (_t519.tag == Expr_ETryExpr) {
        __auto_type inner = _t519.data.ETryExpr.expr;
        /* pass */
    ...
    } else if (_t519.tag == Expr_EAwait) {
        __auto_type inner_await = _t519.data.EAwait.expr;
        /* pass */
        if ((!self->in_async_fn)) {
            /* pass */
            ({ TrStr _at_t584 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("[C-4] 'await' used outside an async function. FIX: Declare '")), _tr_strz(self->current_func_name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' as 'async def "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(self->current_func_name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("(...)' to use await inside it."))); _tr_str_release(_cl); _cres; })); Sema_error(self, _at_t584); _tr_str_release(_at_t584); });
        }
        /* pass */
    ...
    } else if (_t519.tag == Expr_EDict) {
        __auto_type keys = _t519.data.EDict.keys;
__auto_type vals = _t519.data.EDict.vals;
        /* pass */
    ...
    } else if (_t519.tag == Expr_EListComp) {
        __auto_type element = _t519.data.EListComp.element;
__auto_type generators = _t519.data.EListComp.generators;
        /* pass */
        Sema_enter_scope(self);
        /* pass */
    ...
    } else if (_t519.tag == Expr_EGeneratorExpr) {
        __auto_type element = _t519.data.EGeneratorExpr.element;
__auto_type generators = _t519.data.EGeneratorExpr.generators;
        /* pass */
        Sema_enter_scope(self);
        /* pass */
    ...
    } else if (_t519.tag == Expr_ESuperMethodCall) {
        __auto_type base_class = _t519.data.ESuperMethodCall.base_class;
__auto_type method = _t519.data.ESuperMethodCall.method;
__auto_type args = _t519.data.ESuperMethodCall.args;
        /* pass */
        ...
        if (((strcmp(_tr_strz(resolved_base), _tr_strz(_tr_str_lit(""))) == 0) && (strcmp(_tr_strz(self->current_class_name), _tr_strz(_tr_str_lit(""))) != 0))) {
            ...
            if (_tr_dict_contains(self->classes, _tr_strz(self->current_class_name))) {
                ...
                if ((cur_cls->base_classes->len > 0LL)) {
                    /* pass */
                    TrStr _strtmp_t585 = List_TrStr_get(cur_cls->base_classes, 0LL);
                    _tr_str_release(resolved_base);
                    resolved_base = _strtmp_t585;
                }
            }
        }
        /* pass */
    ...
    } else if (_t519.tag == Expr_ESuperPropAccess) {
        __auto_type base_class = _t519.data.ESuperPropAccess.base_class;
__auto_type prop = _t519.data.ESuperPropAccess.prop;
        /* pass */
        ...
        if (((strcmp(_tr_strz(resolved_base2), _tr_strz(_tr_str_lit(""))) == 0) && (strcmp(_tr_strz(self->current_class_name), _tr_strz(_tr_str_lit(""))) != 0))) {
            ...
            if (_tr_dict_contains(self->classes, _tr_strz(self->current_class_name))) {
                ...
                if ((cur_cls2->base_classes->len > 0LL)) {
                    /* pass */
                    TrStr _strtmp_t586 = List_TrStr_get(cur_cls2->base_classes, 0LL);
                    _tr_str_release(resolved_base2);
                    resolved_base2 = _strtmp_t586;
                }
            }
        }
        /* pass */
...
__attribute__((hot)) bool Sema_block_returns(Sema* self, Block* b) {
    ...
    while ((last_idx > 0LL)) {
        ...
        if ((((unsigned long long)(sl_s)) == ((unsigned long long)(0LL)))) {
            ...
            bool is_sline = false;
            /* pass */
            __auto_type _t587 = (*sl_s);
            if (_t587.tag == Stmt_SLine) {
                __auto_type _ = _t587.data.SLine.n;
                is_sline = true;
            } else if (1) {
                __auto_type _ = _t587;
                /* pass */
    ...
    /* pass */
    __auto_type _t588 = (*last_s);
    if (_t588.tag == Stmt_SReturn) {
        __auto_type _ = _t588.data.SReturn.val;
        return true;
    } else if (_t588.tag == Stmt_SRaise) {
        __auto_type _ = _t588.data.SRaise.val;
        return true;
    } else if (_t588.tag == Stmt_SUnsafe) {
        __auto_type body = _t588.data.SUnsafe.body;
        return Sema_block_returns(self, body);
    } else if (_t588.tag == Stmt_SIf) {
        __auto_type cond = _t588.data.SIf.cond;
__auto_type then_b = _t588.data.SIf.then_b;
__auto_type elifs = _t588.data.SIf.elifs;
__auto_type else_b = _t588.data.SIf.else_b;
        /* pass */
    ...
    } else if (_t588.tag == Stmt_SMatch) {
        __auto_type subj = _t588.data.SMatch.expr;
__auto_type arms = _t588.data.SMatch.arms;
        /* pass */
        bool has_wild = false;
        /* pass */
        bool all_ret = true;
        ...
        while ((mi < arms->len)) {
            ...
            /* pass */
            __auto_type _t589 = arm->pat;
            if (_t589.tag == Pattern_PWild) {
                has_wild = true;
            } else if (_t589.tag == Pattern_PBind) {
                __auto_type _ = _t589.data.PBind.name;
                has_wild = true;
            } else if (1) {
                __auto_type _ = _t589;
                /* pass */
...
__attribute__((hot)) bool _expr_is_self_field(Expr* e) {
    ...
    /* pass */
    __auto_type _t590 = (*e);
    if (_t590.tag == Expr_EPropAccess) {
        __auto_type obj = _t590.data.EPropAccess.obj;
        /* pass */
        if ((((unsigned long long)(obj)) == ((unsigned long long)(0LL)))) {
            /* pass */
            return false;
        ...
        if (_t591.tag == Expr_EIdent) {
            __auto_type nm = _t591.data.EIdent.name;
...
__attribute__((hot)) bool _stmt_mutates_self(Stmt* s) {
    ...
    /* pass */
    __auto_type _t592 = (*s);
    if (_t592.tag == Stmt_SAssign) {
        __auto_type tgt = _t592.data.SAssign.target;
        return _expr_is_self_field(tgt);
    } else if (_t592.tag == Stmt_SIf) {
        __auto_type then_b = _t592.data.SIf.then_b;
__auto_type elifs = _t592.data.SIf.elifs;
__auto_type else_b = _t592.data.SIf.else_b;
        /* pass */
    ...
    } else if (_t592.tag == Stmt_SWhile) {
        __auto_type body = _t592.data.SWhile.body;
        return _block_mutates_self(body);
    } else if (_t592.tag == Stmt_SFor) {
        __auto_type body = _t592.data.SFor.body;
        return _block_mutates_self(body);
    } else if (_t592.tag == Stmt_SForUnpack) {
        __auto_type body = _t592.data.SForUnpack.body;
        return _block_mutates_self(body);
    } else if (_t592.tag == Stmt_SUnsafe) {
        __auto_type body = _t592.data.SUnsafe.body;
        return _block_mutates_self(body);
    } else if (_t592.tag == Stmt_SWith) {
        __auto_type body = _t592.data.SWith.body;
        return _block_mutates_self(body);
    } else if (_t592.tag == Stmt_STaskGroup) {
        __auto_type body = _t592.data.STaskGroup.body;
        return _block_mutates_self(body);
    } else if (_t592.tag == Stmt_SGpuBlock) {
        __auto_type body = _t592.data.SGpuBlock.body;
        return _block_mutates_self(body);
    } else if (_t592.tag == Stmt_SMatch) {
        __auto_type arms = _t592.data.SMatch.arms;
        /* pass */
        long long i = 0LL;
        /* pass */
        while ((i < arms->len)) {
        ...
        /* pass */
        return false;
    } else if (_t592.tag == Stmt_STry) {
        __auto_type try_body = _t592.data.STry.try_body;
__auto_type catches = _t592.data.STry.catches;
__auto_type finally_b = _t592.data.STry.finally_b;
        /* pass */
    ...
    } else if (_t592.tag == Stmt_SDefer) {
        __auto_type inner = _t592.data.SDefer.stmt;
