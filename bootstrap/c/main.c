...
__attribute__((hot)) int main(int argc, char** argv) {
    ...
    /* pass */
    TrStr rt_h = ({ TrStr _at_t2425 = (List_TrStr_get(args, 0LL)); __auto_type _wr = (read_runtime_header(_at_t2425, input_path)); _tr_str_release(_at_t2425); _wr; });
    /* pass */
    ...
    /* pass */
    TrStr _strtmp_t2426 = ({ TrStr _cr = (CGenerator_generate_module_compat(c_gen, resolver->all_decl_modules, resolver->all_decls)); TrStr _cres = _tr_strx_concat(_tr_strz(types_h), _cr.data); _tr_str_release(_cr); _cres; });
    _tr_str_release(types_h);
    types_h = _strtmp_t2426;
    /* pass */
    ...
    if (static_link) {
        /* pass */
        TrStr _strtmp_t2427 = _tr_strx_concat(_tr_strz(flags_sig), _tr_strz(_tr_str_lit(";static")));
        _tr_str_release(flags_sig);
        flags_sig = _strtmp_t2427;
    }
    /* pass */
    if (debug_mode) {
        /* pass */
        TrStr _strtmp_t2428 = _tr_strx_concat(_tr_strz(flags_sig), _tr_strz(_tr_str_lit(";debug")));
        _tr_str_release(flags_sig);
    ...
    if ((strcmp(_tr_strz(sysroot), _tr_strz(_tr_str_lit(""))) != 0)) {
        /* pass */
        TrStr _strtmp_t2429 = ({ TrStr _cl = (_tr_strx_concat(_tr_strz(flags_sig), _tr_strz(_tr_str_lit(";sysroot=")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(sysroot)); _tr_str_release(_cl); _cres; });
        _tr_str_release(flags_sig);
    ...
    if ((strcmp(_tr_strz(rt_h), _tr_strz(_tr_str_lit(""))) != 0)) {
        /* pass */
        ({ TrStr _at_t2430 = (_tr_strx_concat(_tr_strz(build_dir), _tr_strz(_tr_str_lit("tauraro_rt.h")))); write_file(_at_t2430, rt_h); _tr_str_release(_at_t2430); });
    }
    /* pass */
    ...
    while ((mi < resolver->mod_dot_paths->len)) {
        ...
        while ((k < resolver->all_decl_modules->len)) {
            ...
            if ((strcmp(_tr_strz(List_TrStr_get(resolver->all_decl_modules, k)), _tr_strz(dot_path)) == 0)) {
                /* pass */
                __auto_type _t2431 = (*((Decl*)List_ptr_get(resolver->all_decls, k)));
                if (_t2431.tag == Decl_DClass) {
                    __auto_type c = _t2431.data.DClass.cls;
                    /* pass */
                    _tr_dict_set(class_set, _tr_strz(c->name), true);
                } else if (_t2431.tag == Decl_DFunction) {
                    __auto_type f = _t2431.data.DFunction.func;
                    /* pass */
                    _tr_dict_set(fn_set, _tr_strz(f->name), true);
                    _tr_obj_release(f, _trdrop_FunctionDef);
                } else if (_t2431.tag == Decl_DEnum) {
                    __auto_type e = _t2431.data.DEnum.enm;
                    /* pass */
                    _tr_dict_set(class_set, _tr_strz(e->name), true);
                } else if (_t2431.tag == Decl_DInterface) {
                    __auto_type iface = _t2431.data.DInterface.iface;
                    /* pass */
                    _tr_dict_set(class_set, _tr_strz(iface->name), true);
                } else if (_t2431.tag == Decl_DExtend) {
                    __auto_type target = _t2431.data.DExtend.target;
__auto_type methods = _t2431.data.DExtend.methods;
                    /* pass */
        ...
        if (is_builtin_mod(dot_path)) {
            /* pass */
            TrStr _strtmp_t2432 = ensure_builtin_dirs(build_dir, dot_path);
            _tr_str_release(c_path);
            c_path = _strtmp_t2432;
            /* pass */
            depth = get_dot_depth(dot_path);
        } else {
            /* pass */
            TrStr _strtmp_t2433 = ({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(build_dir), _tr_strz(_tr_str_lit("module_")))); TrStr _cr = (dot_to_safe(dot_path)); TrStr _cres = _tr_strx_concat(_cl.data, _cr.data); _tr_str_release(_cl); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(".c"))); _tr_str_release(_cl); _cres; });
            _tr_str_release(c_path);
        ...
        if ((c_gen->emit_line_info && (mi < resolver->mod_file_paths->len))) {
            /* pass */
            c_gen->cur_src_file = ({ TrStr _at_t2434 = (List_TrStr_get(resolver->mod_file_paths, mi)); __auto_type _wr = (to_fwd_slashes(_at_t2434)); _tr_str_release(_at_t2434); _wr; });
        }
        /* pass */
    ...
    while ((k2 < resolver->all_decl_modules->len)) {
        ...
        if ((strcmp(_tr_strz(List_TrStr_get(resolver->all_decl_modules, k2)), _tr_strz(_tr_str_lit(""))) == 0)) {
            /* pass */
            __auto_type _t2435 = (*((Decl*)List_ptr_get(resolver->all_decls, k2)));
            if (_t2435.tag == Decl_DClass) {
                __auto_type c = _t2435.data.DClass.cls;
                /* pass */
                _tr_dict_set(main_class_set, _tr_strz(c->name), true);
            } else if (_t2435.tag == Decl_DFunction) {
                __auto_type f = _t2435.data.DFunction.func;
                /* pass */
                _tr_dict_set(main_fn_set, _tr_strz(f->name), true);
                _tr_obj_release(f, _trdrop_FunctionDef);
            } else if (_t2435.tag == Decl_DEnum) {
                __auto_type e = _t2435.data.DEnum.enm;
                /* pass */
                _tr_dict_set(main_class_set, _tr_strz(e->name), true);
            } else if (_t2435.tag == Decl_DInterface) {
                __auto_type iface = _t2435.data.DInterface.iface;
                /* pass */
                _tr_dict_set(main_class_set, _tr_strz(iface->name), true);
            } else if (_t2435.tag == Decl_DExtend) {
                __auto_type target = _t2435.data.DExtend.target;
__auto_type methods = _t2435.data.DExtend.methods;
                /* pass */
    ...
    if ((strcmp(_tr_strz(emit_ld), _tr_strz(_tr_str_lit(""))) != 0)) {
        ...
        if (_tr_str_contains(_tr_strz(target), _tr_strz(_tr_str_lit("riscv")))) {
            /* pass */
            ({ TrStr _at_t2436 = (linker_script_riscv()); write_file(emit_ld, _at_t2436); _tr_str_release(_at_t2436); });
        } else {
            /* pass */
            ({ TrStr _at_t2437 = (linker_script_cortex_m()); write_file(emit_ld, _at_t2437); _tr_str_release(_at_t2437); });
        }
        /* pass */
    ...
    if ((strcmp(_tr_strz(emit_mode), _tr_strz(_tr_str_lit("c"))) == 0)) {
        ...
        while ((pi < (all_c_files->len - 1LL))) {
            /* pass */
            ({ TrStr _at_t2438 = (List_TrStr_get(all_c_files, pi)); ({ printf("%s", _tr_strz(({ TrStr _cr = (get_filename(_at_t2438)); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("  ")), _cr.data); _tr_str_release(_cr); _cres; }))); printf("\n"); }); _tr_str_release(_at_t2438); });
            /* pass */
            pi = (pi + 1LL);
        }
        /* pass */
    ...
    if ((strcmp(_tr_strz(output_path), _tr_strz(_tr_str_lit(""))) != 0)) {
        /* pass */
        TrStr _strtmp_t2439 = _tr_str_retain(output_path);
        _tr_str_release(exe_name);
        exe_name = _strtmp_t2439;
    } else {
        /* pass */
        TrStr _strtmp_t2441 = ({ TrStr _at_t2440 = (get_filename(input_path)); __auto_type _wr = (strip_extension(_at_t2440)); _tr_str_release(_at_t2440); _wr; });
        _tr_str_release(exe_name);
        exe_name = _strtmp_t2441;
    }
    /* pass */
    if ((strcmp(_tr_strz(exe_name), _tr_strz(_tr_str_lit(""))) == 0)) {
        /* pass */
        TrStr _strtmp_t2442 = _tr_str_lit("a");
        _tr_str_release(exe_name);
            ...
            if ((((long long)((*(en_p + (en_len - 3LL))))) == 101LL)) {
                ...
                if ((((long long)((*(en_p + (en_len - 2LL))))) == 120LL)) {
                    ...
                    if ((((long long)((*(en_p + (en_len - 1LL))))) == 101LL)) {
                        /* pass */
                        TrStr _strtmp_t2443 = _tr_str_wrap(_tr_str_slice(_tr_strz(exe_name), 0LL, (en_len - 4LL)));
                        _tr_str_release(exe_name);
    ...
    if (lib_mode) {
        ...
        if (_tr_is_windows()) {
            /* pass */
            TrStr _strtmp_t2444 = _tr_str_lit(".dll");
            _tr_str_release(exe_ext);
            exe_ext = _strtmp_t2444;
        } else {
            /* pass */
            TrStr _strtmp_t2445 = _tr_str_lit(".so");
            _tr_str_release(exe_ext);
            exe_ext = _strtmp_t2445;
        }
    } else if ((!_tr_is_windows())) {
        /* pass */
        TrStr _strtmp_t2446 = _tr_str_lit("");
        _tr_str_release(exe_ext);
    ...
    if ((strcmp(_tr_strz(output_path), _tr_strz(_tr_str_lit(""))) != 0)) {
        /* pass */
        TrStr _strtmp_t2447 = _tr_strx_concat(_tr_strz(exe_name), _tr_strz(exe_ext));
        _tr_str_release(exe_path);
        exe_path = _strtmp_t2447;
    } else {
        /* pass */
        TrStr _strtmp_t2448 = ({ TrStr _cl = (_tr_strx_concat(_tr_strz(build_dir), _tr_strz(exe_name))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(exe_ext)); _tr_str_release(_cl); _cres; });
        _tr_str_release(exe_path);
    ...
    if (lib_mode) {
        ...
        if ((strcmp(_tr_strz(output_path), _tr_strz(_tr_str_lit(""))) != 0)) {
            /* pass */
            TrStr _strtmp_t2449 = _tr_strx_concat(_tr_strz(exe_name), _tr_strz(_tr_str_lit(".h")));
            _tr_str_release(hdr_path);
            hdr_path = _strtmp_t2449;
        }
        /* pass */
    ...
    if (run_after) {
        ...
        if (_tr_is_windows()) {
            /* pass */
            TrStr _strtmp_t2450 = path_to_native(run_path);
            _tr_str_release(run_path);
            run_path = _strtmp_t2450;
        }
        /* pass */
        long long run_rc = ({ TrStr _aet_t2451 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("\"")), _tr_strz(run_path))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\""))); _tr_str_release(_cl); _cres; })); __auto_type _wr = (_tr_system(_aet_t2451.data)); _tr_str_release(_aet_t2451); _wr; });
        /* pass */
        exit((int)(run_rc));
    }
    _tr_str_release(subcmd);
