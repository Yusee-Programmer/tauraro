...
__attribute__((hot)) char* read_runtime_header(char* bin_path, char* input_path) {
    ...
    /* pass */
    char* bin_dir = dir_of_path(bin_path);
    /* pass */
...
__attribute__((hot)) int main(int argc, char** argv) {
    ...
    /* pass */
    char* bin_dir = strip_trailing_sep_inline(dir_of_path(List_str_get(args, 0LL)));
    /* pass */
    ...
    while ((mi < resolver->mod_dot_paths->len)) {
        ...
        while ((k < resolver->all_decl_modules->len)) {
            ...
            if ((strcmp((char*)List_str_get(resolver->all_decl_modules, k), (char*)dot_path) == 0)) {
                /* pass */
                __auto_type _t259 = (*((Decl*)List_ptr_get(resolver->all_decls, k)));
                if (_t259.tag == Decl_DClass) {
                    __auto_type c = _t259.data.DClass.cls;
                    /* pass */
                    _tr_dict_set(class_set, c->name, true);
                } else if (_t259.tag == Decl_DFunction) {
                    __auto_type f = _t259.data.DFunction.func;
                    /* pass */
                    _tr_dict_set(fn_set, f->name, true);
                } else if (_t259.tag == Decl_DEnum) {
                    __auto_type e = _t259.data.DEnum.enm;
                    /* pass */
                    _tr_dict_set(class_set, e->name, true);
                } else if (_t259.tag == Decl_DInterface) {
                    __auto_type iface = _t259.data.DInterface.iface;
                    /* pass */
                    _tr_dict_set(class_set, iface->name, true);
                } else if (_t259.tag == Decl_DExtend) {
                    __auto_type target = _t259.data.DExtend.target;
__auto_type methods = _t259.data.DExtend.methods;
                    /* pass */
    ...
    while ((k2 < resolver->all_decl_modules->len)) {
        ...
        if ((strcmp((char*)List_str_get(resolver->all_decl_modules, k2), (char*)"") == 0)) {
            /* pass */
            __auto_type _t260 = (*((Decl*)List_ptr_get(resolver->all_decls, k2)));
            if (_t260.tag == Decl_DClass) {
                __auto_type c = _t260.data.DClass.cls;
                /* pass */
                _tr_dict_set(main_class_set, c->name, true);
            } else if (_t260.tag == Decl_DFunction) {
                __auto_type f = _t260.data.DFunction.func;
                /* pass */
            ...
            } else if (_t260.tag == Decl_DExtend) {
                __auto_type target = _t260.data.DExtend.target;
__auto_type methods = _t260.data.DExtend.methods;
                /* pass */
