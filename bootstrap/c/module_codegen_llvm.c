...
__attribute__((hot)) void LlvmGenerator_emit_type_decls(LlvmGenerator* self, HirProgram* prog) {
    ...
    while ((i < prog->classes->len)) {
        ...
        while ((fi < c->fields->len)) {
            ...
            /* pass */
            ({ TrStr _at_t2207 = (llvm_type(((HirField*)List_ptr_get(c->fields, fi))->ty)); LlvmGenerator_w(self, _at_t2207); _tr_str_release(_at_t2207); });
            /* pass */
            first = false;
            /* pass */
            fi = (fi + 1LL);
...
__attribute__((hot)) void LlvmGenerator_emit_func_sig(LlvmGenerator* self, HirFunction* f, TrStr class_name) {
    ...
    while ((i < f->params->len)) {
        ...
        /* pass */
        ({ TrStr _at_t2208 = (llvm_type(p->ty)); LlvmGenerator_w(self, _at_t2208); _tr_str_release(_at_t2208); });
        /* pass */
        LlvmGenerator_w(self, _tr_str_lit(" %"));
        /* pass */
...
__attribute__((hot)) TrStr LlvmGenerator_gen_expr(LlvmGenerator* self, HirExpr* e_ptr) {
    ...
    __auto_type e = (*e_ptr);
    /* pass */
    __auto_type _t2209 = e;
    if (_t2209.tag == HirExpr_ELitInt) {
        __auto_type v = _t2209.data.ELitInt.val;
__auto_type ty = _t2209.data.ELitInt.ty;
        /* pass */
        ...
        /* pass */
        ({ TrStr _at_t2210 = (_tr_str_wrap(_tr_int_to_str((long long)(v)))); LlvmGenerator_w(self, _at_t2210); _tr_str_release(_at_t2210); });
        /* pass */
        LlvmGenerator_w(self, _tr_str_lit("\n"));
        /* pass */
        _tr_str_release(lty);
        return reg;
    } else if (_t2209.tag == HirExpr_ELitBool) {
        __auto_type v = _t2209.data.ELitBool.val;
        /* pass */
        TrStr reg = LlvmGenerator_next_reg(self);
        /* pass */
        ...
        if (v) {
            /* pass */
            TrStr _strtmp_t2211 = _tr_str_lit("1");
        ...
        _tr_str_release(bval);
        return reg;
    } else if (_t2209.tag == HirExpr_ELitFloat) {
        __auto_type v = _t2209.data.ELitFloat.val;
__auto_type ty = _t2209.data.ELitFloat.ty;
        /* pass */
        TrStr reg = LlvmGenerator_next_reg(self);
        /* pass */
        LlvmGenerator_w(self, _tr_str_lit("    "));
        ...
        /* pass */
        ({ TrStr _at_t2212 = (_tr_str_wrap(_tr_float_to_str((double)(v)))); LlvmGenerator_w(self, _at_t2212); _tr_str_release(_at_t2212); });
        /* pass */
        LlvmGenerator_w(self, _tr_str_lit("\n"));
        /* pass */
        return reg;
    } else if (_t2209.tag == HirExpr_ELitStr) {
        __auto_type v = _t2209.data.ELitStr.val;
        /* pass */
        TrStr reg = LlvmGenerator_next_reg(self);
        /* pass */
        ...
        /* pass */
        ({ TrStr _at_t2213 = (_tr_str_wrap(_tr_int_to_str((long long)(len)))); LlvmGenerator_w(self, _at_t2213); _tr_str_release(_at_t2213); });
        /* pass */
        ...
        return reg;
    } else if (_t2209.tag == HirExpr_ELitNone) {
        __auto_type _ = _t2209.data.ELitNone.ty;
        /* pass */
        return _tr_str_lit("null");
    } else if (_t2209.tag == HirExpr_EIdent) {
        __auto_type name = _t2209.data.EIdent.name;
__auto_type is_move = _t2209.data.EIdent.is_move;
        ...
        /* pass */
        return reg;
    } else if (_t2209.tag == HirExpr_EBinOp) {
        __auto_type op = _t2209.data.EBinOp.op;
__auto_type left = _t2209.data.EBinOp.left;
__auto_type right = _t2209.data.EBinOp.right;
__auto_type ty = _t2209.data.EBinOp.ty;
        /* pass */
        return LlvmGenerator_gen_binop_llvm(self, op, left, right, ty);
    } else if (_t2209.tag == HirExpr_ECall) {
        __auto_type callee = _t2209.data.ECall.callee;
__auto_type args = _t2209.data.ECall.args;
        /* pass */
...
__attribute__((hot)) TrStr LlvmGenerator_gen_binop_llvm(LlvmGenerator* self, TrStr op, HirExpr* left, HirExpr* right, AstType* ty) {
    ...
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("+"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2214 = _tr_str_lit("add");
    ...
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("-"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2215 = _tr_str_lit("sub");
        _tr_str_release(instr);
        instr = _strtmp_t2215;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("*"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2216 = _tr_str_lit("mul");
        _tr_str_release(instr);
        instr = _strtmp_t2216;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("/"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2217 = _tr_str_lit("sdiv");
        _tr_str_release(instr);
        instr = _strtmp_t2217;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("%"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2218 = _tr_str_lit("srem");
        _tr_str_release(instr);
        instr = _strtmp_t2218;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("&"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2219 = _tr_str_lit("and");
        _tr_str_release(instr);
        instr = _strtmp_t2219;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("|"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2220 = _tr_str_lit("or");
        _tr_str_release(instr);
        instr = _strtmp_t2220;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("^"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2221 = _tr_str_lit("xor");
        _tr_str_release(instr);
        instr = _strtmp_t2221;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("<<"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2222 = _tr_str_lit("shl");
        _tr_str_release(instr);
        instr = _strtmp_t2222;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit(">>"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2223 = _tr_str_lit("ashr");
        _tr_str_release(instr);
        instr = _strtmp_t2223;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("=="))) == 0)) {
        /* pass */
        TrStr _strtmp_t2224 = _tr_str_lit("icmp eq");
        _tr_str_release(instr);
        instr = _strtmp_t2224;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("!="))) == 0)) {
        /* pass */
        TrStr _strtmp_t2225 = _tr_str_lit("icmp ne");
        _tr_str_release(instr);
        instr = _strtmp_t2225;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("<"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2226 = _tr_str_lit("icmp slt");
        _tr_str_release(instr);
        instr = _strtmp_t2226;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit("<="))) == 0)) {
        /* pass */
        TrStr _strtmp_t2227 = _tr_str_lit("icmp sle");
        _tr_str_release(instr);
        instr = _strtmp_t2227;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit(">"))) == 0)) {
        /* pass */
        TrStr _strtmp_t2228 = _tr_str_lit("icmp sgt");
        _tr_str_release(instr);
        instr = _strtmp_t2228;
    }
    /* pass */
    if ((strcmp(_tr_strz(op), _tr_strz(_tr_str_lit(">="))) == 0)) {
        /* pass */
        TrStr _strtmp_t2229 = _tr_str_lit("icmp sge");
        _tr_str_release(instr);
        instr = _strtmp_t2229;
    }
    /* pass */
...
__attribute__((hot)) TrStr LlvmGenerator_gen_call_llvm(LlvmGenerator* self, HirExpr* callee, List_ptr* args) {
    ...
    /* pass */
    __auto_type _t2230 = (*callee);
    if (_t2230.tag == HirExpr_EIdent) {
        __auto_type n = _t2230.data.EIdent.name;
__auto_type is_move = _t2230.data.EIdent.is_move;
        TrStr _strtmp_t2231 = _tr_str_retain(n);
        _tr_str_release(callee_name);
        callee_name = _strtmp_t2231;
    } else if (1) {
    ...
    if ((strcmp(_tr_strz(callee_name), _tr_strz(_tr_str_lit(""))) == 0)) {
        /* pass */
        TrStr _strtmp_t2232 = _tr_str_lit("unknown");
        _tr_str_release(callee_name);
        callee_name = _strtmp_t2232;
    }
    /* pass */
    LlvmGenerator_w(self, _tr_str_lit("    "));
    /* pass */
    ...
    while ((i < args->len)) {
        ...
        /* pass */
        ({ TrStr _at_t2233 = (llvm_type(arg_ty)); LlvmGenerator_w(self, _at_t2233); _tr_str_release(_at_t2233); });
        /* pass */
        LlvmGenerator_w(self, _tr_str_lit(" "));
        /* pass */
...
__attribute__((hot)) void LlvmGenerator_gen_stmt(LlvmGenerator* self, HirStmt* s_ptr) {
    ...
    __auto_type s = (*s_ptr);
    /* pass */
    __auto_type _t2234 = s;
    if (_t2234.tag == HirStmt_SExpr) {
        __auto_type e = _t2234.data.SExpr.expr;
        /* pass */
        LlvmGenerator_gen_expr(self, e);
    } else if (_t2234.tag == HirStmt_SReturn) {
        __auto_type e = _t2234.data.SReturn.val;
        /* pass */
    ...
    } else if (_t2234.tag == HirStmt_SLet) {
        __auto_type name = _t2234.data.SLet.name;
__auto_type ownership = _t2234.data.SLet.ownership;
__auto_type is_mut = _t2234.data.SLet.is_mut;
__auto_type is_const = _t2234.data.SLet.is_const;
__auto_type is_shared = _t2234.data.SLet.is_shared;
__auto_type ty = _t2234.data.SLet.ty;
__auto_type val = _t2234.data.SLet.val;
        ...
        /* pass */
        ({ TrStr _at_t2236 = (llvm_type(ty)); LlvmGenerator_w(self, _at_t2236); _tr_str_release(_at_t2236); });
        /* pass */
        LlvmGenerator_w(self, _tr_str_lit("\n"));
        /* pass */
        if ((((unsigned long long)(val)) != ((unsigned long long)(0LL)))) {
            ...
            /* pass */
            ({ TrStr _at_t2237 = (llvm_type(ty)); LlvmGenerator_w(self, _at_t2237); _tr_str_release(_at_t2237); });
            /* pass */
            LlvmGenerator_w(self, _tr_str_lit(" "));
            /* pass */
            LlvmGenerator_w(self, v);
    ...
    } else if (_t2234.tag == HirStmt_SIf) {
...
__auto_type else_b = _t2234.data.SIf.else_b;
    ...
    } else if (_t2234.tag == HirStmt_SWhile) {
        __auto_type cond = _t2234.data.SWhile.cond;
__auto_type body = _t2234.data.SWhile.body;
        /* pass */
        ...
        _tr_str_release(end_lbl);
        _tr_str_release(cond_v);
    } else if (_t2234.tag == HirStmt_SBreak) {
        __auto_type _ = _t2234.data.SBreak.val;
        /* pass */
        LlvmGenerator_w(self, _tr_str_lit("    ; break (unresolved in stub)\n"));
    } else if (_t2234.tag == HirStmt_SContinue) {
        /* pass */
        LlvmGenerator_w(self, _tr_str_lit("    ; continue (unresolved in stub)\n"));
    } else if (_t2234.tag == HirStmt_SPass) {
        /* pass */
        /* pass */
    } else if (1) {
...
__attribute__((hot)) TrStr LlvmGenerator_generate(LlvmGenerator* self, HirProgram* prog) {
    ...
    while ((i < prog->functions->len)) {
        ...
        if (f->is_extern) {
            ...
            /* pass */
            ({ TrStr _at_t2238 = (llvm_type(f->ret_ty)); LlvmGenerator_w(self, _at_t2238); _tr_str_release(_at_t2238); });
            /* pass */
            LlvmGenerator_w(self, _tr_str_lit(" @"));
            /* pass */
