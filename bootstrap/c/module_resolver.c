...
__attribute__((hot)) Program* ModuleResolver_resolve_main(ModuleResolver* self, TrStr main_path) {
    ...
    if ((dir_end > 0LL)) {
        ...
        /* unsafe block */
        /* pass */
        TrStr _strtmp_t213 = _tr_str_wrap(_tr_str_slice(_tr_strz(main_path), 0LL, dir_end));
        _tr_str_release(dir_str);
        dir_str = _strtmp_t213;
        /* pass */
        ...
        if ((par_end > 0LL)) {
            ...
            /* unsafe block */
            /* pass */
            TrStr _strtmp_t214 = _tr_str_wrap(_tr_str_slice(_tr_strz(dir_str), 0LL, par_end));
            _tr_str_release(parent_dir);
            parent_dir = _strtmp_t214;
            /* pass */
            ...
            if ((gp_end > 0LL)) {
                ...
                /* unsafe block */
                /* pass */
                TrStr _strtmp_t215 = _tr_str_wrap(_tr_str_slice(_tr_strz(parent_dir), 0LL, gp_end));
        ...
        if ((((unsigned long long)(vh)) != ((unsigned long long)(0LL)))) {
            ...
            while ((strcmp(_tr_strz(vname), _tr_strz(_tr_str_lit(""))) != 0)) {
                ...
                /* pass */
                TrStr _strtmp_t216 = _tr_str_wrap(_tr_readdir(vh));
...
__attribute__((hot)) void ModuleResolver_resolve_file(ModuleResolver* self, TrStr path, bool is_root) {
    ...
    if ((strcmp(_tr_strz(source), _tr_strz(_tr_str_lit(""))) == 0)) {
        ...
        if ((!has_tr)) {
            /* pass */
            TrStr _strtmp_t218 = ({ TrStr _at_t217 = (_tr_strx_concat(_tr_strz(path), _tr_strz(_tr_str_lit(".tr")))); __auto_type _wr = (read_file(_at_t217)); _tr_str_release(_at_t217); _wr; });
    ...
    while ((i < Program_len(prog))) {
        ...
        /* pass */
        __auto_type _t219 = (*decl_ptr);
        if (_t219.tag == Decl_DImport) {
            __auto_type mod_path = _t219.data.DImport.path;
__auto_type alias = _t219.data.DImport.alias;
            /* pass */
            ModuleResolver_resolve_module_path(self, mod_path);
        } else if (_t219.tag == Decl_DFromImport) {
            __auto_type mod_path = _t219.data.DFromImport.path;
__auto_type items = _t219.data.DFromImport.items;
            /* pass */
            ModuleResolver_resolve_module_path(self, mod_path);
        } else if (1) {
            __auto_type _ = _t219;
...
__attribute__((hot)) void ModuleResolver_resolve_module_path(ModuleResolver* self, TrStr mod_path) {
    ...
    while ((sidx < self->search_paths->len)) {
        ...
        if (file_exists(pkg_src_path)) {
            /* pass */
            ({ TrStr _at_t220 = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(base), _tr_strz(_tr_str_lit("/")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(dir_path)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("/src"))); _tr_str_release(_cl); _cres; })); ModuleResolver_add_search_path(self, _at_t220); _tr_str_release(_at_t220); });
            /* pass */
...
__attribute__((hot)) bool decl_is_pub(Decl d) {
    /* pass */
    __auto_type _t221 = d;
    if (_t221.tag == Decl_DFunction) {
        __auto_type f = _t221.data.DFunction.func;
        /* pass */
        return f->is_public;
    } else if (_t221.tag == Decl_DClass) {
        __auto_type c = _t221.data.DClass.cls;
        /* pass */
        return c->is_public;
    } else if (_t221.tag == Decl_DEnum) {
        __auto_type e = _t221.data.DEnum.enm;
        /* pass */
        return e->is_public;
    } else if (_t221.tag == Decl_DInterface) {
        __auto_type i = _t221.data.DInterface.iface;
        /* pass */
        return i->is_public;
    } else if (_t221.tag == Decl_DActor) {
        __auto_type c = _t221.data.DActor.cls;
        /* pass */
        return c->is_public;
    } else if (_t221.tag == Decl_DExtern) {
        __auto_type abi = _t221.data.DExtern.abi;
__auto_type funcs = _t221.data.DExtern.functions;
        /* pass */
        return true;
    } else if (_t221.tag == Decl_DExtend) {
        __auto_type target = _t221.data.DExtend.target;
__auto_type methods = _t221.data.DExtend.methods;
        /* pass */
        return true;
    } else if (_t221.tag == Decl_DDecoratorDef) {
        __auto_type f = _t221.data.DDecoratorDef.func;
        /* pass */
        return f->is_public;
    } else if (1) {
        __auto_type _ = _t221;
