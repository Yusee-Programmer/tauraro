    ...
    if ((dir_end > 0LL)) {
        ...
        if ((((unsigned long long)(vh)) != ((unsigned long long)(0LL)))) {
            ...
            while ((strcmp(_tr_strz(vname), _tr_strz(_tr_str_lit(""))) != 0)) {
                ...
                /* pass */
                TrStr _strtmp_t238 = _tr_str_wrap(_tr_readdir(vh));
...
__attribute__((hot)) void ModuleResolver_resolve_file(ModuleResolver* self, TrStr path, bool is_root) {
    ...
    while ((i < Program_len(prog))) {
        ...
        /* pass */
        __auto_type _t241 = (*decl_ptr);
        if (_t241.tag == Decl_DImport) {
            __auto_type mod_path = _t241.data.DImport.path;
__auto_type alias = _t241.data.DImport.alias;
            /* pass */
            ModuleResolver_resolve_module_path(self, mod_path);
        } else if (_t241.tag == Decl_DFromImport) {
            __auto_type mod_path = _t241.data.DFromImport.path;
__auto_type items = _t241.data.DFromImport.items;
            /* pass */
            ModuleResolver_resolve_module_path(self, mod_path);
        } else if (1) {
            __auto_type _ = _t241;
...
__attribute__((hot)) void ModuleResolver__mark_decl_lib(ModuleResolver* self, Decl* dp) {
    ...
    /* pass */
    __auto_type _t244 = (*dp);
    if (_t244.tag == Decl_DFunction) {
        __auto_type f = _t244.data.DFunction.func;
        f->is_lib = true;
    } else if (_t244.tag == Decl_DClass) {
        __auto_type c = _t244.data.DClass.cls;
        ModuleResolver__mark_methods_lib(self, c);
    } else if (_t244.tag == Decl_DActor) {
        __auto_type c = _t244.data.DActor.cls;
        ModuleResolver__mark_methods_lib(self, c);
    } else if (_t244.tag == Decl_DExtend) {
        __auto_type ms = _t244.data.DExtend.methods;
        /* pass */
        long long i = 0LL;
        /* pass */
        while ((i < ms->len)) {
...
__attribute__((hot)) void ModuleResolver_resolve_module_path(ModuleResolver* self, TrStr mod_path) {
    ...
    while ((sidx < self->search_paths->len)) {
        ...
        if (file_exists(pkg_src_path)) {
            /* pass */
            ({ TrStr _at_t245 = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(base), _tr_strz(_tr_str_lit("/")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(dir_path)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("/src"))); _tr_str_release(_cl); _cres; })); ModuleResolver_add_search_path(self, _at_t245); _tr_str_release(_at_t245); });
            /* pass */
...
__attribute__((hot)) bool decl_is_pub(Decl d) {
    /* pass */
    __auto_type _t246 = d;
    if (_t246.tag == Decl_DFunction) {
        __auto_type f = _t246.data.DFunction.func;
        /* pass */
        return f->is_public;
    } else if (_t246.tag == Decl_DClass) {
        __auto_type c = _t246.data.DClass.cls;
        /* pass */
        return c->is_public;
    } else if (_t246.tag == Decl_DEnum) {
        __auto_type e = _t246.data.DEnum.enm;
        /* pass */
        return e->is_public;
    } else if (_t246.tag == Decl_DInterface) {
        __auto_type i = _t246.data.DInterface.iface;
        /* pass */
        return i->is_public;
    } else if (_t246.tag == Decl_DActor) {
        __auto_type c = _t246.data.DActor.cls;
        /* pass */
        return c->is_public;
    } else if (_t246.tag == Decl_DExtern) {
        __auto_type abi = _t246.data.DExtern.abi;
__auto_type funcs = _t246.data.DExtern.functions;
        /* pass */
        return true;
    } else if (_t246.tag == Decl_DExtend) {
        __auto_type target = _t246.data.DExtend.target;
__auto_type methods = _t246.data.DExtend.methods;
        /* pass */
        return true;
    } else if (_t246.tag == Decl_DDecoratorDef) {
        __auto_type f = _t246.data.DDecoratorDef.func;
        /* pass */
        return f->is_public;
    } else if (1) {
        __auto_type _ = _t246;
