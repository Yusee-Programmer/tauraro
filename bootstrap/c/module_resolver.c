...
__attribute__((hot)) bool ModuleResolver__source_is_trusted(ModuleResolver* self, TrStr source) {
    ...
    /* pass */
    return ({ TrStr _at_t225 = (_tr_str_wrap(_tr_str_slice(_tr_strz(source), 0LL, lim))); __auto_type _wr = (ModuleResolver__contains(self, _at_t225, _tr_str_lit("@trusted"))); _tr_str_release(_at_t225); _wr; });
}
...
__attribute__((hot)) bool ModuleResolver__contains(ModuleResolver* self, TrStr hay, TrStr needle) {
    ...
    while (((i + nl) <= hl)) {
        /* pass */
        if (({ TrStr _wt_t226 = (_tr_str_wrap(_tr_str_slice(_tr_strz(hay), i, (i + nl)))); __auto_type _wr = ((strcmp(_wt_t226.data, _tr_strz(needle)) == 0)); _tr_str_release(_wt_t226); _wr; })) {
            /* pass */
            return true;
        }
        /* pass */
...
__attribute__((hot)) void ModuleResolver__mark_decl_lib(ModuleResolver* self, Decl* dp) {
    ...
    /* pass */
    __auto_type _t227 = (*dp);
    if (_t227.tag == Decl_DFunction) {
        __auto_type f = _t227.data.DFunction.func;
        f->is_lib = true;
    } else if (_t227.tag == Decl_DClass) {
        __auto_type c = _t227.data.DClass.cls;
        ModuleResolver__mark_methods_lib(self, c);
    } else if (_t227.tag == Decl_DActor) {
        __auto_type c = _t227.data.DActor.cls;
        ModuleResolver__mark_methods_lib(self, c);
    } else if (_t227.tag == Decl_DExtend) {
        __auto_type ms = _t227.data.DExtend.methods;
        /* pass */
        long long i = 0LL;
        /* pass */
        while ((i < ms->len)) {
...
__attribute__((hot)) void ModuleResolver_resolve_module_path(ModuleResolver* self, TrStr mod_path) {
    ...
    while ((sidx < self->search_paths->len)) {
        ...
        if (file_exists(pkg_src_path)) {
            /* pass */
            ({ TrStr _at_t228 = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(base), _tr_strz(_tr_str_lit("/")))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(dir_path)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("/src"))); _tr_str_release(_cl); _cres; })); ModuleResolver_add_search_path(self, _at_t228); _tr_str_release(_at_t228); });
            /* pass */
...
__attribute__((hot)) bool decl_is_pub(Decl d) {
    /* pass */
    __auto_type _t229 = d;
    if (_t229.tag == Decl_DFunction) {
        __auto_type f = _t229.data.DFunction.func;
        /* pass */
        return f->is_public;
    } else if (_t229.tag == Decl_DClass) {
        __auto_type c = _t229.data.DClass.cls;
        /* pass */
        return c->is_public;
    } else if (_t229.tag == Decl_DEnum) {
        __auto_type e = _t229.data.DEnum.enm;
        /* pass */
        return e->is_public;
    } else if (_t229.tag == Decl_DInterface) {
        __auto_type i = _t229.data.DInterface.iface;
        /* pass */
        return i->is_public;
    } else if (_t229.tag == Decl_DActor) {
        __auto_type c = _t229.data.DActor.cls;
        /* pass */
        return c->is_public;
    } else if (_t229.tag == Decl_DExtern) {
        __auto_type abi = _t229.data.DExtern.abi;
__auto_type funcs = _t229.data.DExtern.functions;
        /* pass */
        return true;
    } else if (_t229.tag == Decl_DExtend) {
        __auto_type target = _t229.data.DExtend.target;
__auto_type methods = _t229.data.DExtend.methods;
        /* pass */
        return true;
    } else if (_t229.tag == Decl_DDecoratorDef) {
        __auto_type f = _t229.data.DDecoratorDef.func;
        /* pass */
        return f->is_public;
    } else if (1) {
        __auto_type _ = _t229;
