...
__attribute__((hot)) void GpuEmitter_add_var(GpuEmitter* self, TrStr name, AstType* ty) {
    ...
    if (((strcmp(_tr_strz(ty->name), _tr_strz(_tr_str_lit("Pointer"))) == 0) && (ty->args->len > 0LL))) {
        ...
        List_TrStr_append(self->var_names, name);
        /* pass */
        ({ TrStr _at_t3500 = (_gpu_scalar_ty(ty->name)); List_TrStr_append(self->var_ll, _at_t3500); _tr_str_release(_at_t3500); });
        /* pass */
...
__attribute__((hot)) void GpuEmitter_scan_vars_stmt(GpuEmitter* self, HirStmt s) {
    /* pass */
    __auto_type _t3501 = s;
    if (_t3501.tag == HirStmt_SLet) {
        __auto_type name = _t3501.data.SLet.name;
__auto_type ownership = _t3501.data.SLet.ownership;
__auto_type is_mut = _t3501.data.SLet.is_mut;
__auto_type is_const = _t3501.data.SLet.is_const;
__auto_type is_shared = _t3501.data.SLet.is_shared;
__auto_type ty = _t3501.data.SLet.ty;
__auto_type val = _t3501.data.SLet.val;
        /* pass */
        AstType* vt = ty;
        /* pass */
    ...
    } else if (_t3501.tag == HirStmt_SIf) {
        __auto_type cond = _t3501.data.SIf.cond;
__auto_type then_b = _t3501.data.SIf.then_b;
__auto_type else_b = _t3501.data.SIf.else_b;
        /* pass */
        GpuEmitter_scan_vars_block(self, then_b);
        /* pass */
        GpuEmitter_scan_vars_block(self, else_b);
    } else if (_t3501.tag == HirStmt_SWhile) {
        __auto_type cond = _t3501.data.SWhile.cond;
__auto_type body = _t3501.data.SWhile.body;
        /* pass */
        GpuEmitter_scan_vars_block(self, body);
    } else if (_t3501.tag == HirStmt_SUnsafe) {
        __auto_type body = _t3501.data.SUnsafe.body;
        /* pass */
        GpuEmitter_scan_vars_block(self, body);
    } else if (1) {
...
__attribute__((hot)) TrStr GpuEmitter_emit_kernel(GpuEmitter* self, HirFunction* f) {
    ...
    if ((strcmp(_tr_strz(self->target), _tr_strz(_tr_str_lit("nvptx"))) == 0)) {
        /* pass */
        TrStr _strtmp_t3502 = _tr_str_lit("ptx_kernel");
        _tr_str_release(cc);
        cc = _strtmp_t3502;
    }
    /* pass */
    ({ TrStr _at_t3503 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("define ")), _tr_strz(cc))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" void @"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(f->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("("))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3503); _tr_str_release(_at_t3503); });
    /* pass */
    long long pj = 0LL;
    /* pass */
    while ((pj < f->params->len)) {
        ...
        /* pass */
        ({ TrStr _at_t3504 = (({ TrStr _cl = (({ TrStr _cl = (GpuEmitter_var_slot_ty(self, pp->name)); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" %arg_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pp->name)); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3504); _tr_str_release(_at_t3504); });
        /* pass */
        pj = (pj + 1LL);
    }
    /* pass */
    ...
    while ((vi < self->var_names->len)) {
        /* pass */
        ({ TrStr _at_t3505 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cr = (List_TrStr_get(self->var_names, vi)); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("  %var_")), _cr.data); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" = alloca "))); _tr_str_release(_cl); _cres; })); TrStr _cr = (List_TrStr_get(self->var_ll, vi)); TrStr _cres = _tr_strx_concat(_cl.data, _cr.data); _tr_str_release(_cl); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3505); _tr_str_release(_at_t3505); });
        /* pass */
        vi = (vi + 1LL);
    }
    /* pass */
    ...
    while ((pk < f->params->len)) {
        ...
        /* pass */
        ({ TrStr _at_t3506 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cr = (GpuEmitter_var_slot_ty(self, pn)); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("  store ")), _cr.data); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" %arg_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pn)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(", ptr %var_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pn)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3506); _tr_str_release(_at_t3506); });
        /* pass */
        pk = (pk + 1LL);
        _tr_str_release(pn);
    }
...
__attribute__((hot)) TrStr GpuEmitter_emit_device_fn(GpuEmitter* self, HirFunction* f) {
    ...
    GpuEmitter_scan_vars_block(self, f->body);
    /* pass */
    ({ TrStr _at_t3507 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("define ")), _tr_strz(self->ret_llty))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" @"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(f->name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("("))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3507); _tr_str_release(_at_t3507); });
    /* pass */
    long long pj = 0LL;
    /* pass */
    while ((pj < f->params->len)) {
        ...
        /* pass */
        ({ TrStr _at_t3508 = (({ TrStr _cl = (({ TrStr _cl = (GpuEmitter_var_slot_ty(self, pp->name)); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" %arg_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pp->name)); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3508); _tr_str_release(_at_t3508); });
        /* pass */
        pj = (pj + 1LL);
    }
    /* pass */
    ...
    while ((vi < self->var_names->len)) {
        /* pass */
        ({ TrStr _at_t3509 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cr = (List_TrStr_get(self->var_names, vi)); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("  %var_")), _cr.data); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" = alloca "))); _tr_str_release(_cl); _cres; })); TrStr _cr = (List_TrStr_get(self->var_ll, vi)); TrStr _cres = _tr_strx_concat(_cl.data, _cr.data); _tr_str_release(_cl); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3509); _tr_str_release(_at_t3509); });
        /* pass */
        vi = (vi + 1LL);
    }
    /* pass */
    ...
    while ((pk < f->params->len)) {
        ...
        TrStr pn = _tr_str_retain(((HirParam*)List_ptr_get(f->params, pk))->name);
        /* pass */
        ({ TrStr _at_t3510 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cr = (GpuEmitter_var_slot_ty(self, pn)); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("  store ")), _cr.data); _tr_str_release(_cr); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" %arg_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pn)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(", ptr %var_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(pn)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3510); _tr_str_release(_at_t3510); });
        /* pass */
        pk = (pk + 1LL);
        _tr_str_release(pn);
    }
    ...
    if ((strcmp(_tr_strz(self->ret_llty), _tr_strz(_tr_str_lit("void"))) == 0)) {
        ...
        /* pass */
        ({ TrStr _at_t3511 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  ret ")), _tr_strz(self->ret_llty))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" 0.0\n}\n\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3511); _tr_str_release(_at_t3511); });
    } else if ((strcmp(_tr_strz(self->ret_llty), _tr_strz(_tr_str_lit("ptr addrspace(1)"))) == 0)) {
        /* pass */
        ({ TrStr _at_t3512 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  ret ")), _tr_strz(self->ret_llty))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" null\n}\n\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3512); _tr_str_release(_at_t3512); });
    } else {
        /* pass */
        ({ TrStr _at_t3513 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  ret ")), _tr_strz(self->ret_llty))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" 0\n}\n\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3513); _tr_str_release(_at_t3513); });
    }
    /* pass */
    if ((!self->ok)) {
        /* pass */
...
__attribute__((hot)) void GpuEmitter_emit_stmt(GpuEmitter* self, HirStmt s) {
    /* pass */
    __auto_type _t3514 = s;
    if (_t3514.tag == HirStmt_SLet) {
        __auto_type name = _t3514.data.SLet.name;
__auto_type ownership = _t3514.data.SLet.ownership;
__auto_type is_mut = _t3514.data.SLet.is_mut;
__auto_type is_const = _t3514.data.SLet.is_const;
__auto_type is_shared = _t3514.data.SLet.is_shared;
__auto_type ty = _t3514.data.SLet.ty;
__auto_type val = _t3514.data.SLet.val;
        /* pass */
        ...
        /* pass */
        ({ TrStr _at_t3515 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  store ")), _tr_strz(slot))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(cv)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(", ptr %var_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3515); _tr_str_release(_at_t3515); });
        _tr_obj_release(v, _trdrop_GVal);
        _tr_str_release(slot);
        _tr_str_release(cv);
    } else if (_t3514.tag == HirStmt_SAssign) {
        __auto_type target = _t3514.data.SAssign.target;
__auto_type val = _t3514.data.SAssign.val;
        /* pass */
        GpuEmitter_emit_assign(self, (*target), val);
    } else if (_t3514.tag == HirStmt_SExpr) {
        __auto_type expr = _t3514.data.SExpr.expr;
        /* pass */
        GpuEmitter_emit_stmt_expr(self, (*expr));
    } else if (_t3514.tag == HirStmt_SIf) {
        __auto_type cond = _t3514.data.SIf.cond;
__auto_type then_b = _t3514.data.SIf.then_b;
__auto_type else_b = _t3514.data.SIf.else_b;
        /* pass */
        ...
        /* pass */
        ({ TrStr _at_t3516 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  br i1 ")), _tr_strz(cc))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(", label %"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(lt)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(", label %"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(le)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3516); _tr_str_release(_at_t3516); });
        /* pass */
        ({ TrStr _at_t3517 = (_tr_strx_concat(_tr_strz(lt), _tr_strz(_tr_str_lit(":\n")))); GpuEmitter_w(self, _at_t3517); _tr_str_release(_at_t3517); });
        /* pass */
        GpuEmitter_emit_block(self, then_b);
        /* pass */
        ({ TrStr _at_t3518 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  br label %")), _tr_strz(lend))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3518); _tr_str_release(_at_t3518); });
        /* pass */
        ({ TrStr _at_t3519 = (_tr_strx_concat(_tr_strz(le), _tr_strz(_tr_str_lit(":\n")))); GpuEmitter_w(self, _at_t3519); _tr_str_release(_at_t3519); });
        /* pass */
        GpuEmitter_emit_block(self, else_b);
        /* pass */
        ({ TrStr _at_t3520 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  br label %")), _tr_strz(lend))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3520); _tr_str_release(_at_t3520); });
        /* pass */
        ({ TrStr _at_t3521 = (_tr_strx_concat(_tr_strz(lend), _tr_strz(_tr_str_lit(":\n")))); GpuEmitter_w(self, _at_t3521); _tr_str_release(_at_t3521); });
        _tr_obj_release(c, _trdrop_GVal);
        _tr_str_release(cc);
        _tr_str_release(lt);
        _tr_str_release(le);
        _tr_str_release(lend);
    } else if (_t3514.tag == HirStmt_SWhile) {
        __auto_type cond = _t3514.data.SWhile.cond;
__auto_type body = _t3514.data.SWhile.body;
        /* pass */
        ...
        /* pass */
        ({ TrStr _at_t3522 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  br label %")), _tr_strz(lc))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3522); _tr_str_release(_at_t3522); });
        /* pass */
        ({ TrStr _at_t3523 = (_tr_strx_concat(_tr_strz(lc), _tr_strz(_tr_str_lit(":\n")))); GpuEmitter_w(self, _at_t3523); _tr_str_release(_at_t3523); });
        /* pass */
        GVal* cnd = GpuEmitter_emit_expr(self, cond);
        /* pass */
        TrStr cb = GpuEmitter_coerce_bool(self, cnd);
        /* pass */
        ({ TrStr _at_t3524 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  br i1 ")), _tr_strz(cb))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(", label %"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(lb)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(", label %"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(lend2)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3524); _tr_str_release(_at_t3524); });
        /* pass */
        ({ TrStr _at_t3525 = (_tr_strx_concat(_tr_strz(lb), _tr_strz(_tr_str_lit(":\n")))); GpuEmitter_w(self, _at_t3525); _tr_str_release(_at_t3525); });
        /* pass */
        GpuEmitter_emit_block(self, body);
        /* pass */
        ({ TrStr _at_t3526 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  br label %")), _tr_strz(lc))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3526); _tr_str_release(_at_t3526); });
        /* pass */
        ({ TrStr _at_t3527 = (_tr_strx_concat(_tr_strz(lend2), _tr_strz(_tr_str_lit(":\n")))); GpuEmitter_w(self, _at_t3527); _tr_str_release(_at_t3527); });
        _tr_str_release(lc);
        _tr_str_release(lb);
        _tr_str_release(lend2);
        _tr_obj_release(cnd, _trdrop_GVal);
        _tr_str_release(cb);
    } else if (_t3514.tag == HirStmt_SUnsafe) {
        __auto_type body = _t3514.data.SUnsafe.body;
        /* pass */
        GpuEmitter_emit_block(self, body);
    } else if (_t3514.tag == HirStmt_SReturn) {
        __auto_type val = _t3514.data.SReturn.val;
        /* pass */
        if (((strcmp(_tr_strz(self->ret_llty), _tr_strz(_tr_str_lit("void"))) == 0) || (((unsigned long long)(val)) == ((unsigned long long)(0LL))))) {
            ...
            if ((strcmp(_tr_strz(self->ret_llty), _tr_strz(_tr_str_lit("ptr addrspace(1)"))) == 0)) {
                /* pass */
                ({ TrStr _at_t3528 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  ret ptr addrspace(1) ")), _tr_strz(rv->val))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3528); _tr_str_release(_at_t3528); });
            } else {
                /* pass */
                TrStr cv = GpuEmitter_coerce(self, rv, self->ret_llty);
                /* pass */
                ({ TrStr _at_t3529 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  ret ")), _tr_strz(self->ret_llty))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(cv)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3529); _tr_str_release(_at_t3529); });
            }
        }
        /* pass */
        TrStr lu = GpuEmitter_newlbl(self);
        /* pass */
        ({ TrStr _at_t3530 = (_tr_strx_concat(_tr_strz(lu), _tr_strz(_tr_str_lit(":\n")))); GpuEmitter_w(self, _at_t3530); _tr_str_release(_at_t3530); });
        _tr_str_release(lu);
    } else if (_t3514.tag == HirStmt_SPass) {
        /* pass */
        /* pass */
    } else if (_t3514.tag == HirStmt_SLineMarker) {
        __auto_type n = _t3514.data.SLineMarker.n;
        /* pass */
        /* pass */
    } else if (1) {
        __auto_type _ = _t3514;
        /* pass */
...
__attribute__((hot)) void GpuEmitter_emit_assign(GpuEmitter* self, HirExpr target, HirExpr* val) {
    /* pass */
    __auto_type _t3531 = target;
    if (_t3531.tag == HirExpr_EIdent) {
        __auto_type name = _t3531.data.EIdent.name;
__auto_type ty = _t3531.data.EIdent.ty;
__auto_type is_move = _t3531.data.EIdent.is_move;
        /* pass */
        GVal* v = GpuEmitter_emit_expr(self, val);
        /* pass */
        TrStr slot = GpuEmitter_var_slot_ty(self, name);
        ...
        TrStr cv = GpuEmitter_coerce(self, v, slot);
        /* pass */
        ({ TrStr _at_t3532 = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("  store ")), _tr_strz(slot))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(cv)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(", ptr %var_"))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(name)); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("\n"))); _tr_str_release(_cl); _cres; })); GpuEmitter_w(self, _at_t3532); _tr_str_release(_at_t3532); });
        _tr_obj_release(v, _trdrop_GVal);
        _tr_str_release(slot);
        _tr_str_release(cv);
    } else if (1) {
...
__attribute__((hot)) void GpuEmitter_emit_stmt_expr(GpuEmitter* self, HirExpr e) {
    /* pass */
    __auto_type _t3533 = e;
    if (_t3533.tag == HirExpr_EMethodCall) {
        __auto_type obj = _t3533.data.EMethodCall.obj;
__auto_type method = _t3533.data.EMethodCall.method;
__auto_type args = _t3533.data.EMethodCall.args;
__auto_type ty = _t3533.data.EMethodCall.ty;
        /* pass */
    ...
    } else if (_t3533.tag == HirExpr_ECall) {
        __auto_type callee = _t3533.data.ECall.callee;
__auto_type args = _t3533.data.ECall.args;
__auto_type ty = _t3533.data.ECall.ty;
        /* pass */
...
__attribute__((hot)) void GpuEmitter_emit_store(GpuEmitter* self, HirExpr chain, HirExpr* valp) {
    /* pass */
    __auto_type _t3534 = chain;
    if (_t3534.tag == HirExpr_EMethodCall) {
        __auto_type pobj = _t3534.data.EMethodCall.obj;
__auto_type pmeth = _t3534.data.EMethodCall.method;
__auto_type pargs = _t3534.data.EMethodCall.args;
__auto_type pty = _t3534.data.EMethodCall.ty;
        /* pass */
...
__attribute__((hot)) GVal* GpuEmitter_emit_ptr(GpuEmitter* self, HirExpr e) {
    ...
    if (_t3537.tag == HirExpr_EIdent) {
        __auto_type name = _t3537.data.EIdent.name;
__auto_type ty = _t3537.data.EIdent.ty;
__auto_type is_move = _t3537.data.EIdent.is_move;
        /* pass */
        TrStr et = GpuEmitter_var_elem_ty(self, name);
        /* pass */
        if ((strcmp(_tr_strz(et), _tr_strz(_tr_str_lit(""))) == 0)) {
            /* pass */
            TrStr _strtmp_t3538 = _tr_str_lit("float");
            _tr_str_release(et);
            et = _strtmp_t3538;
        }
        /* pass */
...
__attribute__((hot)) GVal* GpuEmitter_emit_expr_hir(GpuEmitter* self, HirExpr e) {
    /* pass */
    __auto_type _t3540 = e;
    if (_t3540.tag == HirExpr_ELitInt) {
        __auto_type val = _t3540.data.ELitInt.val;
__auto_type ty = _t3540.data.ELitInt.ty;
        /* pass */
        TrStr t = _gpu_scalar_ty(ty->name);
        /* pass */
        if (_gpu_is_float(t)) {
            /* pass */
            TrStr _strtmp_t3541 = _tr_str_lit("i64");
            _tr_str_release(t);
            t = _strtmp_t3541;
        }
        /* pass */
        return ({ TrStr _at_t3542 = (_tr_str_wrap(_tr_int_to_str((long long)(val)))); __auto_type _wr = (GVal_make(t, _at_t3542)); _tr_str_release(_at_t3542); _wr; });
    } else if (_t3540.tag == HirExpr_ELitFloat) {
        __auto_type val = _t3540.data.ELitFloat.val;
__auto_type ty = _t3540.data.ELitFloat.ty;
        /* pass */
        TrStr ft = _gpu_scalar_ty(ty->name);
        /* pass */
        if ((!_gpu_is_float(ft))) {
            /* pass */
            TrStr _strtmp_t3543 = _tr_str_lit("double");
            _tr_str_release(ft);
            ft = _strtmp_t3543;
        }
        /* pass */
        return ({ TrStr _at_t3544 = (GpuEmitter_float_lit(self, val)); __auto_type _wr = (GVal_make(ft, _at_t3544)); _tr_str_release(_at_t3544); _wr; });
    } else if (_t3540.tag == HirExpr_ELitBool) {
        __auto_type val = _t3540.data.ELitBool.val;
__auto_type ty = _t3540.data.ELitBool.ty;
        /* pass */
        if (val) {
            /* pass */
    ...
    } else if (_t3540.tag == HirExpr_EIdent) {
        __auto_type name = _t3540.data.EIdent.name;
__auto_type ty = _t3540.data.EIdent.ty;
__auto_type is_move = _t3540.data.EIdent.is_move;
        /* pass */
        TrStr slot = GpuEmitter_var_slot_ty(self, name);
        /* pass */
        TrStr r = GpuEmitter_fresh(self);
    ...
    } else if (_t3540.tag == HirExpr_EBinOp) {
        __auto_type op = _t3540.data.EBinOp.op;
__auto_type left = _t3540.data.EBinOp.left;
__auto_type right = _t3540.data.EBinOp.right;
__auto_type ty = _t3540.data.EBinOp.ty;
        /* pass */
        return GpuEmitter_emit_binop(self, op, left, right);
    } else if (_t3540.tag == HirExpr_EUnaryOp) {
        __auto_type op = _t3540.data.EUnaryOp.op;
__auto_type expr = _t3540.data.EUnaryOp.expr;
__auto_type ty = _t3540.data.EUnaryOp.ty;
        /* pass */
        ...
        /* pass */
        return v;
    } else if (_t3540.tag == HirExpr_ECast) {
        __auto_type inner = _t3540.data.ECast.expr;
__auto_type target_ty = _t3540.data.ECast.target_ty;
        /* pass */
        ...
        /* pass */
        return ({ TrStr _at_t3548 = (GpuEmitter_coerce(self, iv, tt)); __auto_type _wr = (GVal_make(tt, _at_t3548)); _tr_str_release(_at_t3548); _wr; });
    } else if (_t3540.tag == HirExpr_EMethodCall) {
        __auto_type obj = _t3540.data.EMethodCall.obj;
__auto_type method = _t3540.data.EMethodCall.method;
__auto_type args = _t3540.data.EMethodCall.args;
__auto_type ty = _t3540.data.EMethodCall.ty;
        /* pass */
        ...
        /* pass */
        ({ TrStr _at_t3549 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("unsupported method '")), _tr_strz(method))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' in kernel"))); _tr_str_release(_cl); _cres; })); GpuEmitter_fail(self, _at_t3549); _tr_str_release(_at_t3549); });
        /* pass */
        return GVal_make(_tr_str_lit("i64"), _tr_str_lit("0"));
    } else if (_t3540.tag == HirExpr_ECall) {
        __auto_type callee = _t3540.data.ECall.callee;
__auto_type args = _t3540.data.ECall.args;
__auto_type ty = _t3540.data.ECall.ty;
        /* pass */
...
__attribute__((hot)) GVal* GpuEmitter_emit_call(GpuEmitter* self, HirExpr callee, List_ptr* args) {
    ...
    while ((di < self->dev_fns->len)) {
        ...
        if ((strcmp(_tr_strz(df->name), _tr_strz(nm)) == 0)) {
            ...
            while ((ai < args->len)) {
                ...
                if ((ai > 0LL)) {
                    /* pass */
                    TrStr _strtmp_t3554 = _tr_strx_concat(_tr_strz(argstr), _tr_strz(_tr_str_lit(", ")));
                    _tr_str_release(argstr);
                    argstr = _strtmp_t3554;
                }
                /* pass */
                ...
                if ((ai < df->params->len)) {
                    ...
                    if (((strcmp(_tr_strz(ptt), _tr_strz(_tr_str_lit("ptr addrspace(1)"))) != 0) && (strcmp(_tr_strz(aty), _tr_strz(ptt)) != 0))) {
                        /* pass */
                        TrStr _strtmp_t3555 = GpuEmitter_coerce(self, av, ptt);
                        _tr_str_release(aval);
                        aval = _strtmp_t3555;
                        /* pass */
                        TrStr _strtmp_t3556 = _tr_str_retain(ptt);
                        _tr_str_release(aty);
                        aty = _strtmp_t3556;
                    }
                }
                /* pass */
                TrStr _strtmp_t3557 = ({ TrStr _cl = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(argstr), _tr_strz(aty))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit(" "))); _tr_str_release(_cl); _cres; })); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(aval)); _tr_str_release(_cl); _cres; });
                _tr_str_release(argstr);
                argstr = _strtmp_t3557;
                /* pass */
                ai = (ai + 1LL);
    ...
    /* pass */
    ({ TrStr _at_t3560 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("call to '")), _tr_strz(nm))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' not allowed in kernel (mark it @device, or use a GPU builtin)"))); _tr_str_release(_cl); _cres; })); GpuEmitter_fail(self, _at_t3560); _tr_str_release(_at_t3560); });
    /* pass */
    _tr_str_release(nm);
    return GVal_make(_tr_str_lit("i64"), _tr_str_lit("0"));
}
...
__attribute__((hot)) GVal* GpuEmitter_emit_gpu_builtin(GpuEmitter* self, TrStr nm, long long dim) {
    ...
    if ((strcmp(_tr_strz(self->target), _tr_strz(_tr_str_lit("nvptx"))) == 0)) {
        ...
        if ((dim == 1LL)) {
            /* pass */
            TrStr _strtmp_t3561 = _tr_str_lit("y");
        ...
        if ((dim == 2LL)) {
            /* pass */
            TrStr _strtmp_t3562 = _tr_str_lit("z");
            _tr_str_release(axis);
            axis = _strtmp_t3562;
        }
        /* pass */
    ...
    if ((strcmp(_tr_strz(nm), _tr_strz(_tr_str_lit("gpu_local_id"))) == 0)) {
        /* pass */
        TrStr _strtmp_t3574 = _tr_str_lit("@_Z12get_local_idj");
        _tr_str_release(mangled);
        mangled = _strtmp_t3574;
    }
    /* pass */
    if ((strcmp(_tr_strz(nm), _tr_strz(_tr_str_lit("gpu_group_id"))) == 0)) {
        /* pass */
        TrStr _strtmp_t3575 = _tr_str_lit("@_Z12get_group_idj");
        _tr_str_release(mangled);
        mangled = _strtmp_t3575;
    }
    /* pass */
    if ((strcmp(_tr_strz(nm), _tr_strz(_tr_str_lit("gpu_local_size"))) == 0)) {
        /* pass */
        TrStr _strtmp_t3576 = _tr_str_lit("@_Z14get_local_sizej");
        _tr_str_release(mangled);
        mangled = _strtmp_t3576;
    }
    /* pass */
    if ((strcmp(_tr_strz(nm), _tr_strz(_tr_str_lit("gpu_global_size"))) == 0)) {
        /* pass */
        TrStr _strtmp_t3577 = _tr_str_lit("@_Z15get_global_sizej");
        _tr_str_release(mangled);
        mangled = _strtmp_t3577;
    }
    /* pass */
    if ((strcmp(_tr_strz(nm), _tr_strz(_tr_str_lit("gpu_num_groups"))) == 0)) {
        /* pass */
        TrStr _strtmp_t3578 = _tr_str_lit("@_Z14get_num_groupsj");
        _tr_str_release(mangled);
        mangled = _strtmp_t3578;
    }
    /* pass */
...
__attribute__((hot)) GVal* GpuEmitter_emit_binop(GpuEmitter* self, TrStr op, HirExpr* lp, HirExpr* rp) {
    ...
    if ((strcmp(_tr_strz(pred), _tr_strz(_tr_str_lit(""))) == 0)) {
        /* pass */
        ({ TrStr _at_t3591 = (({ TrStr _cl = (_tr_strx_concat(_tr_strz(_tr_str_lit("unsupported operator '")), _tr_strz(op))); TrStr _cres = _tr_strx_concat(_cl.data, _tr_strz(_tr_str_lit("' in kernel"))); _tr_str_release(_cl); _cres; })); GpuEmitter_fail(self, _at_t3591); _tr_str_release(_at_t3591); });
        /* pass */
        _tr_obj_release(l, _trdrop_GVal);
        _tr_obj_release(r, _trdrop_GVal);
        _tr_str_release(ct);
...
__attribute__((hot)) long long GpuEmitter_literal_int(GpuEmitter* self, HirExpr e) {
    /* pass */
    __auto_type _t3602 = e;
    if (_t3602.tag == HirExpr_ELitInt) {
        __auto_type val = _t3602.data.ELitInt.val;
__auto_type ty = _t3602.data.ELitInt.ty;
        return val;
    } else if (1) {
