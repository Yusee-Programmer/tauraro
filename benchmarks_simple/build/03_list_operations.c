...
TauValue create_list(TauValue size);
TauValue sum_list(TauValue lst);
int main(int argc, char* argv[]);
TauValue reverse_list(TauValue lst);

// Function definitions
TauValue create_list(TauValue size) {
    TauValue temp_result, lst, method_arg_0, temp_object, arg_0;
    // Create list with 0 elements
    { TauList* _list = malloc(sizeof(TauList));
    _list->size = 0;
    _list->capacity = 0;
    _list->items = malloc(sizeof(TauValue) * 0);
    temp_result.type = 4; temp_result.value.list = _list; }
    lst = temp_result;
    arg_0 = size;
    ...
    if (_for_list_1.type == 4) {
        ...
        for(int _for_i_1 = 0; _for_i_1 < _list->size; _for_i_1++) {
            ...
            temp_result = lst;
            temp_object = temp_result;
            temp_result = i;
            method_arg_0 = temp_result;
            temp_result = lst__append(temp_object, method_arg_0);
    }
    }
            temp_result = lst;
...
TauValue sum_list(TauValue lst) {
    TauValue temp_result, total, binop_left, binop_right, item;
    ...
    if (_for_list_1.type == 4) {
        TauList* _list = _for_list_1.value.list;
        for(int _for_i_1 = 0; _for_i_1 < _list->size; _for_i_1++) {
            TauValue item = _list->items[_for_i_1];
            binop_left = total;
            binop_right = item;
...
int main(int argc, char* argv[]) {
    TauValue size, lst, total, fstring_left_1, temp_result, fstring_result_1, fstring_right_1, arg_0, reversed_lst;
    temp_result = (TauValue){.type = 0, .value.i = 100000};
    size = temp_result;
    arg_0 = size;
    temp_result = create_list(arg_0);
    lst = temp_result;
    temp_result = (TauValue){.type = 2, .value.s = strdup("Created list of size: "), .refcount = 1};
    fstring_left_1 = temp_result;
    arg_0 = lst;
    if (arg_0.type == 4) { temp_result = tauraro_list_len((TauList*)arg_0.value.list); } else { temp_result = (TauValue){.type = 0, .value.i = 0, .refcount = 1}; }
    ...
    arg_0 = lst;
    temp_result = sum_list(arg_0);
    total = temp_result;
    temp_result = (TauValue){.type = 2, .value.s = strdup("Sum of list: "), .refcount = 1};
    fstring_left_1 = temp_result;
    temp_result = total;
    fstring_right_1 = temp_result;
    // Runtime type checking for + operation
    if ((fstring_left_1.type == 2 || fstring_right_1.type == 2)) {
        char temp_concat_rt_2[512] = {0};
        if (fstring_left_1.type == 2) {
            strcpy(temp_concat_rt_2, fstring_left_1.value.s);
        } else if (fstring_left_1.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_left_1.value.i);
            strcpy(temp_concat_rt_2, int_buf);
        } else if (fstring_left_1.type == 3) {
            strcpy(temp_concat_rt_2, fstring_left_1.value.i ? "True" : "False");
        } else if (fstring_left_1.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_left_1.value.f);
            strcpy(temp_concat_rt_2, float_buf);
        ...
        if (fstring_right_1.type == 2) {
            strcat(temp_concat_rt_2, fstring_right_1.value.s);
        } else if (fstring_right_1.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_right_1.value.i);
            strcat(temp_concat_rt_2, int_buf);
        } else if (fstring_right_1.type == 3) {
            strcat(temp_concat_rt_2, fstring_right_1.value.i ? "True" : "False");
        } else if (fstring_right_1.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_right_1.value.f);
            strcat(temp_concat_rt_2, float_buf);
        }
    fstring_result_1 = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_2), .refcount = 1};
    } else {
    fstring_result_1 = (TauValue){.type = 0, .value.i = fstring_left_1.value.i + fstring_right_1.value.i};
    }
    temp_result = fstring_result_1;
    ...
    temp_result = (TauValue){.type = 0, .value.i = 0};
    arg_0 = temp_result;
    temp_result = reverse_list(arg_0);
    reversed_lst = temp_result;
    temp_result = (TauValue){.type = 2, .value.s = strdup("First 5 of reversed: "), .refcount = 1};
    fstring_left_1 = temp_result;
    temp_result = (TauValue){.type = 0, .value.i = 0};
    fstring_right_1 = temp_result;
    // Runtime type checking for + operation
    if ((fstring_left_1.type == 2 || fstring_right_1.type == 2)) {
        ...
        if (fstring_left_1.type == 2) {
            strcpy(temp_concat_rt_3, fstring_left_1.value.s);
        } else if (fstring_left_1.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_left_1.value.i);
            strcpy(temp_concat_rt_3, int_buf);
        } else if (fstring_left_1.type == 3) {
            strcpy(temp_concat_rt_3, fstring_left_1.value.i ? "True" : "False");
        } else if (fstring_left_1.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_left_1.value.f);
        ...
        if (fstring_right_1.type == 2) {
            strcat(temp_concat_rt_3, fstring_right_1.value.s);
        } else if (fstring_right_1.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_right_1.value.i);
            strcat(temp_concat_rt_3, int_buf);
        } else if (fstring_right_1.type == 3) {
            strcat(temp_concat_rt_3, fstring_right_1.value.i ? "True" : "False");
        } else if (fstring_right_1.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_right_1.value.f);
            strcat(temp_concat_rt_3, float_buf);
        }
    fstring_result_1 = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_3), .refcount = 1};
    } else {
    fstring_result_1 = (TauValue){.type = 0, .value.i = fstring_left_1.value.i + fstring_right_1.value.i};
    }
    temp_result = fstring_result_1;
...
TauValue reverse_list(TauValue lst) {
    TauValue arg_0_left, arg_1, temp_result, method_arg_0, arg_2, temp_object, arg_0_right, reversed_lst, arg_0;
    // Create list with 0 elements
    { TauList* _list = malloc(sizeof(TauList));
    _list->size = 0;
    _list->capacity = 0;
    ...
    temp_result = (TauValue){.type = 0, .value.i = 0};
    arg_2 = temp_result;
    temp_result = range3(arg_0, arg_1, arg_2);
    // for loop over temp_result
    TauValue _for_list_1 = temp_result;
    if (_for_list_1.type == 4) {
        TauList* _list = _for_list_1.value.list;
