...
// Comprehensive exception handling system with nested try block support
#include <setjmp.h>

// Exception context for nested try-except-finally blocks
#define MAX_EXCEPTION_DEPTH 64
typedef struct {
    jmp_buf buf;
    TauException* exception;
    int has_finally;
    int finally_executed;
} TauExceptionContext;

static TauExceptionContext tauraro_exception_stack[MAX_EXCEPTION_DEPTH];
static int tauraro_exception_depth = -1;
static TauException* tauraro_current_exception = NULL;
...
TauException* tauraro_create_exception(const char* type, const char* message) {
    ...
    if (!exc) return NULL;
    exc->type = strdup(type ? type : "Exception");
    exc->message = strdup(message ? message : "");
    exc->traceback = NULL;
...
int tauraro_push_exception_context(int has_finally) {
    if (tauraro_exception_depth >= MAX_EXCEPTION_DEPTH - 1) {
        fprintf(stderr, "Exception stack overflow\n");
        exit(1);
    }
    tauraro_exception_depth++;
    tauraro_exception_stack[tauraro_exception_depth].exception = NULL;
    tauraro_exception_stack[tauraro_exception_depth].has_finally = has_finally;
    tauraro_exception_stack[tauraro_exception_depth].finally_executed = 0;
    return tauraro_exception_depth;
}

void tauraro_pop_exception_context() {
    if (tauraro_exception_depth >= 0) {
        TauException* exc = tauraro_exception_stack[tauraro_exception_depth].exception;
...
jmp_buf* tauraro_get_exception_buf() {
    if (tauraro_exception_depth >= 0) {
        return &tauraro_exception_stack[tauraro_exception_depth].buf;
    }
    return NULL;
}
...
void tauraro_throw_exception(TauException* exc) {
    if (!exc) {
        exc = tauraro_create_exception("RuntimeError", "NULL exception thrown");
    }
    tauraro_current_exception = exc;
    if (tauraro_exception_depth >= 0) {
        tauraro_exception_stack[tauraro_exception_depth].exception = exc;
        exc->refcount++;
        longjmp(tauraro_exception_stack[tauraro_exception_depth].buf, 1);
    } else {
        // No exception handler, print and exit
        fprintf(stderr, "Unhandled exception: %s: %s\n", exc->type, exc->message);
        if (exc->traceback) fprintf(stderr, "%s\n", exc->traceback);
        exit(1);
    }
}

int tauraro_exception_matches(const char* type) {
    if (!tauraro_current_exception || !type) return 0;
    if (strcmp(type, "*") == 0) return 1; // Catch-all
    if (strcmp(tauraro_current_exception->type, type) == 0) return 1;
    
    // Exception inheritance hierarchy
    const char* exc_type = tauraro_current_exception->type;
    if (strcmp(type, "Exception") == 0) return 1; // All exceptions derive from Exception
    if (strcmp(type, "BaseException") == 0) return 1; // All derive from BaseException
    
    // Standard exception hierarchy checks
    if (strcmp(type, "ArithmeticError") == 0) {
        return strcmp(exc_type, "ZeroDivisionError") == 0 ||
               strcmp(exc_type, "OverflowError") == 0 ||
               strcmp(exc_type, "FloatingPointError") == 0;
    }
    if (strcmp(type, "LookupError") == 0) {
        return strcmp(exc_type, "IndexError") == 0 ||
               strcmp(exc_type, "KeyError") == 0;
    }
    return 0;
}

void tauraro_mark_finally_executed() {
    if (tauraro_exception_depth >= 0) {
        tauraro_exception_stack[tauraro_exception_depth].finally_executed = 1;
    }
}

int tauraro_should_execute_finally() {
    if (tauraro_exception_depth >= 0) {
        return tauraro_exception_stack[tauraro_exception_depth].has_finally &&
               !tauraro_exception_stack[tauraro_exception_depth].finally_executed;
    }
    return 0;
}

void tauraro_reraise() {
    if (tauraro_current_exception) {
        tauraro_throw_exception(tauraro_current_exception);
    } else {
        tauraro_throw_exception(tauraro_create_exception("RuntimeError", "No active exception to re-raise"));
    }
}

TauValue tauraro_get_current_exception() {
    if (tauraro_current_exception) {
        TauValue exc_val;
        exc_val.type = 8; // Exception type
        exc_val.value.exc = tauraro_current_exception;
        exc_val.refcount = 1;
        exc_val.next = NULL;
        return exc_val;
    }
...
void tauraro_clear_exception() {
    tauraro_current_exception = NULL;
}

// Type conversion and compatibility functions

int tauraro_is_type_compatible(TauValue val, const char* expected_type) {
    if (!expected_type) return 1; // No type constraint
    
    if (strcmp(expected_type, "int") == 0) return val.type == 0;
    if (strcmp(expected_type, "float") == 0) return val.type == 1;
    if (strcmp(expected_type, "str") == 0) return val.type == 2;
    if (strcmp(expected_type, "bool") == 0) return val.type == 3;
    if (strcmp(expected_type, "list") == 0) return val.type == 4;
    if (strcmp(expected_type, "dict") == 0) return val.type == 5;
    if (strcmp(expected_type, "object") == 0) return val.type == 6;
    if (strcmp(expected_type, "function") == 0) return val.type == 7;
    if (strcmp(expected_type, "Exception") == 0) return val.type == 8;
    
    return 1; // Unknown type, allow it
}

TauValue tauraro_to_int_unified(TauValue val) {
    switch (val.type) {
        case 0: return val; // Already int
        case 1: return tauraro_int((long long)val.value.f);
        case 2: return tauraro_int(val.value.s ? atoll(val.value.s) : 0);
        case 3: return tauraro_int(val.value.i ? 1 : 0); // Bool stored as int
        default: return tauraro_int(0);
    }
}

TauValue tauraro_to_float_unified(TauValue val) {
    switch (val.type) {
        case 0: return tauraro_float((double)val.value.i);
        case 1: return val; // Already float
        case 2: return tauraro_float(val.value.s ? atof(val.value.s) : 0.0);
        case 3: return tauraro_float(val.value.i ? 1.0 : 0.0); // Bool stored as int
        default: return tauraro_float(0.0);
    }
}

TauValue tauraro_to_str_unified(TauValue val) {
    char buf[256];
    switch (val.type) {
        case 0:
            snprintf(buf, sizeof(buf), "%lld", val.value.i);
            return tauraro_str(buf);
        case 1:
            snprintf(buf, sizeof(buf), "%g", val.value.f);
            return tauraro_str(buf);
        case 2: return val; // Already string
        case 3:
            return tauraro_str(val.value.i ? "True" : "False"); // Bool stored as int
        default:
            return tauraro_str("<object>");
    }
}

TauValue tauraro_to_bool_unified(TauValue val) {
    switch (val.type) {
        ...
        case 3: return val; // Already bool
        case 9: return tauraro_bool(0); // None is False
        default: return tauraro_bool(1); // Objects are True
    }
}
...
TauValue multiply_matrix_elements(TauValue size);
TauValue count_pairs(TauValue n);
int main(int argc, char* argv[]);

// Function definitions
TauValue multiply_matrix_elements(TauValue size) {
    TauValue binop_left, i, arg_0, total, binop_right, temp_result, j;
    temp_result = (TauValue){.type = 0, .value.i = 0};
    total = temp_result;
    arg_0 = size;
    temp_result = range(arg_0);
    // for loop over temp_result
    TauValue _for_list_1 = temp_result;
    if (_for_list_1.type == 4) {
        ...
        for(int _for_i_1 = 0; _for_i_1 < _list->size; _for_i_1++) {
            ...
            temp_result = range(arg_0);
            // for loop over temp_result
            TauValue _for_list_2 = temp_result;
            if (_for_list_2.type == 4) {
                TauList* _list = _for_list_2.value.list;
                for(int _for_i_2 = 0; _for_i_2 < _list->size; _for_i_2++) {
                    TauValue j = _list->items[_for_i_2];
                    binop_left = total;
                    binop_left = i;
                    binop_right = j;
                    // Runtime type checking for * operation (string repeat)
                    if ((binop_left.type == 2 && binop_right.type == 0) || (binop_left.type == 0 && binop_right.type == 2)) {
                        const char* _str = (binop_left.type == 2) ? binop_left.value.s : binop_right.value.s;
                        long long _count = (binop_left.type == 0) ? binop_left.value.i : binop_right.value.i;
                        if (_count <= 0 || !_str) {
                            temp_result = (TauValue){.type = 2, .value.s = strdup(""), .refcount = 1};
                        } else {
                            size_t _len = strlen(_str);
                            char* temp_repeat_rt_3 = malloc(_len * _count + 1);
                            temp_repeat_rt_3[0] = '\0';
                            for (long long _i = 0; _i < _count; _i++) strcat(temp_repeat_rt_3, _str);
                            temp_result = (TauValue){.type = 2, .value.s = temp_repeat_rt_3, .refcount = 1};
                        }
                    } else {
                    ...
                    if ((binop_left.type == 2 || binop_right.type == 2)) {
                        ...
                        if (binop_left.type == 2) {
                            ...
                            strcpy(temp_concat_rt_4, binop_left.value.i ? "True" : "False");
                        } else if (binop_left.type == 1) {
                            char float_buf[64];
                            snprintf(float_buf, sizeof(float_buf), "%f", binop_left.value.f);
                            strcpy(temp_concat_rt_4, float_buf);
                        }
                        if (binop_right.type == 2) {
                            ...
                            strcat(temp_concat_rt_4, binop_right.value.i ? "True" : "False");
                        } else if (binop_right.type == 1) {
                            char float_buf[64];
                            snprintf(float_buf, sizeof(float_buf), "%f", binop_right.value.f);
                            strcat(temp_concat_rt_4, float_buf);
                        }
                    ...
                    temp_result = total;
                    return 0;  // Fixed: main returns int
}

TauValue count_pairs(TauValue n) {
    TauValue arg_1, binop_left, count, i, comp_left, comp_right, arg_0, binop_right, temp_result, j;
    temp_result = (TauValue){.type = 0, .value.i = 0};
    count = temp_result;
    arg_0 = n;
    temp_result = range(arg_0);
    ...
    if (_for_list_1.type == 4) {
        ...
        for(int _for_i_1 = 0; _for_i_1 < _list->size; _for_i_1++) {
            ...
            arg_0 = i;
            arg_1 = n;
            temp_result = range2(arg_0, arg_1);
            // for loop over temp_result
            TauValue _for_list_2 = temp_result;
            if (_for_list_2.type == 4) {
                TauList* _list = _for_list_2.value.list;
                for(int _for_i_2 = 0; _for_i_2 < _list->size; _for_i_2++) {
                    ...
                    // Runtime type checking for + operation
                    if ((binop_left.type == 2 || binop_right.type == 2)) {
                        char temp_concat_rt_3[512] = {0};
                        if (binop_left.type == 2) {
                            strcpy(temp_concat_rt_3, binop_left.value.s);
                        } else if (binop_left.type == 0) {
                            char int_buf[64];
                            snprintf(int_buf, sizeof(int_buf), "%lld", binop_left.value.i);
                            strcpy(temp_concat_rt_3, int_buf);
                        } else if (binop_left.type == 3) {
                            strcpy(temp_concat_rt_3, binop_left.value.i ? "True" : "False");
                        } else if (binop_left.type == 1) {
                            char float_buf[64];
                            snprintf(float_buf, sizeof(float_buf), "%f", binop_left.value.f);
                            strcpy(temp_concat_rt_3, float_buf);
                        }
                        if (binop_right.type == 2) {
                            strcat(temp_concat_rt_3, binop_right.value.s);
                        } else if (binop_right.type == 0) {
                            char int_buf[64];
                            snprintf(int_buf, sizeof(int_buf), "%lld", binop_right.value.i);
                            strcat(temp_concat_rt_3, int_buf);
                        } else if (binop_right.type == 3) {
                            strcat(temp_concat_rt_3, binop_right.value.i ? "True" : "False");
                        } else if (binop_right.type == 1) {
                            char float_buf[64];
                            snprintf(float_buf, sizeof(float_buf), "%f", binop_right.value.f);
                            strcat(temp_concat_rt_3, float_buf);
                        }
                    temp_result = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_3), .refcount = 1};
                    } else {
                    temp_result = (TauValue){.type = 0, .value.i = binop_left.value.i + binop_right.value.i};
                    }
                    binop_left = temp_result;
                    binop_right = (TauValue){.type = 0, .value.i = 2};
                    temp_result = (TauValue){.type = 0, .value.i = binop_left.value.i % binop_right.value.i};
                    comp_left = temp_result;
                    comp_right = (TauValue){.type = 0, .value.i = 0};
                    temp_result = (TauValue){.type = 0, .value.i = comp_left.value.i == comp_right.value.i};
                    if ((temp_result.type == 3 ? temp_result.value.i : (temp_result.type == 0 ? (temp_result.value.i != 0) : 1))) {
                        binop_left = count;
...
int main(int argc, char* argv[]) {
    TauValue fstring_left_2, fstring_right_1, fstring_left_5, binop_right, temp_result, result1, fstring_right_3, fstring_result_1, fstring_right_4, fstring_result_5, fstring_left_1, fstring_right_5, fstring_result_4, binop_left, fstring_left_3, arg_0, fstring_result_3, fstring_left_4, result2, fstring_result_2, size, fstring_right_2;
    temp_result = (TauValue){.type = 0, .value.i = 500};
    size = temp_result;
    arg_0 = size;
    temp_result = multiply_matrix_elements(arg_0);
    result1 = temp_result;
    temp_result = (TauValue){.type = 2, .value.s = strdup("Matrix sum ("), .refcount = 1};
    fstring_left_1 = temp_result;
    temp_result = size;
    fstring_right_1 = temp_result;
    // Runtime type checking for + operation
    if ((fstring_left_1.type == 2 || fstring_right_1.type == 2)) {
        char temp_concat_rt_1[512] = {0};
        if (fstring_left_1.type == 2) {
            strcpy(temp_concat_rt_1, fstring_left_1.value.s);
        } else if (fstring_left_1.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_left_1.value.i);
            strcpy(temp_concat_rt_1, int_buf);
        } else if (fstring_left_1.type == 3) {
            strcpy(temp_concat_rt_1, fstring_left_1.value.i ? "True" : "False");
        } else if (fstring_left_1.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_left_1.value.f);
            strcpy(temp_concat_rt_1, float_buf);
        }
        if (fstring_right_1.type == 2) {
            strcat(temp_concat_rt_1, fstring_right_1.value.s);
        } else if (fstring_right_1.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_right_1.value.i);
            strcat(temp_concat_rt_1, int_buf);
        } else if (fstring_right_1.type == 3) {
            strcat(temp_concat_rt_1, fstring_right_1.value.i ? "True" : "False");
        } else if (fstring_right_1.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_right_1.value.f);
            strcat(temp_concat_rt_1, float_buf);
        }
    fstring_result_1 = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_1), .refcount = 1};
    } else {
    fstring_result_1 = (TauValue){.type = 0, .value.i = fstring_left_1.value.i + fstring_right_1.value.i};
    }
    temp_result = fstring_result_1;
    fstring_left_2 = temp_result;
    fstring_right_2 = (TauValue){.type = 2, .value.s = strdup("x"), .refcount = 1};
    // Runtime type checking for + operation
    if ((fstring_left_2.type == 2 || fstring_right_2.type == 2)) {
        char temp_concat_rt_2[512] = {0};
        if (fstring_left_2.type == 2) {
            strcpy(temp_concat_rt_2, fstring_left_2.value.s);
        } else if (fstring_left_2.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_left_2.value.i);
            strcpy(temp_concat_rt_2, int_buf);
        } else if (fstring_left_2.type == 3) {
            strcpy(temp_concat_rt_2, fstring_left_2.value.i ? "True" : "False");
        } else if (fstring_left_2.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_left_2.value.f);
            strcpy(temp_concat_rt_2, float_buf);
        }
        if (fstring_right_2.type == 2) {
            strcat(temp_concat_rt_2, fstring_right_2.value.s);
        } else if (fstring_right_2.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_right_2.value.i);
            strcat(temp_concat_rt_2, int_buf);
        } else if (fstring_right_2.type == 3) {
            strcat(temp_concat_rt_2, fstring_right_2.value.i ? "True" : "False");
        } else if (fstring_right_2.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_right_2.value.f);
            strcat(temp_concat_rt_2, float_buf);
        }
    fstring_result_2 = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_2), .refcount = 1};
    } else {
    fstring_result_2 = (TauValue){.type = 0, .value.i = fstring_left_2.value.i + fstring_right_2.value.i};
    }
    temp_result = fstring_result_2;
    fstring_left_3 = temp_result;
    temp_result = size;
    fstring_right_3 = temp_result;
    // Runtime type checking for + operation
    if ((fstring_left_3.type == 2 || fstring_right_3.type == 2)) {
        char temp_concat_rt_3[512] = {0};
        if (fstring_left_3.type == 2) {
            strcpy(temp_concat_rt_3, fstring_left_3.value.s);
        } else if (fstring_left_3.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_left_3.value.i);
            strcpy(temp_concat_rt_3, int_buf);
        } else if (fstring_left_3.type == 3) {
            strcpy(temp_concat_rt_3, fstring_left_3.value.i ? "True" : "False");
        } else if (fstring_left_3.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_left_3.value.f);
            strcpy(temp_concat_rt_3, float_buf);
        }
        if (fstring_right_3.type == 2) {
            strcat(temp_concat_rt_3, fstring_right_3.value.s);
        } else if (fstring_right_3.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_right_3.value.i);
            strcat(temp_concat_rt_3, int_buf);
        } else if (fstring_right_3.type == 3) {
            strcat(temp_concat_rt_3, fstring_right_3.value.i ? "True" : "False");
        } else if (fstring_right_3.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_right_3.value.f);
            strcat(temp_concat_rt_3, float_buf);
        }
    fstring_result_3 = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_3), .refcount = 1};
    } else {
    fstring_result_3 = (TauValue){.type = 0, .value.i = fstring_left_3.value.i + fstring_right_3.value.i};
    }
    temp_result = fstring_result_3;
    fstring_left_4 = temp_result;
    fstring_right_4 = (TauValue){.type = 2, .value.s = strdup("): "), .refcount = 1};
    // Runtime type checking for + operation
    if ((fstring_left_4.type == 2 || fstring_right_4.type == 2)) {
        char temp_concat_rt_4[512] = {0};
        if (fstring_left_4.type == 2) {
            strcpy(temp_concat_rt_4, fstring_left_4.value.s);
        } else if (fstring_left_4.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_left_4.value.i);
            strcpy(temp_concat_rt_4, int_buf);
        } else if (fstring_left_4.type == 3) {
            strcpy(temp_concat_rt_4, fstring_left_4.value.i ? "True" : "False");
        } else if (fstring_left_4.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_left_4.value.f);
            strcpy(temp_concat_rt_4, float_buf);
        }
        if (fstring_right_4.type == 2) {
            strcat(temp_concat_rt_4, fstring_right_4.value.s);
        } else if (fstring_right_4.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_right_4.value.i);
            strcat(temp_concat_rt_4, int_buf);
        } else if (fstring_right_4.type == 3) {
            strcat(temp_concat_rt_4, fstring_right_4.value.i ? "True" : "False");
        } else if (fstring_right_4.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_right_4.value.f);
            strcat(temp_concat_rt_4, float_buf);
        }
    fstring_result_4 = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_4), .refcount = 1};
    } else {
    fstring_result_4 = (TauValue){.type = 0, .value.i = fstring_left_4.value.i + fstring_right_4.value.i};
    }
    temp_result = fstring_result_4;
    fstring_left_5 = temp_result;
    temp_result = result1;
    fstring_right_5 = temp_result;
    // Runtime type checking for + operation
    if ((fstring_left_5.type == 2 || fstring_right_5.type == 2)) {
        char temp_concat_rt_5[512] = {0};
        if (fstring_left_5.type == 2) {
            strcpy(temp_concat_rt_5, fstring_left_5.value.s);
        } else if (fstring_left_5.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_left_5.value.i);
            strcpy(temp_concat_rt_5, int_buf);
        } else if (fstring_left_5.type == 3) {
            strcpy(temp_concat_rt_5, fstring_left_5.value.i ? "True" : "False");
        } else if (fstring_left_5.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_left_5.value.f);
            strcpy(temp_concat_rt_5, float_buf);
        }
        if (fstring_right_5.type == 2) {
            strcat(temp_concat_rt_5, fstring_right_5.value.s);
        } else if (fstring_right_5.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_right_5.value.i);
            strcat(temp_concat_rt_5, int_buf);
        } else if (fstring_right_5.type == 3) {
            strcat(temp_concat_rt_5, fstring_right_5.value.i ? "True" : "False");
        } else if (fstring_right_5.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_right_5.value.f);
            strcat(temp_concat_rt_5, float_buf);
        }
    fstring_result_5 = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_5), .refcount = 1};
    } else {
    fstring_result_5 = (TauValue){.type = 0, .value.i = fstring_left_5.value.i + fstring_right_5.value.i};
    }
    temp_result = fstring_result_5;
    arg_0 = temp_result;
    printf("%s\n", tauraro_str_from_value(&arg_0).value.s);
    arg_0 = (TauValue){.type = 0, .value.i = 1000};
    temp_result = count_pairs(arg_0);
    result2 = temp_result;
    temp_result = (TauValue){.type = 2, .value.s = strdup("Even sum pairs: "), .refcount = 1};
    fstring_left_1 = temp_result;
    temp_result = result2;
    fstring_right_1 = temp_result;
    // Runtime type checking for + operation
    if ((fstring_left_1.type == 2 || fstring_right_1.type == 2)) {
        char temp_concat_rt_6[512] = {0};
        if (fstring_left_1.type == 2) {
            strcpy(temp_concat_rt_6, fstring_left_1.value.s);
        } else if (fstring_left_1.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_left_1.value.i);
            strcpy(temp_concat_rt_6, int_buf);
        } else if (fstring_left_1.type == 3) {
            strcpy(temp_concat_rt_6, fstring_left_1.value.i ? "True" : "False");
        } else if (fstring_left_1.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_left_1.value.f);
            strcpy(temp_concat_rt_6, float_buf);
        }
        if (fstring_right_1.type == 2) {
            strcat(temp_concat_rt_6, fstring_right_1.value.s);
        } else if (fstring_right_1.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", fstring_right_1.value.i);
            strcat(temp_concat_rt_6, int_buf);
        } else if (fstring_right_1.type == 3) {
            strcat(temp_concat_rt_6, fstring_right_1.value.i ? "True" : "False");
        } else if (fstring_right_1.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", fstring_right_1.value.f);
            strcat(temp_concat_rt_6, float_buf);
        }
    fstring_result_1 = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_6), .refcount = 1};
    } else {
    fstring_result_1 = (TauValue){.type = 0, .value.i = fstring_left_1.value.i + fstring_right_1.value.i};
    }
    temp_result = fstring_result_1;
    ...
    // Runtime type checking for + operation
    if ((binop_left.type == 2 || binop_right.type == 2)) {
        char temp_concat_rt_7[512] = {0};
        if (binop_left.type == 2) {
            strcpy(temp_concat_rt_7, binop_left.value.s);
        } else if (binop_left.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", binop_left.value.i);
            strcpy(temp_concat_rt_7, int_buf);
        } else if (binop_left.type == 3) {
            strcpy(temp_concat_rt_7, binop_left.value.i ? "True" : "False");
        } else if (binop_left.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", binop_left.value.f);
            strcpy(temp_concat_rt_7, float_buf);
        }
        if (binop_right.type == 2) {
            strcat(temp_concat_rt_7, binop_right.value.s);
        } else if (binop_right.type == 0) {
            char int_buf[64];
            snprintf(int_buf, sizeof(int_buf), "%lld", binop_right.value.i);
            strcat(temp_concat_rt_7, int_buf);
        } else if (binop_right.type == 3) {
            strcat(temp_concat_rt_7, binop_right.value.i ? "True" : "False");
        } else if (binop_right.type == 1) {
            char float_buf[64];
            snprintf(float_buf, sizeof(float_buf), "%f", binop_right.value.f);
            strcat(temp_concat_rt_7, float_buf);
        }
    temp_result = (TauValue){.type = 2, .value.s = strdup(temp_concat_rt_7), .refcount = 1};
    } else {
    temp_result = (TauValue){.type = 0, .value.i = binop_left.value.i + binop_right.value.i};
    }
    return 0;  // Fixed: main returns int
