...
pub struct PureNativeTranspiler {
    ...
    /// Track which types are used to determine needed headers
    used_types: HashSet<NativeCType>,
}
...
impl PureNativeTranspiler {
    ...
    pub fn transpile_program(&mut self, program: &Program) -> Result<String, String> {
        ...
        // Second pass: transpile all functions to track which built-ins are used
        let mut function_bodies = String::new();
        for stmt in &program.statements {
            if let Statement::FunctionDef { .. } = stmt {
                function_bodies.push_str(&self.transpile_function(stmt)?);
        ...
        // Now generate the complete output with proper ordering
        let mut output = String::new();

        // Generate headers (now with knowledge of used types)
        ...
        // Generate built-in function implementations (now we know which are used)
        output.push_str(&self.generate_builtin_implementations());

        // Add user-defined functions
        output.push_str(&function_bodies);

        // Add main function
        output.push_str(&main_func);
    ...
    fn generate_headers(&self) -> String {
        ...
        // Always needed for basic types
        headers.push_str("#include <stdio.h>\n");
        headers.push_str("#include <stdlib.h>\n");

        // Conditionally include headers based on usage
        ...
        let needs_bool = self.used_types.contains(&NativeCType::Bool) ||
                         self.used_builtins.contains("print");

        let needs_math = self.used_types.contains(&NativeCType::Double) ||
                         self.used_types.contains(&NativeCType::Long);

        let needs_stddef = !self.used_collection_types.is_empty();
        ...
        // Add utility macros only if collections are used
    ...
    fn map_type_annotation(&mut self, type_ann: Option<&Type>) -> NativeCType {
        ...
        // Track all types for header optimization
        self.used_types.insert(mapped_type.clone());
        mapped_type
    }
    ...
    fn generate_builtin_implementations(&self) -> String {
        ...
        // Only generate print functions if actually used
...
"#);
        ...
        // Only generate str conversion functions if actually used
        if self.used_builtins.contains("str") {
            output.push_str(r#"// String conversion functions
...
"#);
        ...
        // Only generate int conversion functions if actually used
        if self.used_builtins.contains("int") {
            output.push_str(r#"// Integer conversion functions
...
"#);
        ...
        // Only generate float conversion functions if actually used
        if self.used_builtins.contains("float") {
            output.push_str(r#"// Float conversion functions
...
"#);
        ...
        // Only generate len function if actually used
        if self.used_builtins.contains("len") {
            output.push_str(r#"// Length function
