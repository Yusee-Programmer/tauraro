...
static inline TauValue tauraro_multiprocessing_Process(TauValue target) {
    ...
    proc->target = (void (*)(void))target.value.p;
    proc->pid = 0;
    proc->started = 0;
    
    return (TauValue){.type = 6, .value.p = (void*)proc, .refcount = 1, .next = NULL};
}
...
static inline TauValue tauraro_multiprocessing_Process_start(TauValue proc_obj) {
    ...
    ProcessWrapper* proc = (ProcessWrapper*)proc_obj.value.p;
    proc->started = 1;
...
static inline TauValue tauraro_multiprocessing_Queue(void) {
    ...
    return (TauValue){.type = 6, .value.p = (void*)q, .refcount = 1, .next = NULL};
}
...
static inline TauValue tauraro_multiprocessing_Queue_put(TauValue queue, TauValue item) {
    ...
    QueueWrapper* q = (QueueWrapper*)queue.value.p;
...
static inline TauValue tauraro_multiprocessing_Pipe(void) {
    return (TauValue){.type = 4, .value.p = NULL, .refcount = 1, .next = NULL};  // Tuple of (conn1, conn2)
}
