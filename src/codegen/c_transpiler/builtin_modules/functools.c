...
static inline TauValue tauraro_functools_partial(TauValue func, TauValue args) {
    ...
    return (TauValue){.type = 6, .value.p = (void*)partial, .refcount = 1, .next = NULL};
}
...
static inline TauValue tauraro_functools_lru_cache(TauValue maxsize) {
    ...
    return (TauValue){.type = 6, .value.p = (void*)cache, .refcount = 1, .next = NULL};
}
...
static inline TauValue tauraro_functools_cmp_to_key(TauValue cmp_func) {
    ...
    key->cmp_func = cmp_func.value.p;
    
    return (TauValue){.type = 6, .value.p = (void*)key, .refcount = 1, .next = NULL};
}
