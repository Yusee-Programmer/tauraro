...
static inline TauValue tauraro_asyncio_gather(TauValue coros) {
    ...
    return (TauValue){.type = 4, .value.p = NULL, .refcount = 1, .next = NULL};  // List of results
}
...
static inline TauValue tauraro_asyncio_create_task(TauValue coro) {
    ...
    task->func = (void (*)(void))coro.value.p;
    task->done = 0;
    task->args = NULL;
    
    return (TauValue){.type = 6, .value.p = (void*)task, .refcount = 1, .next = NULL};
}
...
static inline TauValue tauraro_asyncio_get_event_loop(void) {
    ...
    return (TauValue){.type = 6, .value.p = (void*)loop, .refcount = 1, .next = NULL};
}
