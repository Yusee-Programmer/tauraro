...
/// GCC-based native code generator
pub struct GCCCodeGenerator {
    temp_dir: String,
}

impl GCCCodeGenerator {
    pub fn new() -> Self {
        Self {
            temp_dir: std::env::temp_dir().join("tauraro_gcc").to_string_lossy().to_string(),
        }
    }
    ...
    fn compile_with_gcc(&self, c_file: &Path, output_file: &Path, options: &CodegenOptions) -> Result<()> {
        ...
        if !output.status.success() {
            ...
            return Err(anyhow!("GCC compilation failed: {}", stderr));
        }
...
impl CodeGenerator for GCCCodeGenerator {
    fn generate(&self, module: IRModule, options: &CodegenOptions) -> Result<Vec<u8>> {
        ...
        self.compile_with_gcc(&c_file, &output_file, options)?;
    ...
    fn get_supported_features(&self) -> Vec<&'static str> {
        vec!["optimization", "debug_info", "cross_compilation"]
    }
}

impl Default for GCCCodeGenerator {
    fn default() -> Self {
        Self::new()
    }
}