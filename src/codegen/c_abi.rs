    ...
    fn generate_function_definition(&self, function: &IRFunction, module: &IRModule, options: &CodegenOptions) -> Result<String> {
        ...
        for local in &locals {
            let c_type = if let Some(ir_type) = local_types.get(local) {
                ...
                // For all local variables, use TauraroValue to ensure consistent access patterns
        ...
        match instruction {
            ...
            IRInstruction::Call { dest: Some(dest), func, .. } => {
                 ...
                 let return_type = match func.as_str() {
                     ...
                     f if f.starts_with("tauraro_math_") => IRType::Float64, // Math functions return double
        ...
        match instruction {
            ...
            IRInstruction::Call { dest, func, args } => {
                ...
                if func.starts_with("super_") {
                    ...
                    let is_void_function = match func.as_str() {
                        "print" | "printf" | "free" | "tauraro_sys_exit" => true,
            ...
            IRInstruction::Print { value } => {
                ...
                match value {
                    ...
                    IRValue::Variable(var_name) => {
                        // For TauraroValue variables, we need to check the type_tag and access the appropriate data member
                        Ok(format!("    switch ({}.type_tag) {{\n    case 0: printf(\"%lld\\n\", (long long){}.data.int_val); break;\n    case 1: printf(\"%f\\n\", {}.data.float_val); break;\n    case 2: printf(\"%s\\n\", {}.data.string_val); break;\n    case 3: printf(\"%s\\n\", {}.data.bool_val ? \"true\" : \"false\"); break;\n    default: printf(\"%s\\n\", \"<unknown>\"); break;\n    }}\n", var_name, var_name, var_name, var_name, var_name))
                    }
                    _ => {
                        Ok(format!("    printf(\"%s\\n\", {});\n", expr))
                    }
            ...
            IRInstruction::PrintMulti { values } => {
                // Handle printing multiple values
                let mut code = String::new();
                for value in values {
                    let expr = self.ir_value_to_c_expr(value)?;
                    // Handle different types for printf
                    match value {
                        ...
                        IRValue::ConstantInt(_) | IRValue::Int(_) => {
                            code.push_str(&format!("    printf(\"%lld\", (long long){});\n", expr));
                        }
                        IRValue::ConstantFloat(_) | IRValue::Float(_) => {
                            code.push_str(&format!("    printf(\"%f\", {});\n", expr));
                        }
                        IRValue::ConstantBool(_) | IRValue::Bool(_) => {
                            code.push_str(&format!("    printf(\"%s\", {} ? \"true\" : \"false\");\n", expr));
                        }
                        IRValue::Variable(var_name) => {
                            // For TauraroValue variables, we need to check the type_tag and access the appropriate data member
                            code.push_str(&format!("    switch ({}.type_tag) {{\n    case 0: printf(\"%lld\", (long long){}.data.int_val); break;\n    case 1: printf(\"%f\", {}.data.float_val); break;\n    case 2: printf(\"%s\", {}.data.string_val); break;\n    case 3: printf(\"%s\", {}.data.bool_val ? \"true\" : \"false\"); break;\n    default: printf(\"%s\", \"<unknown\"); break;\n    }}\n", var_name, var_name, var_name, var_name, var_name));
                        }
                        _ => {
                            code.push_str(&format!("    printf(\"%s\", {});\n", expr));
                        }
            ...
            IRInstruction::GetAttr { dest, obj, attr } => {
                // Handle built-in module attributes
                let code = match (obj, attr.as_str()) {
                    (IRValue::Variable(ref module_name), "platform") if module_name == "sys" => {
                        format!("    {}.type_tag = 2; {}.data.string_val = TAURARO_SYS_PLATFORM;  // Get attribute\n", dest, dest)
                    },
                    (IRValue::Variable(ref module_name), "version") if module_name == "sys" => {
                        format!("    {}.type_tag = 2; {}.data.string_val = TAURARO_SYS_VERSION;  // Get attribute\n", dest, dest)
                    },
                    (IRValue::Variable(ref module_name), "name") if module_name == "os" => {
                        format!("    {}.type_tag = 2; {}.data.string_val = TAURARO_OS_NAME;  // Get attribute\n", dest, dest)
                    },
                    (IRValue::Variable(ref module_name), "pi") if module_name == "math" => {
                        format!("    {}.type_tag = 1; {}.data.float_val = TAURARO_MATH_PI;  // Get attribute\n", dest, dest)
                    },
                    (IRValue::Variable(ref module_name), "e") if module_name == "math" => {
                        format!("    {}.type_tag = 1; {}.data.float_val = TAURARO_MATH_E;  // Get attribute\n", dest, dest)
                    },
                    (IRValue::Variable(ref module_name), "tau") if module_name == "math" => {
                        format!("    {}.type_tag = 1; {}.data.float_val = TAURARO_MATH_TAU;  // Get attribute\n", dest, dest)
                    },
                    _ => format!("    {}.type_tag = 2; {}.data.string_val = {}.{};  // Get attribute\n", dest, dest, self.ir_value_to_c_expr(obj)?, attr)
                };
                Ok(code)
            }
